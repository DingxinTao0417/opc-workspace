...
> 当前代码基线为 app v0.1.0 / API v1 / SQLite schema v27。Project/Task/Actor、Client 本地事实、Focus（含可选日期范围回顾）、Today、设置与受控工作区头像、统一本地搜索、备份恢复/业务 JSON、Inbox/Reminder/Task 编排，以及显式 follow-up Artifact、Task 阻塞、Task 临期、备份四类操作失败、数据库启动/迁移失败和 Sidecar 启动失败的系统维护来源投影已接通；Focus 高级分析/原生反馈、客户外部来源、完整日志/恢复页、重复/原生通知、本地 Agent、数据导入/含文件导出、回访/财务仍是规划。

## 阅读顺序与事实优先级

1. [产品需求文档（PRD v7.2）](opc-workspace-PRD.md)：产品范围、版本边界、数据/API 目标契约和当前状态。
