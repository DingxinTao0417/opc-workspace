...
> 当前代码基线为 app v0.1.0 / API v1 / SQLite schema v37。v9.51 已完成内容日历 CC1 数据契约与业务导入导出覆盖；API/UI 仍待。v9.50 已完成路线图 R3 首个界面纵切：`/roadmap` 使用真实本地里程碑数据，提供季度/状态浏览、新建、Project 跳转、归档恢复和加载/空/错误状态；编辑、拖拽和本地事件仍待。v9.49 已完成路线图 R2 API，v9.48 已新增数据契约和业务导入导出覆盖。v9.47 修正 PRD 总览中的历史遗留状态：固定原生快捷键及其运行诊断、本地 Client Followup 的客户详情/Today/Inbox 入口、Focus 的 SQLite 设置均已交付；自定义快捷键、托盘/通知、外部客户来源和财务仍为后续。v9.46 已将文档当前基线统一校准到 schema v35；该版本只新增 Client Followup，不改变任务、项目、设置、Focus、桌面、Inbox、本地 Agent 或发票契约。v9.45 继续客户回访 C6：客户详情刷新到 inactive 后会关闭本窗口的新增、编辑和重排草稿，不能带着旧表单继续计划；Sidecar 仍是最终写入门禁。v9.44 包含 planned 的详情列表每 15 秒刷新 Sidecar 当前时钟与到期事实，终态专属历史不轮询。v9.43 的列表 `meta.server_now` 与服务端逾期筛选使用同一 Sidecar 时钟，客户详情不再使用浏览器时钟标记逾期。v9.42 继续停用客户收口：客户详情隐藏安排、编辑、重排与完成时下一计划，保留既有计划的完成、跳过、确认取消；Sidecar v9.41 仍拒绝所有计划类写入，恢复为 `active/lead` 后才重新允许。v9.40 的“仅已逾期”筛选由 Sidecar 当前时钟按纳秒边界和 `planned` 状态计算，可与负责人组合并严格分页；浏览器不再负责决定筛选结果。v9.39 已交付待回访责任的人员停用保护，v9.38 已交付状态与负责人组合筛选，v9.37 已交付状态筛选，v9.36 已保证计划墙上时间按保存的 IANA 时区显示和保存，夏令时前跳不存在的时刻被拒绝，回拨重复时刻固定选择较早物理时刻。v9.35 已交付完成时可选原子安排下一次本地计划，Today/Inbox 不重复回访命令。v9.27 的命令面板/新建任务原生快捷键只发送固定 action，注册失败降级为应用内快捷键。v9.25 的本地 Agent Adapter 仍只允许代码所有清单，平台隔离未验证时固定不可执行，不创建 agent Actor/Assignment/Run。T-20 分层质量门禁继续有效；当前 Windows 主机仍缺少 MSVC `link.exe` 和 Windows SDK。

## 阅读顺序与事实优先级

1. [产品需求文档（PRD v9.51）](opc-workspace-PRD.md)：产品范围、版本边界、数据/API 目标契约和当前状态。
...
| 模块           | 当前状态                                                                                                                         | 目标版本                  | 文档                                 |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | ------------------------------------ |
| 今日工作台     | 部分完成（T-06A–H 日期编排、执行、行内管理、截止风险快捷筛选及客户回访待办已交付）                                               | v0.1                      | [today.md](modules/today.md)         |
...
| 路线图           | R2 数据/API 与 R3 首个界面纵切完成；编辑、拖拽与本地事件待开发                    | v0.3     | [roadmap.md](modules/roadmap.md)                   |
