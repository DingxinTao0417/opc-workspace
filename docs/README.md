...
> 当前代码基线为 app v0.1.0 / API v1 / SQLite schema v39。v9.68 已交付内容日历拖拽即时预移、IANA 时区归日和失败回滚；不调用外部发布平台。其余历史版本说明保持不变。

## 阅读顺序与事实优先级

1. [产品需求文档（PRD v9.68）](opc-workspace-PRD.md)：产品范围、版本边界、数据/API 目标契约和当前状态。
...
| 路线图           | R2/R3/R5 完成，R4 同季度排序、年度跨季度移动与季度内精确日期拖拽已交付                               | v0.3     | [roadmap.md](modules/roadmap.md)                   |
