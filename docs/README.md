...
> 当前代码基线为 app v0.1.0 / API v1 / SQLite schema v28。Project/Task/Actor、Client、Focus、Today、设置/头像、搜索、备份恢复、业务 JSON/含文件 ZIP 导出与空工作区安全 JSON 导入、Sidecar 日志、Inbox/Reminder/Task 编排及已登记来源投影已接通；Focus 原生反馈、客户外部来源、Tauri 壳日志/恢复页、重复/原生通知、本地 Agent、含文件/冲突合并导入、回访/财务仍是规划。

## 阅读顺序与事实优先级

1. [产品需求文档（PRD v8.6）](opc-workspace-PRD.md)：产品范围、版本边界、数据/API 目标契约和当前状态。
...
| 数据、受控文件、备份与恢复 | 迁移、Artifact store、备份完整闭环、失败 Inbox、业务 JSON/含文件 ZIP 导出与空工作区安全 JSON 导入已交付；含文件/冲突合并导入待实现 | v0.1；高级配置 v0.3 | [data-management.md](modules/data-management.md)   |
...
- Task file Artifact、Client Attachment、Project Attachment 与 Workspace Avatar 只保存在 Sidecar 声明的同一受控目录并经鉴权 API 下载；受控根通过身份 marker、进程锁、耐久同步与 quarantine 防止错库、双写和误删。应用已能管理 SQLite+active files 内部备份、含文件业务 ZIP 导出及无文件业务 JSON 安全导入；含文件包导入仍未实现。
