...
> 当前代码基线为 app v0.1.0 / API v1 / SQLite schema v41。v9.73 已把未删除的本地客户备注、会议和 Project 生命周期系统活动接入右侧客户动态；不调用外部服务，也不伪造线上行为。其余历史版本说明保持不变。

## 阅读顺序与事实优先级

1. [产品需求文档（PRD v9.73）](opc-workspace-PRD.md)：产品范围、版本边界、数据/API 目标契约和当前状态。
