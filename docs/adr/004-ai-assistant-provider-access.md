...
- 远程请求外发代码所有系统提示、该次会话预算内历史和当前用户输入；后续 ADR-006 增加用户已确认长期记忆注入。不读取任务/项目/客户等业务对象、不外发整库、无知识库、无遥测/用量上报。模型回复是上游返回值，不作为下一请求之外的额外外发。
- 模型只输出文本建议与结构化建议块（标题/描述/截止）；用户在确认卡片中再次编辑的字段直接由本地任务 API 落地，不二次发送给模型。

### 3. 语义建任务与人工确认

- 系统提示词由 Sidecar 代码所有（常量），指示模型识别到任务意图时先自然语言确认，再于回复末尾输出结构化块：`[opc:task]{title,description?,due?}[/opc:task]`（标题必填，描述/截止尽力推断）。
- 前端隐藏结构化协议，把正常闭合或常见的重复开始标记错误闭合解析为**可编辑待确认建议卡片**；只有结构块时补自然语言确认；缺标题时确认不可用；卡片可丢弃。
...
- 结构化块缺失/JSON 非法时降级为纯文本回复，不创建任务；模型输出视为不可信预览，不直接成为业务事实。任务创建成功但消息挂接失败时，前端保留 task ID 并只重试幂等挂接，禁止重复建任务。
...
- Provider 不可用、超时、限流、无密钥或提示超限 → 稳定大写错误码（`AI_PROVIDER_NOT_FOUND / AI_ENDPOINT_INVALID / AI_PROVIDER_BUSY / AI_STREAM_ERROR / AI_GENERATION_TIMEOUT / AI_PROMPT_TOO_LARGE / AI_KEY_UNAVAILABLE` 等）。
