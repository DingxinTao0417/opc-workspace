...
- `/clients/:id` 展示基础资料、真实关联项目列表及分页、状态调整、危险区、本地活动时间线、客户附件、“本地联系人”和本地回访管理。时间线区分可编辑人工记录与只读 Project 生命周期事实，并隐藏内部 Workflow Event ID。用户可选择已有 active person，或以联系人名称为默认值原子新建并关联；解除必须填写原因，历史按需展开。回访区可分页读取计划/终态，创建、编辑、完成、跳过、确认取消和重排均复用版本化 API；inactive 客户保留完成、跳过、取消既有计划的收口路径，但安排、编辑、重排或完成时附带下一计划会返回明确冲突，须先恢复客户状态；界面明确提示这不会创建账号、发送消息或授予访问权。
...
4. 停用与恢复只改变 `status`，不删除资料、不解除 Project，也不改变 Invoice 引用。停用后，既有 planned 回访仍可完成、跳过或取消以保留收口与历史；但不能新建、编辑、重排，或在完成时创建下一条回访计划。恢复为 `active` 或 `lead` 后才重新允许这些计划类写入。
