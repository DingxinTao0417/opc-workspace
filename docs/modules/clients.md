...
>
> 当前基线：app v0.1.0 / API v1 / SQLite schema v18。客户基础事实由 schema v10 引入，schema v18 追加本地活动事实。v0.1 的资料 CRUD、Project 客户关联、人工备注/会议时间线与可审计软删除已交付，模块仍为**部分完成**；附件、Actor 显式关联、回访和财务聚合尚未交付。
...
当前状态为**部分完成**，已经具备可运行的客户资料事实层、列表、基础详情、Project 客户关联和本地活动时间线。
...
- `/clients/:id` 展示基础资料、真实关联项目列表及分页、状态调整、危险区和本地活动时间线。用户可新增/编辑备注或会议、选择发生时间、查看分页和删除历史，并在填写原因后确认软删除；附件/回访和发票/收入区仍明确标为后续版本。
...
- 当前活动只覆盖人工备注和会议；尚无邮件、文件、回访或其他业务来源写入 `system_reference` 的投影器。
...
3. **v0.1 本地活动（已实现）与附件（待实现）**：人工备注/会议、可追溯时间线和软删除审计已交付；受控附件和来源投影仍待实现，不接线上行为。
4. **Actor 显式关联（待实现）**：联系人转为/关联 person 的确认流程和语义提示。
5. **v0.4 业务增强（待实现）**：回访计划、本地提醒、发票和财务聚合；第一版仍不自动对外发送。

## 验证与验收边界

当前自动化测试覆盖 schema v9→v10 与 v17→v18 数据保留、索引/约束/trigger/外键、Client CRUD/校验、分页/搜索/状态/排序、活动创建幂等、列表/详情/版本化编辑/软删除/删除历史/system reference 只读、聚合版本与最近活动传播、Project 关联传播及 Invoice 删除冲突。Web 侧覆盖 Client 与 Activity API 规范化、Query hooks、活动表单/时间线/删除历史、客户列表/详情及 Project 客户选择/筛选。

以下仍不能据此宣称完成：真实浏览器键盘/焦点/窄屏验收、大数据量分页性能、Attachment、Activity 来源投影、Actor 关联、回访、财务或任何线上互动。
