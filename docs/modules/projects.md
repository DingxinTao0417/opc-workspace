...
>
> 版本边界：项目资料、基础生命周期、任务聚合与树/平铺视图、Client 客户关联、项目笔记、项目附件、所属 Task Artifact 聚合、活动时间线、显式 follow-up、Task 阻塞、Task 临期和 Project 完成节点→Inbox 已实现，模块仍为**部分完成**；项目级筛选、可见分页、高级分析和其他真实里程碑尚未交付。
...
- 项目详情通过串行分页读取全部关联任务，默认筛出根任务并复用 Task `parent_task_id` 分页查询按需展开任意深度子任务；可切换平铺列表查看全部任务和父任务上下文。详情同时支持资料编辑、从项目内新建并预选项目的任务、派生进度/工时、状态操作、归档恢复，以及带二次确认的永久删除；版本冲突会刷新详情事实。
...
- 项目详情内的任务列表和任务表单项目选项会按每页 100 条串行拉取全部结果，避免静默截断；任务区已提供树/平铺切换，子任务展开同样按每页 100 条读取并显示分页控件。但项目级状态筛选、完整集合的可见分页或内嵌 Assignment/Submission 控件仍未实现，大数据量下的请求次数与响应性能仍待验证。Task 标签、父子、并发版本、Assignment 与 manual Submission/Artifact 验收继续由共享任务详情承载。
...
- 项目详情任务树已复用 Task 父子事实、展开分页与共享详情入口；后续按项目范围继续复用状态筛选和完整集合可见分页。标签、Assignment、完整生命周期和 Artifact 验收仍统一在共享任务详情处理，不在 Project 复制写命令。
...
| 模块      | 当前与后续协作方式                                                                                                                                                                                |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 任务      | 当前已支持项目选择、项目名展示、项目任务树/平铺列表、Task 标签/父子/版本、派生进度/工时、共享任务详情中的 D2 产出与验收，以及项目详情的只读 Artifact 聚合；项目级筛选和完整集合可见分页仍待接入。 |
...
3. **任务与工时协作（部分实现）**：项目选择、串行分页拉全项目选项与项目任务、`project_name`、Task 事实版本、派生进度和 `actual_minutes` 已接通；Focus Core 已接入 Task 工时传播。项目详情已复用父子任务树、按需分页展开、平铺列表和共享任务详情；项目级筛选、完整集合可见分页、大数据量性能和项目级 Focus 历史仍待实现。
...
- 超过 100 条项目任务与项目选项已避免截断；项目任务树/平铺切换和子任务展开已有自动化覆盖。完整集合可见分页/项目级筛选、大数据量性能和项目级 Focus 历史/分析仍待验收。Focus → Task → Project 的整数分钟传播已有自动化覆盖。
