...
> 当前基线：app v0.1.0 / API v1 / SQLite schema v27（2026-08-28）；schema v27 不改变 Reminder 契约。
>
> 版本边界：T-11A3 一次性本地 Reminder 已交付。重复提醒、系统原生通知、远程推送、邮件/短信、业务来源自动建提醒和用户可配置扫描频率仍未实现。

导航：[文档中心](../README.md) · [整体功能架构](../functional-architecture.md) · [PRD v6.9](../opc-workspace-PRD.md) · [收件箱](inbox.md) · [桌面平台](desktop-platform.md)
...
迁移为纯加法，不创建 demo Reminder，也不改写 schema v13 的 Task、Inbox、Client、Project、Focus 或 Artifact 事实。其后的 schema v15–v26 增加 Inbox 编排、app_settings、任务保存视图、客户/项目扩展以及 Artifact/Task/系统维护 Inbox 来源；v23–v26 不改变 Reminder 契约，后续迁移必须从 `027_*` 继续。
