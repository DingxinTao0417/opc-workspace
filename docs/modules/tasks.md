...
> 实现状态截止：2026-08-27
>
> 版本边界：当前代码已有三态任务基础链路和项目关联；扩展状态、Actor 分派、产出和验收均为 v0.1 规划，Agent 执行为 v0.2 规划。
...
- 创建 API 校验字段并可接受 `Idempotency-Key`；服务端对规范化任务请求计算 SHA-256，保存首次任务响应与 `201` 状态。同键同请求在任务编辑或删除后仍安全重放，同键不同请求返回 `409 IDEMPOTENCY_CONFLICT`；项目 ID 存在时还会验证外键和归档状态。
- 任务列表、详情、编辑和状态更新读取会左连接项目并返回可选 `project_name`；任务行展示项目名而不是裸 `project_id`。
- 创建任务或把任务改入项目时会验证项目未归档；归档项目返回 `409 PROJECT_ARCHIVED`。既有关联任务仍可编辑，未改变的归档项目关联不会被误拒绝。
- 前端提供按 `in_progress / todo / done` 分组的列表、新建弹窗、客户端关键词搜索、完成/恢复按钮以及加载/空/错误/重试状态。
- 任务页和今日页共用任务详情弹窗；点击任务行可查看并编辑标题、描述、优先级、项目、计划日期、截止时间和预计时长，也可经显式二次确认删除。
- 新建和编辑任务会按每页 100 条串行拉取全部非归档项目供选择；从项目详情新建时自动预选该项目。任务编辑可清除项目，并保留当前已归档项目选项，避免读取后丢失原关联。
- 通用编辑接口拒绝 `status` 等生命周期字段，状态仍由独立状态接口处理。

### 已知缺口

- 任务总览页仍固定读取第一页 100 条，搜索也只覆盖已加载数据，没有分页或服务端搜索 UI；这不影响下述项目详情为单个项目串行拉全任务的行为。
- 状态按钮只在 `done` 与 `todo` 间切换；不能从 UI 开始任务，也没有阻塞、待验收、取消、返工等状态。
- 项目选择器会拉取全部非归档项目，但没有搜索或可见分页，大数据量性能仍待验证；客户 CRUD 尚未实现，不影响任务通过 Project 获取已有项目上下文。
- 项目详情会串行分页拉取全部项目任务，但没有可见分页、状态筛选或任务树，大数据量性能仍待验证。
- 没有父子任务、完成条件、验收策略、Assignment、Artifact、Workflow Event 或乐观并发版本。
- 没有批量操作、拖拽排序、看板、任务依赖和绑定专注工时；`actual_minutes` 字段与项目求和已存在，但专注会话尚不会自动累计。
- Task 与 Project 创建的 schema v4 快照契约尚未扩展 Actor 作用域、过期清理或 Workflow Event 去重；这些仍随本地工作编排实现。
...
- `POST /tasks` 的可选 `Idempotency-Key` 保存规范化请求摘要、首次任务响应与 `201` 状态；同请求可在资源编辑/删除后稳定重放，异体复用 key 返回 409，旧版无快照记录拒绝不安全重放。
- `GET /tasks`、`GET /tasks/:id` 以及编辑/状态更新响应在有关联时返回 `project_id` 和 `project_name`；创建响应本身只保证 `project_id`，随后列表/详情刷新取得项目名。
- schema v5 trigger 会在任务插入/删除、项目关联、状态或 `actual_minutes` 变化时原子递增受影响项目的 `version`，使项目聚合 `ETag` 失效；普通任务资料编辑不会无故递增项目版本。
...
- 扩展状态 CHECK 必须通过 `006_...` 起的递增迁移重建表，不能回写已发布的初始迁移或 schema v3–v5 迁移。
...
| 项目 | 当前已支持新建/编辑任务时选择 Project、返回 `project_name`，并由项目查询实时派生进度与任务 `actual_minutes` 合计；归档项目拒绝新关联，聚合变化会递增项目版本。 |
...
1. **任务基础事实**：详情、非状态字段 PATCH、删除确认、项目选择和 `project_name` 已交付；继续完成分页/筛选/排序 UI、标签、父子任务、批量操作和基础乐观锁，暂时保留三态兼容。
...
- 归档 Project 不能接受新任务或改入任务，既有关联任务仍可编辑；任务聚合变化会使旧 Project `If-Match` 冲突。
