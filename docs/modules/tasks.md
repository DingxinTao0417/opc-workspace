...
> 实现基线：app v0.1.0 / API v1 / SQLite schema v19（2026-08-28）；Task D2 结构仍由 schema v9 引入，schema v11 通过 Focus 精确秒数账本向 `actual_minutes` 追加完整分钟；schema v17 新增保存视图表，schema v18–v19 新增客户活动与附件，均不改写 Task 表。
>
...
导航：[文档中心](../README.md) · [整体功能架构](../functional-architecture.md) · [PRD v5.3](../opc-workspace-PRD.md) · [Actor 与分派](actors.md) · [数据管理](data-management.md)
...
仍属后续：Task/业务来源投影与自动创建 Reminder、Agent Adapter/Run、自动生成 Artifact、Focus Session 历史/周报/高级分析、Client 外部来源/Actor 关联/回访/财务，以及 AI 助手与知识库；受控文件基础备份恢复、Inbox 批量拆分/Assignment/自动结清、一次性 Reminder、已有 Task 关系、Focus Core 工时持久化、Client 基础资料/Project 关联/人工活动时间线和受控附件已经交付。
