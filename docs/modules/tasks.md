...
导航：[文档中心](../README.md) · [整体功能架构](../functional-architecture.md) · [PRD v9.22](../opc-workspace-PRD.md) · [Actor 与分派](actors.md) · [数据管理](data-management.md)
...
- 不改变 Inbox 表或 `inbox_item_tasks.is_required`，不创建 demo 数据。schema v31 已追加 Project→Client Activity 来源约束，schema v32 已扩展 Reminder；下一迁移只能从 `033_*` 追加。
