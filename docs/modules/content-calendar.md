...
> 目标版本：v0.3。该模块提供本地内容排期与准备工作协同，不属于 v0.1。当前已完成 CC1 数据契约、CC2 API 与 CC3 首个 React 工作区（schema v37）；月格拖拽与详情编辑仍待开发。
...
- 当前为按排期时间排序的月度列表，不是月格；详情、编辑、改期、发布确认、准备任务编辑或拖拽排期仍未交付。
...
3. **CC3 基础界面（部分完成）**：月份切换、状态筛选、本月列表、新建和完整反馈状态已交付；月格、详情与编辑仍待。
