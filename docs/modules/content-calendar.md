...
> 目标版本：v0.3。该模块提供本地内容排期与准备工作协同，不属于 v0.1。当前已完成 CC1 数据契约、CC2 API、CC3 首个 React 工作区及 CC4 详情编辑/改期/发布确认（schema v37）；月格拖拽与准备任务编辑仍待开发。
...
- 当前为按排期时间排序的月度列表，不是月格；准备任务编辑或拖拽排期仍未交付。月度工作区的新建表单要求计划日期，避免新建后因不属于当前月范围而不可见；无排期条目仍可由 API 管理，专用 Inbox 待 CC5。
