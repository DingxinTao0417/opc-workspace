...
- 当前为包含前后月日期的固定六周月格；前端以每页 100 条稳定连续读取整个可见范围，聚合全部已加载页面，并在后台加载后续页时显示读取进度。可编辑条目可拖到任一可见日期，包括相邻月份日期。拖拽保留条目原 IANA 时区和墙上发布时间，并使用当前版本写入；DST 不存在时间会阻止写入并提示改用详情表单。
...
- 内容正文、外部发布、准备任务阻塞的专用内容来源、乐观拖拽预移和真实浏览器大数据验收仍待后续阶段。
...
4. **CC4 排期交互（已完成当前范围）**：CC4-A 的详情编辑、表单改期、人工发布确认、乐观锁刷新和错误反馈，以及 CC4-B 的六周月格、跨日/可见跨月拖拽、IANA/DST 保持和表单替代均已交付。乐观视觉预移留到后续交互增强。
