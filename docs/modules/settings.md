...
> 文档状态：部分实现；当前 schema 76。schema v42 将 `app_settings` 契约升为 v2，v43/v44 引入容量历史与计划备份；schema 071 的 Agent 关联、072–073 的 AI 审批、074–075 的 Agent Run 执行身份/交付状态及 076 的会话计划修订均不改变设置值 schema。本地 Agent 已有 Windows 内置 Runner，设置提供真实健康展示与显式启停，不自动创建 Run。其他设置功能与限制见各节。
...
- 窄屏设置模块导航允许横向滚动；当前模块在打开、切换和导航尺寸变化后必须保持在可视区，不能只切换内容而把活动高亮留在屏外。
...
- 通用：默认首页、右侧概览、减少动效和“关闭窗口时隐藏到托盘”。“显示右侧概览”保存宽屏默认值；默认启用时，主工作区右上角提供当前会话的收起/展开按钮，不写 SQLite、不改变设置 draft，也不暂停正在运行的 Focus Session。托盘项点击即通过固定 Tauri command 预览；保存写入 SQLite，取消按顺序恢复 committed，浏览器开发模式只保存偏好。
- 外观：亮色与暗色主题，支持保存前预览。
- 专注：时长、休息时长、循环次数、自动开始休息/专注和结束提示音。
- 人员与责任：从真实 `/api/v1/actors` 读取固定 owner/system 与 person，支持新建/编辑/启用/停用 person，并可单独编辑 owner 展示名称。已保存 person 行可“交给智能体”，只暂存 person ID、`/ai?settings=actors` 和固定提示词，仍需用户在 AI 页重新授权并确认；不会复制备注/metadata 或直接修改人员。该模块每次操作独立保存，不经过设置弹窗的全局保存按钮。
- 自动化：从真实 `/api/v1/automations` 读取五个稳定预设；支持依赖状态、优先级或当地时间/IANA 时区即时服务端预览、规则配置独立保存、启停、下一次运行、最近 Run、空/加载/错误和失败手动重试。四个预设可用，Agent 失败预设因事件未接入仍 unavailable；该模块使用独立规则版本，不经过 `app_settings` 或设置弹窗全局保存按钮。H4-E 增加 `/settings/automation?rule=<uuid>` / `?run=<uuid>` 的同组件页面：直接选择规则/不可变运行、切换尝试链并返回来源对话，查看不执行操作；无效/缺失目标不改选其他记录。详见 [自动化定位契约](automation.md#规则与运行精确定位h4-e已实现源码与确定性测试)。
- 本地 Agent：从真实 `/api/v1/agent-adapters` 读取代码所有清单，支持登记、实际健康/安全闸门、诊断、显式启用与停用、加载和错误重试。Windows builtin 依据 ADR-027 矩阵可返回 healthy/verified/ready；不把健康结果固定显示为隔离未验证。登记/诊断不自动启用，启用事务才幂等建立关联 agent Actor；打开设置不分派、不创建 Run、不调用模型。已登记 Adapter 卡片可“交给智能体”做无授权排查交接：只暂存 Adapter ID、`/ai?settings=agent` 和当前页面状态提示，不复制配置或执行器详情，不调用 Adapter API，也不授予检查、启停、任务分派或 Agent Run 启动能力。该模块使用 Adapter 自身 `If-Match`，不经过 `app_settings` 或全局保存。
- 数据与备份：可预览并保存 1–100 GiB 低空间提醒阈值，默认 1 GiB、下一轮扫描生效；可手动刷新数据库/受控文件/备份三个逻辑位置的容量，展示健康、低空间和局部不可用状态但不展示路径或探测错误。可即时预览并版本化保存每日时间、IANA 时区和 1–365 份自动保留数；取消恢复 committed，默认关闭。从真实 `/api/v1/backups` 读取本机备份并完成创建、计划、校验、演练、恢复、删除；手工/计划创建和导入/恢复内部回滚包在写入备份 staging 前通过仅探测 backup root 的容量准入。计划保留只清理超限 scheduled 包，不清理手工/回滚/pending restore。启动恢复诊断显示待重启、本次已应用、清理残留、失败隔离或无效记录。可分别下载或导入版本化业务 JSON 与包含 manifest/活动受控文件的 ZIP；当前 schema 空目标或零主键冲突目标可安全应用。模块顶部可“交给智能体”做无授权排查交接：只暂存 `/ai?settings=data` 和页面级备份数量、计划状态、恢复诊断状态，不复制备份内容、路径、导入/导出文件或容量细节，不授予创建/校验/演练/恢复/删除/导入导出/保存计划/重启能力。
...
- 运行诊断成功读取后可“交给智能体”做无授权状态解释：只暂存 `/ai?settings=diagnostics`、环境/阶段/API 健康/版本兼容等脱敏枚举，不复制路径、日志、令牌、端口、诊断包或业务数据，也不授予重新检查、打开日志目录、生成诊断包或重启能力；诊断刷新或诊断包生成中禁用交接。
- AI 助手供应商设置里的已保存供应商卡片可“交给智能体”做无授权排查交接：只暂存 Provider ID、`/ai?settings=ai`、名称/模型和当前页面状态提示，不复制 Base URL、API 密钥、评测证据、会话正文或失败生成正文，不调用保存密钥、测试连接、删除、评测或重放 API，也不授予修改供应商配置能力。
...
- 命令面板可分别直达个人资料、通用、外观、专注、人员与责任、本地 Agent、自动化、数据与备份、运行诊断和关于模块；关闭设置后通用 Modal 恢复触发元素焦点。窄屏横向菜单会把请求或点击后的活动模块滚入最近可见位置，并在响应式尺寸变化时重新校正。
...
- 默认首页、右侧概览、减少动效、关闭窗口时隐藏到托盘和主题。右侧概览的设置值是宽屏默认；启用时可从主工作区固定按钮临时收起或恢复，当前会话态不覆盖保存值，窄屏继续按现有响应式规则不渲染该栏。
...
- “交给智能体”只用于把数据维护现状转成排查问题：AI 只收到页面级计数和状态枚举，进入 `/ai?settings=data` 后也不会自动执行任何维护动作；所有备份、恢复、删除、导入导出、计划保存和重启仍必须在原生页面人工点击和确认。
...
5. Sidecar 在数据库和 Artifact lease 打开前执行替换和最终复验；失败恢复旧资源并拒绝启动，成功后正常进入应用。全局恢复页会在数据库打开前显示白名单恢复/迁移阶段，可打开脱敏日志并安全重启；启动失败时也可在恢复页从已有备份安排恢复或跳过（见桌面模块）。
...
| GET /api/v1/diagnostics/storage               | **已实现**：只读即时容量兼容端点，不落历史样本；不返回卷 ID、路径、盘符或探测错误                                                                                                                                                                                        |
| POST /api/v1/diagnostics/storage/check        | **已实现**：显式按物理卷去重检查，并写入/更新无路径逻辑 scope 的当前 15 分钟样本                                                                                                                                                                                         |
| GET /api/v1/diagnostics/storage/history       | **已实现**：读取 1–30 天升序样本；设置页默认显示 7 天折线，样本保留 30 天                                                                                                                                                                                                |
...
| POST /api/v1/imports/business-data/preview    | **已实现**：strict 预检无文件 JSON；同 schema 返回表/总行数、目标行数、逐表主键重叠和 `replace_empty / append` 模式，跨 schema 返回旧/新方向 blocker；全程只读                                                                                                           |
| POST /api/v1/imports/business-data            | **已实现**：按 empty/append 使用不同确认头，维护写锁内先执行回滚包容量准入，再创建回滚备份并原子替换或追加；容量拒绝不改业务事实                                                                                                                                         |
| POST /api/v1/imports/business-package/preview | **已实现**：strict 预检含文件 ZIP 的 manifest、业务 JSON、文件全集/哈希和目标文件碰撞，并复用目标主键冲突、apply mode 和跨 schema 方向 blocker                                                                                                                           |
| POST /api/v1/imports/business-package         | **已实现**：按 empty/append 使用独立确认头、回滚包容量准入、回滚备份、文件无覆盖发布、DB 提交前正文复验与失败补偿                                                                                                                                                        |
...
- `rightOverviewCollapsed`：仅当前 WebView 会话内的布局状态；只在已启用右侧概览的宽屏上影响可见性，刷新后回到 committed/preview 的默认值。
...
- [今日](today.md)：默认首页、右侧概览、当前会话收起状态和减少动效影响展示，不改变今日数据口径或 Focus Session。
...
- “人员与责任”、Assignment，“数据与备份”的完整备份链、启动后恢复结果诊断、业务 JSON/含文件 ZIP 空工作区与同 schema 零主键冲突追加、只读冲突清单/schema 方向分类，以及脱敏诊断、Sidecar/Tauri 壳日志、桌面打开日志目录和含白名单恢复/迁移进度的全局启动故障恢复页已完成；运行诊断已展示固定原生快捷键注册状态和托盘运行时可用性。通知、实际冲突合并/UUID 重映射、跨 schema 升级导入、快捷键自定义和启动前备份选择待实现。
...
- **已验证（真实浏览器）**：数据与备份显示“近 7 天容量趋势”、物理卷逻辑组合、最新可用量和首个样本；手动重新检查成功，同一 15 分钟桶未重复新增卡片，页面无错误提示。
- 启动恢复诊断只显示规范 ID、时间、状态与计数；重新打开设置可恢复待重启门禁，本次恢复成功、清理残留、失败隔离与无效记录不会泄露本机路径或底层错误，也不会触发自动删除。恢复挂起时普通业务 API 仍返回 `RESTORE_RESTART_REQUIRED`，但只读恢复诊断保持可访问，供数据设置与 H5-E9 智能体续办入口解释当前门禁。
...
- [自动化定位页面](../../apps/web/src/pages/AutomationLocationPage.tsx)、[页面验收](../../apps/web/src/pages/AutomationLocationPage.test.tsx)
