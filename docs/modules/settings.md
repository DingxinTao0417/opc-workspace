...
- person UI 已明确说明不会发送或同步；停用受活动 Assignment、active Client contact 关联，以及 Client Activity/Attachment/Project Note/Project Attachment 历史外键保护，历史分派基础由 schema v7 建立并在当前 schema v31 延续。schema v12–v29 的其他迁移不改变 Assignment 约束；schema v30/v31 分别扩展 Task Submission 与 Client Activity 来源，不改变 Actor、Assignment 或设置表契约。
