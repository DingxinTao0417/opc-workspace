...
5. Sidecar 在数据库和 Artifact lease 打开前执行替换和最终复验；失败恢复旧资源并拒绝启动，成功后正常进入应用。启动失败会进入全局恢复页 v1，可打开脱敏日志并安全重启；备份选择和实时恢复进度仍待开发。
...
- “人员与责任”、Assignment，“数据与备份”的完整备份链、启动后恢复结果诊断、业务 JSON/含文件 ZIP 空工作区安全导入导出，以及脱敏诊断、Sidecar/Tauri 壳日志、桌面打开日志目录和全局启动故障恢复页 v1 已完成；通知、非空目标/跨 schema 冲突合并导入、快捷键和数据库打开前备份选择/实时恢复进度待实现。
...
- person UI 已明确说明不会发送或同步；停用受活动 Assignment、active Client contact 关联，以及 Client Activity/Attachment/Project Note/Project Attachment 历史外键保护，历史分派基础由 schema v7 建立并在当前 schema v30 延续。schema v12–v29 的其他迁移不改变 Assignment 约束；schema v30 只增加 `task_submissions.origin` 与父任务推进规则，不改变 Actor、Assignment 或设置表契约。
