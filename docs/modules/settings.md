...
> 文档状态：部分实现；当前 schema v17。v0.1-A 已交付由 schema v16 引入的版本化 `app_settings`、服务端 schema 清洗、GET/PATCH API、前端 Query 接入和旧设置按模块兼容迁移；schema v17 保存视图不改变设置契约。Focus Core 已完成设置运行态解耦，“人员与责任”已接真实 Actor API，“数据与备份”已接手动一致性备份创建/列表/校验、隔离恢复演练及重启前安全恢复，“关于”已接真实健康与版本事实。头像仍是仅保存在本地 WebView 的兼容 Data URL；受控头像文件、备份删除/导出和完整桌面诊断入口仍是后续范围。
...
- 数据与备份：从真实 `/api/v1/backups` 读取本机备份，支持可选说明的幂等创建、展示 schema/Artifact 数量/大小、显式重新完整校验、不触碰当前数据的隔离恢复演练，以及二次确认后创建自动回滚点并安排下次启动恢复；具备加载、空、错误、进行中与成功状态。该模块没有设置“保存”按钮，也不伪造尚未实现的删除或导出。
...
- 已有 Actor 设置页和手动备份创建/列表/校验/恢复演练/重启恢复，任务详情也已接负责人/审核人选择与分派历史；仍没有通知、备份删除/导出、快捷键、完整诊断或 Agent 设置页。
...
8. owner 关闭并重新打开应用；Sidecar 在打开正式数据库前应用恢复，失败时恢复旧数据库/objects。当前不自动重启应用，也不提供删除或 JSON 导出入口。
...
- “人员与责任”的 Actor 管理、任务详情 Assignment 入口，以及“数据与备份”的创建/列表/校验/恢复演练/重启恢复已完成；通知、备份删除/导出、快捷键和完整诊断模块待实现。
...
- “数据与备份”只在 Sidecar 完成 SQLite+Artifact 全量验证、隔离恢复演练或安全挂起恢复后显示相应成功；列表、空态、读取失败、创建中、创建失败、重新校验、演练中/失败、二次确认、挂起提示和 invalid 包均有明确状态。
