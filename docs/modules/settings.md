...
> 文档状态：部分实现；当前 schema v28。v0.1-A 已交付由 schema v16 引入的版本化 `app_settings`、服务端 schema 清洗、GET/PATCH API、前端 Query 接入和旧设置按模块兼容迁移；schema v27 已交付受控工作区头像文件、严格 multipart 原子保存、鉴权读取、旧 Data URL 一次性迁移及备份恢复；schema v28 不改变设置契约。Focus Core 已完成设置运行态解耦，“人员与责任”已接真实 Actor API，“数据与备份”已接一致性备份完整闭环与基础业务 JSON 下载，“关于”已接真实健康与版本事实，“运行诊断”已接脱敏桌面生命周期与版本兼容检查。数据导入、完整日志/诊断包和恢复页仍是后续范围。
...
- 运行诊断：联合 `/health` 与桌面 `sidecar_status` 展示浏览器开发/Tauri 环境、生命周期、app/API/schema 与版本兼容；支持重新检查、错误重试和复制脱敏摘要。桌面返回先经白名单规范化，`sessionToken`、`baseUrl` 和原始 `message` 不进入诊断对象或 UI。
...
- 已有 Actor 设置页、手动备份完整闭环、基础业务 JSON 下载和脱敏运行诊断，任务详情也已接负责人/审核人选择与分派历史；仍没有通知、数据导入、快捷键、完整日志/诊断包、恢复页或 Agent 设置页。
...
- 已提供重新检查服务和复制脱敏诊断摘要；手动恢复 Sidecar、打开日志与生成完整诊断包仍待实现。
...
- “人员与责任”的 Actor 管理、任务详情 Assignment 入口，“数据与备份”的创建/列表/校验/恢复演练/重启恢复/确认删除/业务 JSON 下载，以及脱敏“运行诊断”已完成；通知、数据导入、快捷键、完整日志/诊断包和恢复页待实现。
- **已完成**：UI store、Focus 页入口和命令面板均支持指定 activeModule；命令面板注册全部当前设置模块的直达入口。
- **已完成**：展示真实健康和版本信息，移除硬编码“关于”运行事实，并提供加载、失败重试、手动重新检查和只读页脚。
- **已完成**：运行诊断区分浏览器/Tauri 环境，白名单化桌面 Sidecar 状态，对照 health 版本并可复制脱敏摘要；命令面板可精确直达。
...
- “运行诊断”不展示或复制会话令牌、监听地址、原始错误、本地路径和业务正文；桌面状态畸形时拒绝使用，浏览器开发模式不伪造 Tauri 事实。
