...
> 文档状态：部分实现；当前 schema v28。设置持久化、受控头像、Focus 解耦、Actor、备份闭环、启动后恢复结果诊断、业务 JSON/含文件 ZIP 的空工作区安全导入导出、健康版本、诊断包 v1、Sidecar 脱敏轮转日志和桌面打开日志目录已交付。非空目标/跨 schema 高级导入、Tauri 壳日志和数据库打开前恢复页仍是后续范围。
...
- 已提供重新检查服务、复制脱敏诊断摘要、下载诊断包和桌面打开日志目录；该 command 不接受路径，浏览器模式禁用。手动恢复 Sidecar 与 Tauri 壳自身日志仍待实现。
...
- “人员与责任”、Assignment，“数据与备份”的完整备份链、启动后恢复结果诊断、业务 JSON/含文件 ZIP 空工作区安全导入导出，以及脱敏诊断、Sidecar 日志与桌面打开日志目录已完成；通知、非空目标/跨 schema 冲突合并导入、快捷键、Tauri 壳自身日志和数据库打开前恢复页待实现。
