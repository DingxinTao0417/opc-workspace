...
- `OPC_LOG_DIR` 已用于启动前安全故障 journal 和下一次健康启动补偿；设置“运行诊断”已能白名单化展示桌面生命周期/版本、复制基础脱敏摘要并下载诊断包 v1。诊断包包含版本/平台、SQLite 健康/迁移和维护错误码汇总，但 Go/Tauri 完整日志仍未落盘、轮转，原始日志不进入该包。
...
当前已交付其中的启动故障最小安全层：Tauri 已传入 `OPC_LOG_DIR`，Sidecar 也支持 `--logs` 和数据库同级默认 `logs/`；数据库启动/迁移及 Sidecar 启动失败只写白名单 journal，下一次成功启动在 ready 前补偿为 Inbox Item。设置诊断页与脱敏诊断包 v1 已交付；完整进程日志、request ID 串联、轮转和打开日志目录仍是目标功能，不得标记为已交付。
...
- 已接通 OPC_LOG_DIR 的启动故障 journal、原子更新、损坏隔离和 ready 前补偿；继续实现 Tauri/Go 脱敏日志、轮转和 request ID。
- 增加诊断页、打开日志和脱敏摘要。
- 数据库启动/迁移、Sidecar 启动和备份恢复失败已接 maintenance 状态；继续接运行期数据库不可写等故障。
