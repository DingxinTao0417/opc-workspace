...
- `OPC_LOG_DIR` 已用于启动前安全故障 journal、下一次健康启动补偿和 Go Sidecar 脱敏轮转日志；设置“运行诊断”可白名单化展示桌面生命周期/版本、复制基础脱敏摘要并下载诊断包 v1。诊断包包含版本/平台、SQLite 健康/迁移和维护错误码汇总，原始日志不进入该包。Tauri 壳日志和打开日志入口尚未实现。
...
- 诊断页、脱敏摘要和诊断包 v1 已完成；继续增加打开日志目录。
- 数据库启动/迁移、Sidecar 启动、备份恢复和运行期数据库操作失败已接 maintenance 状态；继续接主动低磁盘阈值等故障。
