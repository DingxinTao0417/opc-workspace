...
- `OPC_LOG_DIR` 已用于启动前安全故障 journal 和下一次健康启动补偿；设置“运行诊断”已能白名单化展示桌面生命周期/版本并复制基础脱敏摘要，但 Go/Tauri 完整日志仍未落盘、轮转，也没有完整脱敏诊断包。
...
当前已交付其中的启动故障最小安全层：Tauri 已传入 `OPC_LOG_DIR`，Sidecar 也支持 `--logs` 和数据库同级默认 `logs/`；数据库启动/迁移及 Sidecar 启动失败只写白名单 journal，下一次成功启动在 ready 前补偿为 Inbox Item。完整进程日志、request ID 串联、轮转、诊断页和打开日志目录仍是目标功能，不得标记为已交付。
