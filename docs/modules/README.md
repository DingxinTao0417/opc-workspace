...
导航：[文档中心](../README.md) · [整体功能架构](../functional-architecture.md) · [PRD v6.0](../opc-workspace-PRD.md)
...
- [项目管理](projects.md)（基础纵切、Client 选择/筛选、可编辑人工笔记、Task Artifact 产出聚合和追加式项目活动时间线已交付；项目附件与 Inbox 来源投影仍待开发）
...
- [数据、受控文件、备份与恢复](data-management.md)（Task Artifact/Client Attachment store、手动一致性备份完整闭环、桌面安全重启和基础业务 JSON 导出已交付；迁移前备份、恢复诊断、导入/含文件外部包仍待开发）
...
- 受控文件能力必须同时说明数据库身份绑定 marker、进程锁、staging/objects/trash/quarantine、跨 Task Artifact/Client Attachment 的 object ID 唯一、相对路径、大小/哈希、耐久同步、软删与聚合硬删；不能把目录预留写成备份已完成。
