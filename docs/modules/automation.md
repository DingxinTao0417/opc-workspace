...
- Workflow Event、Task 六状态命令、manual Submission/Artifact 验收、独立手工 Inbox Item，以及一次性 Reminder/到期 Inbox 投影已经交付，可作为未来自动化的只读事实和动作目标基础；业务来源到 Inbox 的投影、规则驱动创建 Reminder、自动化专用事件订阅与执行器仍处于规划阶段。
...
1. **A1 前置事实层（部分完成）**：Workflow Event、Task 生命周期、Submission/Artifact 审计、独立手工 Inbox Item 和一次性 Reminder/到期投影已交付；继续实现业务来源投影与自动化消费约定。
