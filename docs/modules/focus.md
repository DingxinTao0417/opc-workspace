...
> 当前基线：app v0.1.0 / API v1 / SQLite schema v28（2026-08-28）。Focus 结构仍由 schema v11 引入；schema v12–v28 不改 Focus 表契约。Focus Core v0.1-A/B/C、v0.1-D1（历史与周期报告）、D2a（Task 详情记录），以及 D2b 的本地日期范围回顾、项目/当前标签时间分布、最佳小时段与二维热力图已经交付；原生桌面反馈仍属后续。
