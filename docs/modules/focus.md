...
> 当前基线：app v0.1.0 / API v1 / SQLite schema v28（2026-08-28）。Focus 结构仍由 schema v11 引入；schema v12–v28 不改 Focus 表契约。Focus Core v0.1-A/B/C、v0.1-D1（历史与周期报告）、D2a（Task 详情记录），以及 D2b 的本地日期范围回顾、项目时间分布与最佳小时段已经交付；标签分布、二维热力图和原生桌面反馈仍属后续。
...
- 白噪音、网站屏蔽、系统勿扰、原生通知、标签分布和“周几 × 小时”二维热力图均未交付。
...
- 报告返回区间 distinct Session、精确秒数、向下取整分钟、逐日事实、项目分布、0–23 点小时分布、截至 `date_to` 的当前连续天数，以及区间内最长连续天数；零事实日和零事实小时保留在序列中。
...
- 已交付自定义日期/月度报告、按 Task 当前归属派生的项目时间分布，以及 DST 安全的 24 小时分布/最佳时段；标签分布和二维热力图仍未实现。
- 原生通知、托盘、应用通知暂停与系统勿扰引导。
- 真实桌面环境的后台挂起、休眠、异常退出与三平台矩阵；当前已有可控时钟、跨午夜、DST、并发和恢复自动测试，不能替代桌面验收。

### 后续增强

- 标签分布、二维热力图、长休息、白噪音和网站屏蔽。
