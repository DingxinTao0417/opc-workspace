...
> 当前基线：app v0.1.0 / API v1 / SQLite schema v42（2026-08-29）。Focus 结构仍由 schema v11 引入；后续迁移不改 Focus 表契约。Focus Core v0.1-A/B/C、v0.1-D1（历史与周期报告）、D2a（Task 详情记录）、项目详情的项目级报告/终态 Session 历史，以及 D2b 的本地日期范围回顾、项目/当前标签时间分布、最佳小时段与二维热力图已经交付。通用桌面托盘已接显示/隐藏/退出源码，但不读取 Focus 状态；专注控制、原生通知和勿扰仍属后续。
