...
2. **发起会话**：进入 `/ai`，多个就绪供应商时在输入区下拉选择本次使用的供应商（本地供应商带「本地」标识），输入问题（Enter 发送）；输入框聚焦时由整个输入区显示单层圆角焦点边框，文本区不叠加方形轮廓，键盘切到上下文、供应商或发送按钮时保留各控件的独立焦点提示。回答流式逐字显示，可点「停止」，取消保留已生成部分。
