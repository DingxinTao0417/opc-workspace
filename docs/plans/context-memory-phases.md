...
> 状态：设计已固化（[ADR-007](../adr/007-session-context-compaction-and-memory-tools.md)）；实施待用户确认启动
...
当前长程上下文 = 会话历史硬截断 + 长期记忆注入，缺摘要与工作状态（缺口清单见 ADR-007 背景）。目标形态：**分层上下文栈**（系统提示 / 长期记忆 / 前情摘要 / 关键事实 / 滚动窗口，各段独立预算）+ **后台异步压缩**（append-only 快照）+ **记忆工具**（push 保底、pull 增强）。

## G1：组装器重构与窗口修复（小改动，先行）

1. `chatHistory` 重构为纯函数组装器：按 turn 边界从最新往回装载至剩余预算；修复 `ORDER BY ASC LIMIT 200` 取到最老 200 条的方向性 bug。
2. 历史消息发送前剥离 `[opc:task]` / `[opc:memory]` / `[opc:selfcheck]` 块。
3. 此阶段摘要/事实两段留空（G2 接入）。

**验收**：组装器单测（预算切分、turn 边界、200-bug 回归、块剥离）；既有 AI API 测试全绿（提示词行为变化仅限块剥离）。
