# AI 助手 MVP 实施计划（草稿，待评审）

> **状态**：规划草稿，待同事评审确认，**尚未实施**。
> **日期**：2026-09-01
> **分支**：`feature/ai-assistant`（当前与 `origin/main` 一致，无任何改动）
> **排期**：独立于 v0.1–v0.4 的独立轨道；目标版本仍待定。
> **说明**：本文是已获用户批准的实施方案评审稿。仓库现有权威文档（`docs/modules/ai-assistant.md`、PRD §5.10、功能架构）仍描述"未开始 / 仅本地模型"的现状；两者冲突时，本文是**提案**，权威文档是**现状**。评审通过并实施后，才按 AGENTS.md 要求同步更新权威文档。
...
- 数据外发边界：仅外发该次用户输入与该次回复；不读未选择数据、不外发整库、无知识库、无遥测。任务正文不经模型外发：模型只输出文本建议+结构化建议块，字段由人工确认后在既有任务 API 落地。
...
- 失败边界：provider 不可用/超时/限流/无 key → 稳定错误码；取消与断链终止上游；AI 不可用时核心模块完全可用。结构化块缺失/非法 → 降级纯文本回复，不建任务。
...
- 语义识别的完整性依赖模型输出结构化块；块缺失/非法时降级纯文本（不建任务），后续可加"手动唤起建议卡片"兜底。
