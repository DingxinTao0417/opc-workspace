...
| 右侧概览面板 | 右侧固定   | 280px（可拖拽 240–480px） | 双形态：收起时仅 AI 助手页显示宽屏悬浮状态卡，展开时带页签停靠面板（概要/子智能体/文件/浏览器）；专注控制移至左侧边栏 |
...
- **工作区外壳**：亮/暗主题都以中性灰画布承托桌面工作区；主内容区在窗口四周保留间距，使用带边框和轻阴影的圆角工作面。左侧导航独立于主面；展开的右侧概览与主工作面同处一个框架，共用同一条外边框、圆角与阴影，二者之间只用 1px 发丝线分隔，不出现两条独立卡片或第二层投影。已启用的宽屏右侧概览可在当前会话收起/恢复，并可在 240–480px 之间拖拽调整宽度（方向键 ±12px、Shift 为 ±48px、Home 恢复 280px 默认值），均不改变业务状态或持久化默认值。概览收起时改由 AI 助手页的悬浮状态卡承接，该卡保持独立卡片外观（18px 圆角、发丝描边、低强度阴影）；概览内容按“分组标题 + 图标行 + 右对齐取值”排布，页签为纯文字下划线样式；`/ai` 页面另有一层作用域限定的令牌覆盖，详见 [AI 助手模块](modules/ai-assistant.md)。
