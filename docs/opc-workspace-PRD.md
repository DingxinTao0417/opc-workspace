...
> **v9.9 更新说明**：交付任务看板跨列受控生命周期交互。拖拽只解析为既有 `start / block / unblock / complete / cancel / reopen` 命令，确认后携带 `expected_version` 执行；阻塞/取消必须填写审计原因，解除阻塞只能回到服务端记录的阻塞前状态，manual 任务不能拖到完成，`waiting_review → done` 继续要求在详情中人工接受。界面不乐观伪造状态，服务端成功后才刷新；版本冲突会关闭旧确认并刷新列表。API v1/schema v29 不变。
