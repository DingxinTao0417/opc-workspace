...
> **v9.41 更新说明**：客户进入 `inactive` 后不再允许新建、编辑、重排回访，或在完成时原子续排下一条计划；既有 `planned` 回访仍可完成、跳过或确认取消，保留历史和业务收口。恢复为 `active` 或 `lead` 后才重新允许计划类写入。无新 migration、无外发消息、无 AI/LLM。app v0.1.0 / API v1 / schema v35 不变。
