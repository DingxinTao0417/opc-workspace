...
> 文档日期：2026-08-28
> 实现基线：app v0.1.0 / API v1 / SQLite schema v17

> **v5.0 更新说明**：交付 T-04B 基础业务 JSON 导出，不增加 SQLite schema。`GET /exports/business-data` 在一个 SQLite 读事务内按显式业务表白名单生成 business-export format v1 attachment；包记录导出时间、应用/commit/API/schema、稳定表/列/行及 active file Artifact 数量/字节，保留 Artifact 元数据但不嵌入文件正文。schema migrations、workspace identity、幂等响应、删除墓碑、派生 Focus totals、会话令牌和机器绝对路径明确排除；任一白名单表不可用时整体失败。设置页提供真实下载、进行中/成功/失败反馈，并明确它不是完整恢复包。迁移前自动备份、桌面一键重启、数据导入和含文件导出包继续独立实现。
...
> **当前状态**：部分完成。当前 schema v17 保留并验证了由 schema v3–v5 交付的 Project model、CRUD、分页/搜索/筛选、快照式创建幂等、覆盖聚合事实的 `If-Match` 乐观锁、受控状态流转、归档恢复、确认硬删除，以及真实卡片/详情/任务聚合；v12–v17 的 Inbox、Reminder、编排、设置与保存视图迁移均不改 Project 结构。Client CRUD、客户选择/改绑/解除和客户筛选已接通。Focus 完成会更新绑定 Task 工时并沿既有聚合链刷新 Project；项目级产出/附件/事件与 Inbox 来源投影仍未实现。Task 六状态上线后 Project 仍只把 done 计为完成，cancelled 留在总数/剩余口径。
...
> **状态**：未开始；不属于 v0.1，目标版本和本地模型/运行时待单独评审。当前仓库没有模型 SDK、密钥、AI API、会话表或助手页面；PRD v5.0 当前阶段不接入线上模型服务。
...
版本边界：手动一致性备份的创建、内部列表、完整校验、隔离恢复演练、恢复前自动回滚包、下一次 Sidecar 启动原子替换、确认删除和基础业务 JSON 导出已经交付；v0.1 仍须交付迁移前备份和桌面一键重启/恢复诊断。v0.3 只增加可配置计划、外部目录、保留策略、CSV 映射和高级导入工具。
...
3. **v0.1 恢复与基础导入**
   ...
   - 当前由用户关闭并重新打开应用触发；桌面一键重启、恢复进度页和脱敏错误日志入口仍待实现
...
首次启动不下载业务运行时或后端镜像。PRD v5.0 当前阶段不提供线上更新或第三方连接；安装、升级和核心使用均可在离线环境完成。
...
| 数据持久化       | Tauri `appDataDir`、SQLite 迁移、受控 Artifact、手动/迁移前一致性备份、基础 JSON 导出与原子恢复 | **部分完成**：正式/开发隔离、WAL、外键、schema v17、重建迁移安全、设置/保存视图事实、数据库身份强绑定和 Artifact store 已实现；手动 SQLite+Artifact 备份完整闭环及基础业务 JSON 导出已交付；迁移前备份、数据导入、含文件导出包和桌面一键重启仍待实现；每日计划和高级导入归 v0.3 |
...
> 状态截止：2026-08-28。当前版本是可运行、可扩展的 v0.1 基座；T-18A/B/C/D D1/D2、T-12 Focus Core A+B+C、T-11A1/A2/A3/B/C/F、T-06A–H、T-07A–D 和 T-04B 手动备份完整闭环/基础业务 JSON 导出已经交付，但这不代表第 9.1 节的完整 MVP、Focus D、非 Reminder 来源投影、Agent、数据导入、含文件导出包、桌面一键重启或迁移前自动备份已经交付。
...
| API / Schema | API v1；SQLite schema v17                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 数据默认值   | 开发数据库默认空白，不自动注入 demo 业务数据                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 明确边界     | 当前代码不使用 Docker；已实现 Task 事实层、Actor/Assignment、D1 生命周期、Task 时间线、D2 manual Submission/Artifact/受控文件、Client 基础资料/Project 客户关联、Focus Core A+B+C、手工 Inbox 受理/分诊、Reminder、Inbox Task 批量拆分/分派/自动结清、基础备份闭环及业务 JSON 导出；未实现 Client 活动/附件/Actor 显式关联、Focus D、非 Reminder 来源投影、重复/原生通知、Agent 执行、数据导入/含文件导出、AI 助手、知识库、客户回访或收入/支出/发票业务；person 只做本地责任记录，线上账号、云同步和远程协作均不在当前范围 |
...
迁移文件位于 `services/sidecar/internal/database/migrations/`，通过 Go `embed` 编入 Sidecar。当前 schema 为 v17，新增结构只能从 `018_*` 继续追加递增文件：
...
| T-04 SQLite 初始化与迁移             | 已完成            | schema v17、PRAGMA、嵌入迁移、demo 清理、Project/Task/Actor、六状态、workspace identity、Submission/Artifact、Client、Focus Session/interval/精确 Task ledger、Inbox、Reminder、app_settings 与 task_saved_views                                                                       |
| T-04B 手动一致性备份                 | 基础安全闭环完成  | 专用 backup root、维护写锁、SQLite `VACUUM INTO`、active Artifact+marker+manifest、完整校验、同卷原子发布、创建幂等、列表/重新校验、隔离演练、自动回滚点、pending/applied 提交点、重启原子恢复、确认删除、白名单业务 JSON 导出及设置 UI 已交付；桌面一键重启和迁移前自动备份待后续纵切 |
...
- **验证/剩余**：Go 集成测试覆盖含真实 file Artifact 的快照边界、包结构、列表、幂等重放/冲突、显式校验、隔离演练、自动回滚点、pending 同目标重放/异目标冲突、业务冻结、启动恢复、目标/回滚内容、WAL 部分移动补偿、临时清理、有效/损坏包确认删除、篡改拒绝，以及业务导出版本/白名单/稳定排序/文件正文与机器私有信息排除/失败关闭；前端覆盖严格契约、恢复/删除确认请求序列化、导出 attachment 契约和设置入口交互。迁移前备份、含文件导出包/导入、大数据量进度/取消、桌面一键重启和真实磁盘故障仍未实现。
...
- **建议开发流程**：
  1. 先完成本地运行时、资源预算、上下文权限和质量评测 ADR；PRD v5.0 当前阶段不评审远程 Provider。
...
6. **补数据安全链路**：手动一致性备份的创建、列表、完整校验、维护写锁、原子发布、隔离演练、恢复前回滚点、重启原子恢复、确认删除与基础业务 JSON 导出已经交付；下一步依次实现迁移前自动备份、桌面一键重启、诊断包内容选择、数据导入和含文件导出包。
...
| 9    | 数据安全                  | 已交付 v0.1 手动一致性快照、manifest/SHA-256、维护写锁、完整验证、原子发布、隔离演练、自动回滚点、重启原子恢复、确认删除和基础业务 JSON；迁移前备份待实现；v0.3 增加计划/映射         | 已交付手动创建、列表、重新校验、恢复演练、恢复/删除二次确认、关闭重开指引与业务 JSON 下载；含文件导出/导入仍不开放                                                             | 已覆盖 WAL 活跃、真实 Artifact、幂等、篡改拒绝、演练/恢复/删除，以及导出白名单、稳定结构和敏感运行事实排除；仍须覆盖低磁盘、桌面一键重启、含文件包和真实磁盘故障                                 |
...
在对应纵切完成前，客户活动/附件、发票“新建”和收入时间范围等无业务行为控件必须禁用或明确标记“后续版本”，不得用可点击外观暗示已经实现。Client 新建/编辑/基础详情、Project 客户选择/筛选、Inbox“全部已读”/已有 Task 关系/批量拆分分派/自动结清，以及一次性 Reminder 已接入真实 API，不再属于占位清单；非 Reminder 来源投影、重复/原生通知、任务看板与生命周期批量操作仍未实现。
