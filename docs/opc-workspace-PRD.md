...
> **v9.50 更新说明**：路线图 R3 首个界面纵切已交付：`/roadmap` 读取真实本地季度里程碑，支持季度/状态浏览、新建、Project 深链、归档恢复及加载/空/错误状态；schema 仍为 v36。编辑、删除、拖拽、Inbox/Reminder 自动化仍待。无 AI/LLM、外发或 demo 数据。
...
> **当前状态**：路线图 `/roadmap` 已完成基础真实本地界面，R2 的 schema v36/API 提供 CRUD、筛选、项目关联、只读派生进度、乐观锁、归档恢复与重排；当前界面已接季度/状态浏览、新建、Project 深链和归档恢复，编辑、删除、拖拽与 Inbox/Reminder 自动化仍待。内容日历路由仍为明确的“后续版本”占位页且无表、API 或业务交互。目标版本为 v0.3。
