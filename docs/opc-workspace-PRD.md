...
> **v9.42 更新说明**：客户详情在 `inactive` 状态下直接隐藏“安排、编辑、重排”和完成时“下一次本地回访”入口，改为明确说明既有计划仍可完成、跳过或取消；恢复客户状态后计划类入口自动恢复。Sidecar 的 v9.41 强制门禁仍是最终事实。无新 migration、无外发消息、无 AI/LLM。app v0.1.0 / API v1 / schema v35 不变。
