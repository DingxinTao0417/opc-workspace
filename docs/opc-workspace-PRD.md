...
> **v9.18 更新说明**：交付 Project Artifact → Inbox → Task 的下一纵向人工闭环。`GET /api/v1/projects/:id/artifacts` 为每项返回 nullable `followup`，包含 Inbox Item ID/version、状态、解决策略、`source_deleted_at` 与实时 required 进度；列表保持兼容的 Project 聚合数值 `ETag`，`meta.project_version` 仍只表示同一 Project 聚合版本，`followup.inbox_item_version` 才是后续 Inbox 写入的并发版本（Project 页面当前只提供深链）。项目详情把产出区上移到任务区之后，显示待拆分/跟进中/已解决/已忽略以及必需任务阻塞/待验收/取消提示，并可深链 Inbox。Inbox 拆分会从可信本地来源快照带入 Project，仍允许逐项清除或改选；面板新增真实完成条件输入，并明确 person 仅是本地责任记录；活动及可用历史关系行都可打开共享 Task 详情。命令面板和业务 Modal 现共用叠层、滚动锁、最上层 Escape 与延迟焦点恢复，覆盖逐层关闭和父子同批卸载。所有会改变 follow-up 的成功 Inbox mutation 会失效来源 Project，split 另失效 Task、Today 与 Project，以刷新不参与 Project version 传播的实时跟进事实。Go 金链覆盖 `requires_followup → split(owner/person + manual owner reviewer) → complete + submit(waiting_review) → accept → Inbox automatic resolved/100%`，前端自动化另覆盖 person 本地责任提示与提交载荷。app v0.1.0 / API v1 / SQLite schema v31 不变，无迁移；v0.1 不包含 AI、LLM 或 Agent Runtime。自动化证据不等于所有端到端人工浏览器验收，真实浏览器、窄屏及 1,000/10,000 条数据专项仍待完成。
...
| `pnpm --filter @opc/web typecheck` | 通过         | 本次覆盖 Project Artifact follow-up 严格契约、Inbox/Project 缓存协作、共享 overlay stack 与既有 Task D2、Client/Project/Inbox 契约                                                                                                                                                                             |
| 前端测试                           | 通过         | 全量 78 个文件 / 510 项通过，覆盖 nullable follow-up/实时 required 进度、Project 产出状态与深链、来源 Project 继承/清除/改选、完成条件、person 本地责任、split 成败语义、cancel→invalidate、AbortSignal，以及 Modal/命令面板叠层、同批卸载和失效触发器焦点回退；既有回归继续通过。组件测试不替代浏览器视觉验收 |
| Web 生产构建                       | 通过         | `pnpm build:web` 的 TypeScript 与 Vite 构建通过；当前主入口压缩后约 816 kB，仍有非阻断的 500 kB chunk 警告，后续按页面拆包                                                                                                                                                                                     |
| 前端格式                           | 通过         | Web 范围 Prettier format check 通过                                                                                                                                                                                                                                                                            |
| Go 全量测试与静态检查              | 通过         | `go test ./... -count=1` 与 `go vet ./...` 均通过；不虚构未统计的用例总数                                                                                                                                                                                                                                      |
| 文档链接/过期措辞/diff             | 通过         | 本次 v9.18 文档更新后运行相对 Markdown 链接、绝对机器路径、过期状态和 `git diff --check` 审计                                                                                                                                                                                                                  |
| Rust / Tauri 链接                  | 环境受限     | 当前主机缺少 MSVC `link.exe`（Visual C++ Build Tools / Windows SDK）；无法把 `cargo check`/Rust tests 或完整桌面链接声明为通过                                                                                                                                                                                 |
| 浏览器渲染 / 窄屏视觉              | 本次未验收   | 当前环境未提供可用浏览器控制运行时；尚无本轮真实浏览器/WebView 人工深链、键盘与焦点、Portal 窄屏浮层及 1,000/10,000 条 Project/Task/Inbox 性能证据                                                                                                                                                             |
