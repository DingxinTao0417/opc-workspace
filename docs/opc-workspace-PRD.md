...
> **v7.5 更新说明**：项目详情任务浏览器改为 Task API 服务端分页，每页 20 项；无搜索/状态条件时可在根任务树和平铺列表间切换，有条件时自动使用平铺结果并显示父任务上下文。标题/描述搜索使用 280ms 防抖，状态筛选、分页、加载/空/错误/重试均基于服务端事实；不再为展示项目任务串行拉全完整集合。schema 保持 v28，高级分析与内嵌 Assignment/Submission 写控件继续延期。
