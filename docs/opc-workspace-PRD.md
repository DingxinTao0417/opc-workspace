...
> **v9.33 更新说明**：交付 T-17/C5 客户详情回访命令纵切。客户详情已提供本地计划的新建、编辑、完成、跳过、确认取消与重排表单，复用 active owner/person、创建幂等与写入 `If-Match`，冲突会刷新事实且不伪造成功；Today 展示与 Inbox 反向执行仍待。app v0.1.0 / API v1 / schema v35 不变。
...
- 已实现回访管理：客户详情使用 `GET /api/v1/clients/:id/followups` 分页读取并严格校验响应，显示计划/完成/跳过/取消、优先级、负责人、渠道、结果与下一步；`overdue` 只由 planned 的计划时间在页面派生。用户可在同一页面创建、编辑、完成、跳过、确认取消和重排本地计划，选择既有 active owner/person；创建使用幂等键，写入使用当前 `If-Match`，冲突会刷新最新事实。页面同时提供加载、空、失败重试和分页，不会发送外部消息。
- 待实现：外部活动来源集成、关联发票详情和财务聚合；回访 Today 展示与 Inbox 反向处理入口；线上下载、邮件或其他客户沟通只有在未来明确集成并记录真实事实后才能出现。

#### 客户回访（后续版本）

> **状态**：部分完成，不属于 v0.1。schema v35 已提供本地回访计划和终态的安全数据契约，C3/C4 已提供计划与执行 API，C5 已提供到期 Inbox 投影、客户详情时间线和本地创建/执行表单；Today 展示和 Inbox 反向入口仍在后续纵切。
...
| 客户管理         | 客户列表表格、客户详情、基本 CRUD                                                              | **部分完成**：基础资料 CRUD、分页/搜索/状态筛选/排序、创建幂等、并发控制、基础详情、受约束删除、Project 关联、本地活动、受控附件和 person 显式关联已实现；schema v35 回访计划/终态、C3/C4 API、C5 到期 Inbox 投影及客户详情时间线/创建/执行表单已交付，Today 和 Inbox 反向入口仍待；外部来源投影和财务待实现                                                                                                                                       |
