...
> **v9.51 更新说明**：内容日历 CC1 已交付 schema v37：空的内容条目、准备 Task 关联、计划 IANA 时区、状态/发布确认/归档约束及项目/任务删除保护，并纳入业务导入导出。API、页面、调度与发布确认仍待；无 AI/LLM、外发或 demo 数据。
...
> **当前状态**：路线图 `/roadmap` 已完成基础真实本地界面，R2 的 schema v36/API 提供 CRUD、筛选、项目关联、只读派生进度、乐观锁、归档恢复与重排；当前界面已接季度/状态浏览、新建、Project 深链和归档恢复，编辑、删除、拖拽与 Inbox/Reminder 自动化仍待。内容日历路由仍为明确的“后续版本”占位页；schema v37 已有内容/准备任务数据契约与导入导出覆盖，但无 API 或业务交互。目标版本为 v0.3。
