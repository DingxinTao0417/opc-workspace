...
> 文档日期：2026-09-09
> 实现基线：app v0.1.1 / API v1 / SQLite schema v67

> **v10.8 更新说明**：ADR-024/AI7-Q3 本地用量 UTC 时间趋势已交付。既有 usage summary 新增 1–30 天连续零填充趋势，只统计 completed/failed/cancelled 根步骤；会话面板可切换 7/30 天，展示运行数与 token coverage。累计 totals 不因趋势窗口变化；不增加 schema、价格、货币、费用估算或线上查询。
...
> **状态**：AI 助手纵向切片持续交付（schema 052–067）。Provider、双协议会话、Harness、AI5/AI6、citation、离线 scorer、Q2 8-case smoke/四类 6-case topic/24-case full Actor/suite 趋势/category/failure/Wilson/候选/人工决定审计，以及 Q3 无正文 steps/Provider token/unknown/本地聚合/UTC 时间趋势已交付。生产只注册记忆三工具；费用、句子级覆盖率与自动路由仍为后续纵切。
...
- 本地大模型接入：`kind=local` Provider 走回环 OpenAI 兼容端点（Ollama、LM Studio 等），无需且禁止保存 API 密钥（`AI_KEY_NOT_ALLOWED`），复用流式适配器与健康探测；回环流量不出本机、不走代理。
- Provider 安全与历史保护：模型请求和健康探测拒绝跨 origin 重定向；配置/健康与 Keyring 写入使用进程级读写互斥，非 2xx/流错误按当前密钥脱敏。已有 generation 历史的 Provider 返回 `AI_PROVIDER_HAS_SESSIONS`，必须先显式删除会话，不隐式级联删除对话。
- Agent Harness 运行时：Sidecar `internal/harness` 承载调用/执行循环、显式工具 allowlist、执行器、预算与取消；OpenAI/Anthropic 两侧均支持工具定义、跨帧单/多调用聚合及 call/result 回填。生产只注册 ADR-007 的记忆三工具，其他工具须逐个评审授权。
- 纠错与反思：工具失败以错误结果回填供模型自纠正（单次运行上限 3 次）；模型在每次回答末尾自主输出 selfcheck，自评不充分时在全局轮次预算内做一次非流式修订，无 `reflect` 请求参数或前端开关；修订成功通过可选 `replace` SSE 帧替换界面草稿。
- 长期记忆（自进化，数据层）：模型输出 `[opc:memory]` 建议块 → 前端待确认卡片 → 人工确认后存入 `ai_memories`（056），后续会话注入 system 上下文（前 20 条、8 KiB 预算）；设置区可管理删除；提示词与工具集永远代码所有，模型无权自我改写。
- 会话摘要与关键事实：schema 057 以 `ai_memory_entries` 保存会话级 append-only 快照。窗口外未压缩正文超过 16 KiB 时，Run 收尾后后台压缩最多 32 KiB；严格 JSON 只接受 summary ≤4 KiB、五类 facts ≤32 条且注入 ≤4 KiB。快照替换、水位线和 `ai_session_compacted` 脱敏事件同事务提交；失败保持旧快照并继续使用硬截断。
- 记忆工具与透明化：`memory_write` 写当前会话事实，`memory_search` 检索当前会话 active 记忆和历史消息，`memory_propose` 只创建 pending 建议；对话卡片或设置区确认后才进入跨会话 `ai_memories`，忽略则 supersede。会话 API 返回已被 active 水位线覆盖的消息数，页面显示压缩标记。
- 会话隐私与归属：`persist=false` 只保留 generation 运行元数据，不保存 user/assistant message 或 generation 正文；生成期间前端锁定会话/Provider 切换和删除，流内容不会挂到其他会话。长期记忆长度按 Unicode 字符校验。
- 显式业务上下文：每条消息最多各选一个 Task/Project/Client；Sidecar 生成有界最小快照，远程/本地发送前逐字段预览。Provider/source version 变化返回 `AI_CONTEXT_PROVIDER_CHANGED / AI_CONTEXT_CHANGED` 且不创建会话、消息或 generation；持久 user message 保存 schema 058 context_snapshot，后续历史可解释实际外发来源。
- 显式知识与可验证引用：用户本地搜索并逐段确认最多 3 个 chunk；Sidecar 在 AI 写入前重验 source/document version。模型 citation 只能声明 allowlisted chunk IDs，来源/版本/位置由 Sidecar 重建；schema 060 保存 validated/no_evidence/missing/invalid，UI 显示来源卡或质量警告，Raw 控制块不落库。
- 运行步骤与本地指标：schema 061 保存内容无关 steps/字节/耗时，schema 062 只保存 Provider 明确返回的完整 input/output token；OpenAI 读根 usage，Anthropic 合并 start/delta，缺失保持 unknown，不强制 stream_options。会话/Provider 本地聚合只读根步骤并分列终态、活动、coverage、token、bytes、duration；ADR-024 再以根 `completed_at` 的 UTC 日返回 1–30 天连续终态趋势，窗口不改变累计。Steps、聚合和趋势均不含正文/凭据，bytes 不冒充 token，token 不自动换算费用。
- 显式本地模型评测：schema 063 的单邮箱 Actor 只消费用户选择的 ready/healthy 本地 Provider；schema 064 保留 dataset 版本，ADR-020 当前为 v3/24；ADR-021/schema 065 增加 8-case smoke 与 24-case full，并给旧 Run 默认回填 full；ADR-023/schema 067 再增加四个各 6-case 的 category 专题套件。创建与执行都重读 embedded version/suite/case count；Run succeeded 只表示所选套件执行完成。只保存 Provider/dataset/suite 快照、failure code、citation 数和无正文指标，支持取消、恢复与终态删除，不保存问题、知识片段或回答。
- 本地质量趋势与 category：只读 summary 把全部 Run 状态计数与 succeeded-only 质量比例分开；质量严格按 Provider ID + 名称/模型快照 + dataset version + suite 分组，最近完整 Run 按时间正序显示 `passed/total`。同一套件组再分解四类 category，类别合计必须与父组对账；8/24 分母不混算，failed/cancelled/active 不进入质量分母。
- 本地失败原因：同一 summary 只展开 succeeded Run 下 failed Result 的代码所有 failure code；`affected_cases` 按 Result 去重，`occurrences` 保留重复命中。每个 code 仍归属同一 Provider/名称/模型/dataset/suite 组，不返回 failure detail、短语或跨组榜单。
- 本地质量不确定性：同一 summary 为总体/category 即时计算固定 95% Wilson 区间，以 0–10000 basis points 返回并由 Web 从原始 passed/total 精确重算；总体组按完整 Run 数标记 `single_run / limited_runs / repeated_runs`。设置页显示点估计、分母、上下界与单次/有限重复/重复观察，明确不代表发布许可；无物化表、自动阈值或业务副作用。
- 本地人工评审候选：summary 公开 `mode=advisory / required_suite=full` 的当前 dataset、3 Run、总体/category 下界和严重 code；质量组另返回 Provider version min/max、三状态与稳定原因。旧数据集、smoke、Run 不足和配置版本混合先判证据不足；只有 full 达到前置后才检查总体 8000、四类 6000 与严重 code。Web 完整重算；候选不启停 Provider、不阻止聊天、不创建 Inbox/Task。
- 本地人工决定审计：schema 066 对精确 Provider/模型/dataset/suite 质量组保存 Run/case、Wilson、最低 category、readiness、严重 code、builtin owner、三值决定与必填理由快照；创建在写事务内重算当前证据，不一致返回 `AI_EVALUATION_REVIEW_STALE`。记录只可追加，数据库拒绝 UPDATE/DELETE；设置页提供组内表单与只读历史，并明确不改变模型、聊天、Provider 或业务状态。
...
- 语义建任务（Linear 式建议卡片）：先自然语言确认，再在回复末尾给出结构化块；前端隐藏协议块并展示可编辑待确认卡片（标题必填）→ 用户确认后经既有 `POST /api/v1/tasks` 创建（固定 `todo`）→ 消息挂静态引用卡片 → 点击跳转任务详情。兼容重复开始标记形式的错误闭合；只有结构块时补自然语言确认；任务创建后挂接失败的重试不会重复建任务。
- 失败边界：稳定错误码族（`AI_*`）；AI 不可用时核心模块完全可用，AI 失败不投影 Inbox。

#### 边界与后续规划

- 模型输出只读，视为不可信预览；自动建任务、Shell/SQL/业务写、自主代理均禁止。Harness 生产 Registry 仅含记忆三工具，不读取业务对象或自动搜索知识库；AI6 只注入用户预览确认的静态知识快照，不是新工具。
- 远程 Provider 外发范围包括系统提示、长期记忆、摘要/事实、最近回合、当前输入、memory_search 命中、用户逐字段预览确认的 Task/Project/Client 最小快照，以及最多 3 个逐段确认的知识 chunk。不自动扩展关联对象或检索未选知识；preview 展示完整外发正文、位置、版本和字节数。
- 长程上下文管理设计固化于 [ADR-007](adr/007-session-context-compaction-and-memory-tools.md)：G1–G5 已交付。记忆三工具是对原工具禁令的唯一突破；其余工具仍须逐个评审授权。
- 本地部署 Provider 仅限回环 OpenAI 兼容端点，不保存密钥；协议扩展与工具启用路径见 ADR-005。
- `ai_providers / ai_sessions / ai_generations / ai_messages / ai_memories / ai_memory_entries / ai_run_steps / ai_evaluation_runs / ai_evaluation_results / ai_evaluation_reviews` 十张 AI 表为操作态/隐私边界，排除出业务 JSON/ZIP 导出；一致性备份仍覆盖。
...
| 明确边界     | 当前代码不使用 Docker；已实现 Task/Actor/D1/D2、Project、Client、Focus、Inbox/Reminder/Task 编排、已登记来源投影、首个受限预设自动化纵切、运行期数据库故障、可配置低空间安全投影、基础备份闭环、业务 JSON/ZIP 空目标及同 schema 零主键冲突追加、只读冲突/schema 方向预检、脱敏诊断、数据库打开前白名单启动进度与启动恢复、AI5/AI6 与知识库 Actor/文本检索基线；通用托盘最小闭环已接源码但原生链接未验收；未实现 Client 外部来源、Focus 专用原生反馈、启动前备份选择、复杂/原生通知、完整 Agent Runtime、自由规则、真实冲突策略/UUID 重映射/跨 schema 升级、知识库 PDF/授权引用/变化检测或结构化 AI citation；自动化没有 Shell/SQL/HTTP/外发能力，person 只做本地责任记录，线上账号、云同步和远程协作均不在当前范围 |
...
| T-15 AI 助手                         | AI7-Q2/Q3 本地质量与用量推进     | Provider、Harness、AI5/AI6、citation、Q2 scorer + schema 063–067 评测 Actor/dataset/suite/人工决定审计、8-case smoke/四类 6-case topic/24-case full、suite 趋势/category/failure/Wilson/候选、schema 061–062 steps/Provider token/unknown、本地聚合与 UTC 用量趋势 UI 已交付；费用、句子级覆盖率与自动路由待后续                                                                                                                                                                                                                                                  |
...
- **需求映射**：5.10、9.1、9.2。
- **当前状态**：AI5/AI6、ADR-011 Q1、Q2 离线门禁、ADR-013 本地 Provider Actor、ADR-014 趋势、ADR-015 category 与 ADR-016 failure-code、ADR-012 Q3 schema 061–062 已交付。`/ai`、Provider、Harness、显式上下文、citation、离线 scorer、本地评测/趋势/category/失败原因、无正文 run steps/Provider token/unknown 历史 UI，以及 session/provider 本地聚合均有代码与测试。
- **后续开发流程**：
  1. ADR-007 G1–G5 已完成；保持记忆工具之外的能力关闭，并维护长会话/工具协议回归集。
  2. AI5 与 AI6 已完成；保持业务白名单和知识三层 identity/version 回归，不把自动整库检索带入 AI。
  3. 回答级 citation、dataset v3、8-case smoke/四类 6-case topic/24-case full、显式本地模型 Actor、suite 趋势/category/failure/Wilson、人工评审候选与不可变人工决定审计已完成；Q3 已有本地 UTC 用量趋势；下一步评审费用配置和跨平台安全存储验证。
...
| GET / DELETE         | /api/v1/ai/memory-proposals               | 管理待确认长期记忆建议                                                                   | 已实现；GET 返回 active proposal、来源会话与标签，DELETE 受控 supersede；确认复用 `POST /ai/memories` + `proposal_id`                                                                                                                                                                                                                                                                                           |
...
| GET / POST           | /api/v1/ai/evaluations                    | 本地模型质量评测历史/创建                                                                | 已实现；GET 支持 Provider/status 与分页，POST 携带本地 Provider ID/version、六个代码所有 suite key 和可选 Idempotency-Key；缺省 full，只接受 ready/healthy local Provider。单邮箱 Actor 运行 dataset v3 的 8-case smoke、四类 6-case topic 或 24-case full，旧历史按 full 可读                                                                                                                                  |
...
| GET                  | /api/v1/ai/evaluation-summary             | 本地评测状态、suite 分组、category、失败原因、Wilson、候选与趋势                         | 已实现；可选 Provider 与 trend limit；单只读事务按 Provider/名称/模型/dataset/suite 分开 succeeded 质量，返回 category/failure、Provider version、Wilson 和 advisory policy/status/reasons。smoke/topic 固定只诊断；当前 full + 3 Run + 单一配置版本 + 总体 8000/category 6000 + 无严重 code 才是人工评审候选。无发布许可、自动动作、正文或线上调用                                                             |
| GET                  | /api/v1/ai/evaluation-reviews             | 查询本机人工决定审计                                                                     | 已实现；按创建时间倒序稳定分页，可选 Provider snapshot/decision 筛选；返回精确质量组、Wilson/readiness/严重 code、三值决定、必填理由与 builtin owner 快照，不返回问题、知识、回答、URL 或 key；无编辑/删除端点                                                                                                                                                                                                  |
| POST                 | /api/v1/ai/evaluation-reviews             | 对当前质量证据追加人工决定                                                               | 已实现；接收质量组精确身份/计数/readiness 快照、三值决定与 1–1000 字理由；同一写事务重算 succeeded Run/Result，变化返回 `AI_EVALUATION_REVIEW_STALE`，不存在返回 `AI_EVALUATION_GROUP_NOT_FOUND`；支持 Idempotency-Key。只写审计表，不改变 Provider、聊天、Task、Inbox 或其他业务状态                                                                                                                           |
| POST                 | /api/v1/ai/chat                           | 显式调用 AI 助手                                                                         | 已实现；SSE 返回 meta/delta/reasoning/replace/done/error/cancelled；AI5/AI6 上下文写前重验，ADR-011 在完成时剥离/校验 citation block 并保存五状态；生产无 knowledge tool                                                                                                                                                                                                                                        |
