...
> 文档日期：2026-08-29
> 实现基线：app v0.1.0 / API v1 / SQLite schema v38

> **v9.65 更新说明**：路线图已新增贴合历史原型的年度 Q1–Q4 时间线；在无细分筛选且年度完整集合不超过 100 条时，可拖动卡片到目标季度，或用上一/下一季度按钮移动，目标日期设为目标季度末并携带当前版本 PATCH。失败刷新服务端事实，超过容量不做局部拖拽；精确日期拖拽仍待，schema 仍为 v38，无 AI/LLM、外发或 demo 数据。
...
> **当前状态**：路线图 `/roadmap` 的 R3 基础界面已完成；R4 已交付完整同季度集合的安全卡片排序、年度 Q1–Q4 时间线、跨季度拖拽/键盘移动，以及 PATCH 跨季度追加到目标季度尾部的事实规则。R2 的 schema v36/API 提供 CRUD、筛选、项目关联、只读派生进度、乐观锁、归档恢复与重排。当前界面已接季度/年度、状态/Project 浏览、稳定分页、新建、基础事实编辑、按 ID 详情、Project 深链、归档恢复、保护性永久删除，以及安全排序/季度移动；精确目标日期拖拽和 Inbox/Reminder 自动化仍待。内容日历已交付 schema v37 数据/API、六周月格、新建/编辑、IANA/DST 安全改期、人工发布确认、Task 关联及 schema v38 Inbox 来源契约，并完成审核/发布到期的启动/周期扫描、版本化去重、变更终结、删除协调、Inbox 来源详情与六周范围自动分页聚合。目标版本为 v0.3。
...
| 前端测试                           | 通过         | 全量 89 个文件 / 582 项通过；新增路线图年度时间线读取、跨季度键盘替代和拖拽 PATCH 覆盖，既有路线图同季度原子排序/容量保护、筛选分页/编辑删除/按 ID 详情、内容日历、启动进度、Agent Adapter、Automation、Reminder、内部回滚容量、业务与连接世代回归继续通过。组件测试不替代真实 WebView 验收  |
...
9. **扩展本地规划与预设自动化**：预设自动化首个纵切已交付 Project 完成 Inbox 与 daily/weekly Reminder；路线图已完成数据/API、R3 基础界面和 R4 同季度安全排序首个纵切，内容日历已完成 CC1–CC5-B。下一步完成路线图目标日期/跨季度拖拽与来源投影、内容日历大数据量与真实 WebView 专项，并在发票/Agent 事实上线后分别启用对应预设；Task 动作和自由规则仍待后续，任何规则都不得自动对外发送。
...
| 13   | 路线图（v0.3）            | **R2 已完成**：roadmap_milestones、季度/日期/状态/项目关联，进度从项目/任务派生，编辑/归档/恢复/删除使用乐观版本                                                                                                                                                                                                                                                                                      | **R3 已完成；R4 部分完成**：季度/年度、状态/Project 筛选、稳定分页、新增/编辑/按 ID 详情、项目关联/跳转、归档恢复和二次确认删除已交付；完整同季度集合可原子排序，年度 Q1–Q4 时间线支持拖拽或上一/下一季度按钮移动，目标日期明确设为季度末；精确目标日期拖拽待补                                                         | API 已覆盖季度边界、跨季度尾部排序、进度口径、项目删除约束和并发冲突；Web 已覆盖筛选分页、详情最新事实、编辑载荷、版本头、保护性删除、同季度重排和跨季度移动。真实浏览器拖拽/焦点、窄屏、超过 100 条提示及长列表仍待专项验收                                                                                                         |
