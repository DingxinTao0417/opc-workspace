...
> **v9.82 更新说明**：schema v43 新增本地容量历史。后台扫描与显式检查按真实物理卷去重，但仅以逻辑位置组合保存 15 分钟桶，保留 30 天；失败探测不造样本，历史写入失败不阻断低空间提醒。只读 API 支持 1–30 天窗口，设置页默认展示 7 天折线、最新值和变化；数据库/API 均不保存或返回路径、盘符或卷 ID。app v0.1.0、API v1 不变，无 AI/LLM。
...
版本边界：手动一致性备份的创建/低空间准入、内部列表、完整校验、隔离恢复演练、恢复前自动回滚包、破坏性迁移前自动回滚包、下一次 Sidecar 启动原子替换、桌面一键安全重启、启动后恢复结果诊断、确认删除，以及业务 JSON/含文件业务 ZIP 的空工作区同 schema 安全导入导出已经交付；导入 preview 已增加非空目标只读冲突清单和跨 schema 方向分类，但不开放合并或升级应用。容量准入现已覆盖手工 `POST /api/v1/backups` 与导入、恢复、迁移内部创建的全部当前自动回滚包。数据库打开前的实时进度/恢复页仍需桌面壳实现。v0.3 只增加可配置计划、外部目录、保留策略、CSV 映射和高级冲突导入工具。
...
| 明确边界     | 当前代码不使用 Docker；已实现 Task/Actor/D1/D2、Project、Client、Focus、Inbox/Reminder/Task 编排、已登记来源投影、首个受限预设自动化纵切、运行期数据库故障、可配置低空间安全投影、基础备份闭环、业务 JSON/ZIP 空工作区导入导出及只读冲突/schema 方向预检、脱敏诊断、数据库打开前白名单启动进度与启动恢复；通用托盘最小闭环已接源码但原生链接未验收；未实现 Client 外部来源、Focus 专用原生反馈、启动前备份选择、复杂/原生通知、Agent、自由规则、非空目标实际合并/跨 schema 升级、AI、知识库或财务；自动化没有 Shell/SQL/HTTP/外发能力，person 只做本地责任记录，线上账号、云同步和远程协作均不在当前范围 |
...
| 前端测试                                           | 通过         | 全量 90 个文件 / 613 项通过；新增容量历史严格解析、路径/卷标识拒绝和设置趋势交互，以及既有业务导入、桌面、设置、规划、客户、Automation、Reminder 与连接世代回归。组件测试不替代真实 WebView 验收                                                                                             |
...
| 统一源码质量门禁                                   | 通过         | 本轮源码门禁覆盖 Prettier、195 个 Go 文件与 Rust 格式、26 份 Markdown/本地链接、Web 类型/90 文件 613 项测试/构建、Go 无缓存测试/vet 与 Sidecar 构建                                                                                                                                          |
...
6. **补数据安全链路**：一致性备份完整闭环、迁移前自动回滚、启动后恢复结果诊断、业务 JSON 与含文件 ZIP 的空工作区同 schema 安全导入导出、非空目标只读冲突清单、跨 schema 方向分类、脱敏诊断包 v1、数据库打开前白名单恢复进度和全局启动故障恢复页已交付；下一步设计显式合并策略/UUID 重映射与版本升级转换，再补大数据量进度和启动前备份选择。
...
| 9    | 数据安全                  | 已交付一致性快照、验证/演练/恢复/删除、迁移/恢复/导入前回滚点、启动后恢复结果诊断、业务 JSON 与含文件 ZIP 的空工作区安全导入导出、非空目标冲突清单/跨 schema 方向分类、失败 Inbox 投影；v0.3 增加计划/合并映射                                                                                                                                                                                                                             | 已交付备份全流程、恢复状态重查/门禁、两类业务下载及两类导入的预检、确认与结果反馈；非空目标展示逐表源/目标/主键重叠，跨 schema 展示旧/新方向，实际确认按钮保持禁用                                                                                                                                                                      | 已覆盖真实 Artifact、篡改、恢复、恢复诊断脱敏、导出隐私/ZIP 完整性、JSON/ZIP 导入门禁/回滚/文件补偿，以及冲突计数、方向 blocker、严格响应聚合和无副作用；仍须覆盖实际跨 schema 升级/非空目标合并、大数据量和真实磁盘故障                                                                                                                                                             |
| 10   | 桌面与发布                | **部分完成**：generation-aware 两次有界重启、数据库运行锁、父管道 EOF、并发 shutdown、日志/request ID、全局恢复页、安全应用重启、固定命令面板/新建任务的原生快捷键注册/状态查询、托盘显示/隐藏/显式退出、不可用安全降级、固定布尔 `set_close_to_tray_enabled`，以及白名单 `desktop_capabilities` 已交付；托盘原生链接/三平台交互、真实父崩溃/进程树、孤儿/孙进程治理、通知/自启/快捷键自定义/签名更新及三平台 CI 待实现                    | **部分完成**：starting/restarting/ready/error + generation、非 ready 连接/查询清理、状态重查、日志入口、安全重启、版本白名单、fixed shortcut 状态和托盘运行时能力诊断已交付；设置横向模块菜单已在 480 px 保持活动项可见；基础托盘菜单不经 WebView，关闭到托盘开关已交付；专注状态/业务动作、启动前备份选择/恢复反馈与离线更新反馈待实现 | 当前源码门禁已通过 Web 90 文件/613 测试、Go 无缓存全量测试/vet、文档、Sidecar 构建、Rust 格式与锁定 Cargo metadata。真实浏览器已验证关闭到托盘的预览、取消恢复与保存刷新持久化；cargo check/test 受缺失 MSVC link.exe/Windows SDK 阻塞，托盘原生交互、真实父崩溃、进程树、签名、公证、干净机和三平台仍须验收                                                                         |
