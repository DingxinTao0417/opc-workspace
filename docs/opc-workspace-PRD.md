...
> **v3.5 更新说明**：交付 T-06D Today 同组拖拽：所选精确日期与未排期活动 Task 可通过拖动手柄立即预览组内顺序，Sidecar 重新读取完整计划组、校验活动 ID 集合和全部版本后原子持久化；done/cancelled 槽位保持，成功刷新事实，失败/冲突清除预览并回滚。上移/下移按钮继续作为键盘替代；跨日期改期和行内编辑/删除/专注快捷操作仍未交付。
...
> **状态**：未开始；不属于 v0.1，目标版本和本地模型/运行时待单独评审。当前仓库没有模型 SDK、密钥、AI API、会话表或助手页面；PRD v3.5 当前阶段不接入线上模型服务。
...
首次启动不下载业务运行时或后端镜像。PRD v3.5 当前阶段不提供线上更新或第三方连接；安装、升级和核心使用均可在离线环境完成。
...
- **建议开发流程**：
  1. 先完成本地运行时、资源预算、上下文权限和质量评测 ADR；PRD v3.5 当前阶段不评审远程 Provider。
...
| 8    | 设置与命令面板            | Actor API 与严格校验的 health/version 已接入；版本化 app_settings、旧 localStorage 一次性迁移、头像文件引用和统一 search API 待实现                                                                                   | “人员与责任”已支持 owner/person；Focus 页与命令面板可直达指定设置模块，“关于”展示真实健康/版本并可重试；通知、数据/备份、快捷键和完整诊断待实现                       | 已覆盖 Focus 设置隔离、Task 搜索定位、IME、键盘圈闭/焦点恢复及 health 成功/非法/失败重试；仍需验证旧设置迁移、真实浏览器/WebView 键盘与窄屏                                        |
