...
> **v9.40 更新说明**：客户详情的“仅已逾期”筛选使用 `GET /api/v1/clients/:id/followups?status=planned&due_state=overdue`，由 Sidecar 当前时钟按纳秒时间键判断计划是否早于当前时刻；可与负责人组合并保持严格分页。无新 migration、无外发消息、无 AI/LLM。app v0.1.0 / API v1 / schema v35 不变。
