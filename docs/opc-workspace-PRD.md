...
- **实现方法**：生产配置通过 `externalBin` 内置 Go Sidecar，开发配置通过 `OPC_SIDECAR_URL` 连接外部 Sidecar。Rust 为每个 generation 注入新令牌、通过端口 `0` 重新请求动态分配，并注入 `OPC_EXIT_ON_STDIN_CLOSE=true`；已启动代只有真实 `Terminated` 才按 500 ms、2 s 最多重拉两次，当前代连续 Ready 30 秒重置预算，外部模式、显式 shutdown 和无 `Terminated` 的事件流关闭不重拉。Go 在 pending restore、迁移和 DB open 前取得数据库父目录固定运行锁，并通过 stdout 输出固定 startup stage；Rust 拒绝未知协议并以 `sidecar_status.startupStage` 向恢复页提供无敏感信息的阶段文案，再取得 Artifact root 锁并协调 store。React 在非 ready 或 generation 改变时清连接与 TanStack Query。桌面启用 Tauri `tray-icon` feature，以代码生成的本地图标作为无 bundle icon 时的安全 fallback；仅在托盘构建成功后发布 available 状态并拦截主窗口 CloseRequested。托盘菜单只接受固定显示/退出 ID，退出调用 `app.exit(0)` 并复用现有 `shutdown\n`、父管道 EOF、并发 stop 和 WAL checkpoint 链。`desktop_capabilities` 只返回 `available / unavailable / not_implemented` 白名单枚举，React 严格解析并把能力读取与生命周期读取隔离。
- **关键路径**：`apps/desktop/src-tauri/src/lib.rs`、`apps/desktop/src-tauri/src/desktop_tray.rs`、`apps/desktop/src-tauri/src/desktop_log.rs`、`apps/desktop/src-tauri/src/sidecar.rs`、`tauri.conf.json`、`tauri.dev.conf.json`。
- **验证/剩余**：托盘源码单元测试覆盖固定菜单白名单、完成的左键点击门禁、fallback RGBA 尺寸/透明度、available 状态、关闭偏好决策和能力快照；Web 覆盖严格能力解析、畸形降级、设置即时预览/取消/保存和启动同步。Cargo metadata/lock 与 Rust 格式通过。当前主机缺 MSVC `link.exe`/Windows SDK，`cargo check/test` 在依赖 build script 链接前受阻，因此不能宣称托盘原生编译或交互通过。T-02 仍部分完成：托盘专注状态/业务动作、首次行为提示、真实 Tauri/Sidecar 父崩溃、OS 进程树、三平台和安装包未验收；没有 Job Object、进程组或孙进程治理，hard-hung orphan 只被锁挡住且不自动回收；启动前备份选择仍待实现。
