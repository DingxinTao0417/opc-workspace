...
> **v9.23 更新说明**：交付首个 v0.2 预设自动化纵向切片。schema v33 新增代码所有的五个稳定规则及不可变终态 Run 事实；当前可用 Project 完成→核对开票 Inbox、daily 今日任务 Reminder、weekly 周五复盘 Reminder，发票和 Agent 预设保持 unavailable。设置模块已接配置即时预览、IANA 当地时间、保存、启停、运行历史与失败重试；事件/计划执行具备事务动作、来源失败隔离、稳定去重、最多三次 attempt、DST 与离线窗口折叠。业务 JSON/ZIP 同步纳入规则与 Run；没有 Shell/SQL/HTTP、外发、自由规则、AI/LLM 或 Agent Runtime。app v0.1.0 / API v1 不变。
