...
| 明确边界     | 当前代码不使用 Docker；已实现 Task/Actor/D1/D2、Project、Client、Focus、Inbox/Reminder/Task 编排、已登记来源投影、首个受限预设自动化纵切、运行期数据库故障、可配置低空间安全投影、基础备份闭环、业务 JSON/ZIP 空工作区导入导出、脱敏诊断与启动恢复；通用托盘最小闭环已接源码但原生链接未验收；未实现 Client 外部来源、Focus 专用原生反馈、卷级趋势、数据库打开前恢复进度、复杂/原生通知、Agent、自由规则、非空目标/跨 schema 合并、AI、知识库或财务；自动化没有 Shell/SQL/HTTP/外发能力，person 只做本地责任记录，线上账号、云同步和远程协作均不在当前范围 |
