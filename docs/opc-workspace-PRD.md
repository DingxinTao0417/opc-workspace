...
> 文档日期：2026-08-28
> 实现基线：app v0.1.0 / API v1 / SQLite schema v22

> **v6.1 更新说明**：交付 Project Attachment 完整纵切。schema v22 新增受控项目附件、完整性事实、不可变删除墓碑、跨 Task/Client/Project object ID 唯一保护和 Project 聚合版本传播；API 支持稳定分页、严格 multipart 幂等上传、鉴权校验下载、带原因软删除与归档只读。项目永久删除、启动恢复、手动备份/恢复演练和业务 JSON 元数据导出已覆盖项目附件；项目详情新增文件预览、上传、下载、完整性、删除和历史交互。Inbox 来源投影仍待开发。
...
> **当前状态**：部分完成。当前 schema v22 保留并验证了由 schema v3–v5 交付的 Project model、CRUD、分页/搜索/筛选、快照式创建幂等、覆盖聚合事实的 `If-Match` 乐观锁、受控状态流转、归档恢复、确认硬删除，以及真实卡片/详情/任务聚合。schema v21 新增独立、版本化 Project Note；schema v22 新增受控 Project Attachment，支持幂等上传、稳定分页、鉴权完整性下载、带原因软删除、删除历史、归档只读和父项目删除补偿，并原子传播 Project 聚合版本。项目详情只读聚合所属 Task Artifact 和来源 Task/Submission 上下文，不复制产出或验收事实。Client CRUD、客户选择/改绑/解除和客户筛选已接通。Focus 完成会更新绑定 Task 工时并沿既有聚合链刷新 Project。Project 写命令继续同事务追加不可变 Workflow Event；项目笔记和附件不混入命令审计。Inbox 来源投影仍未实现。Task 六状态上线后 Project 仍只把 done 计为完成，cancelled 留在总数/剩余口径。
...
> **状态**：未开始；不属于 v0.1，目标版本和本地模型/运行时待单独评审。当前仓库没有模型 SDK、密钥、AI API、会话表或助手页面；PRD v6.1 当前阶段不接入线上模型服务。
...
首次启动不下载业务运行时或后端镜像。PRD v6.1 当前阶段不提供线上更新或第三方连接；安装、升级和核心使用均可在离线环境完成。
...
| 数据持久化       | Tauri `appDataDir`、SQLite 迁移、受控文件、手动/迁移前一致性备份、基础 JSON 导出与原子恢复 | **部分完成**：正式/开发隔离、WAL、外键、schema v22、重建迁移安全、设置/保存视图/客户活动/附件/person 关联/项目笔记/项目附件事实、数据库身份强绑定和共享受控文件 store 已实现；手动 SQLite+受控文件备份完整闭环、桌面一键安全重启及基础业务 JSON 导出已交付；迁移前备份、恢复诊断、数据导入和含文件导出包仍待实现；每日计划和高级导入归 v0.3 |
...
| API / Schema | API v1；SQLite schema v22                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
迁移文件位于 `services/sidecar/internal/database/migrations/`，通过 Go `embed` 编入 Sidecar。当前 schema 为 v22，新增结构只能从 `023_*` 继续追加递增文件：
...
| T-04 SQLite 初始化与迁移             | 已完成           | schema v22、PRAGMA、嵌入迁移、demo 清理、Project/Project Note/Project Attachment/Task/Actor、六状态、workspace identity、Submission/Artifact、Client/Activity/Attachment/Actor Link、Focus Session/interval/精确 Task ledger、Inbox、Reminder、app_settings 与 task_saved_views                       |
...
| T-08 项目管理                        | 部分完成         | CRUD、分页/搜索/筛选、创建幂等、乐观锁、受控状态、归档恢复、确认硬删除、卡片/详情、任务聚合、客户选择/筛选、可编辑人工笔记、受控项目附件、Task Artifact 产出聚合和追加式项目活动时间线；Inbox 来源投影待实现                                                                                          |
...
- **验证/剩余**：迁移测试覆盖连续旧版本数据保留、Task/Project/Client 版本与关系、Submission/Artifact 约束、Client 空值/索引/trigger、Focus interval/ledger、保存视图、客户扩展、Project Note 以及 v21→v22 Project Attachment 事实保留、约束、墓碑和版本传播。手动一致性快照、隔离恢复演练、重启原子恢复和业务 JSON 导出见 T-04B；迁移前自动备份、数据导入和含文件导出包仍未实现。
...
- **需求映射**：5.3。
- **当前状态**：部分完成；项目资料、基础生命周期、任务聚合、Client 客户关联、可编辑人工笔记、受控项目附件、只读 Task Artifact 产出聚合和追加式项目活动时间线已接通，Inbox 来源投影仍未完成。
...
- **建议开发流程**：
  1. 先完成本地运行时、资源预算、上下文权限和质量评测 ADR；PRD v6.1 当前阶段不评审远程 Provider。
...
2. **收口项目基础纵切**：CRUD、schema v4 幂等快照、schema v5 聚合版本、乐观锁、归档关联约束、状态流转、归档恢复、确认硬删除、任务聚合、Client 选择/筛选、schema v21 可编辑人工笔记、schema v22 受控项目附件、只读 Task Artifact 产出聚合和追加式活动时间线已交付；继续验证大数据量串行分页、真实浏览器/窄屏/焦点，并保留 Inbox 来源投影为明确缺口。
