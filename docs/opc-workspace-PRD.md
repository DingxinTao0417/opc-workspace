...
> **v9.16 更新说明**：交付 Project/Task 共用的服务端分页搜索 Client 选择器。Project 新建/编辑、Projects 客户筛选和 Tasks 客户筛选统一使用 `ClientSelect`，每次只读取 20 条，输入经 250 ms 防抖后复用既有 `GET /api/v1/clients?q=...&sort=name`，支持稳定上一页/下一页、请求取消、按 ID 去重、跨页或失败时保留当前选择，以及 active/lead/inactive 明确状态。加载、等待、空、错误重试、更多结果、显式清除和 combobox/listbox 键盘语义均已覆盖；已移除串行拉取全部 Client 的 `getAllClients`。真实浏览器、窄屏及 1,000/10,000 条客户性能专项仍未验收。app v0.1.0 / API v1 / SQLite schema v31 不变，无迁移；AI、知识库、客户回访和财务仍保持后续版本边界。
...
| T-08 项目管理                        | 部分完成                         | CRUD、分页/搜索/筛选、创建幂等、乐观锁、受控状态、归档恢复、确认硬删除、任务聚合与树/平铺/项目内检索分页、服务端分页搜索客户选择/筛选、笔记、项目附件、Task Artifact 聚合、活动时间线、项目级 Focus 分析/终态历史、显式 follow-up、Task 阻塞/临期与 Project 完成节点→Inbox；财务与真实里程碑增强待实现                                                                                                                                                                                                                                          |
...
| Client / Project 客户关联          | 通过         | Go 测试覆盖 Client CRUD/校验、分页/搜索/状态/稳定排序、创建快照幂等、PATCH/DELETE 并发、Invoice 冲突无副作用、Project 解除数量和双向版本传播；Web 测试覆盖单页 Query/取消、共享选择器搜索分页、跨页保留、Project 表单及 Projects/Tasks 客户筛选                           |
...
| `pnpm --filter @opc/web typecheck` | 通过         | 本次覆盖 Client 单页查询、AbortSignal、共享选择器及 Project/Task 三处接线；既有 Task D2、Client/Project/Inbox 契约继续通过                                                                                                                                                |
| 前端测试                           | 通过         | 本模块定向 7 个文件 / 44 项及全量 76 个文件 / 479 项通过，覆盖 ClientSelect 首屏/搜索/分页/跨页选中、inactive、反馈与键盘语义、单页 Query/实际取消，以及 Project 表单和 Projects/Tasks 筛选接线；既有父任务、任务、项目、设置等回归继续通过。组件测试不替代浏览器视觉验收 |
...
| 文档链接/过期措辞/diff             | 通过         | 本次 v9.16 文档更新后运行相对 Markdown 链接、绝对机器路径、过期状态和 `git diff --check` 审计                                                                                                                                                                             |
| Rust / Tauri 链接                  | 环境受限     | 当前主机缺少 MSVC `link.exe`（Visual C++ Build Tools / Windows SDK）；无法把 `cargo check`/Rust tests 或完整桌面链接声明为通过                                                                                                                                            |
| 浏览器渲染 / 窄屏视觉              | 本次未验收   | ClientSelect 已由组件测试覆盖，尚无本轮真实浏览器键盘、焦点、Portal 窄屏浮层及 1,000/10,000 条客户性能证据                                                                                                                                                                |
...
2. **收口项目基础纵切**：CRUD、schema v4 幂等快照、schema v5 聚合版本、乐观锁、归档关联约束、状态流转、归档恢复、确认硬删除、任务聚合、共享服务端分页搜索 Client 选择/筛选、schema v21 项目笔记、schema v22 项目附件、只读 Task Artifact 聚合、活动时间线、项目级 Focus 报告/终态历史、schema v23–v25 Task 相关来源、schema v28 Project 完成节点→Inbox 和 schema v31 complete/reopen→Client Activity 已交付；下一步验证 ClientSelect 真实浏览器/窄屏/焦点与 1,000/10,000 条数据，并独立消除 Task 表单 Project 选项仍串行拉全分页的问题。
...
| 3    | 项目                      | CRUD、schema v4 快照式创建幂等、schema v5 聚合版本、乐观锁、归档关联约束、状态流转、归档恢复、确认硬删除、任务派生进度/工时、Client 选择/筛选、schema v21 Project Note、schema v22 Project Attachment、Project Workflow Event、只读 Task Artifact 聚合、项目级 Focus 读模型、schema v23 follow-up、schema v28 Project 完成→Inbox 及 schema v31 complete/reopen→Client Activity 已交付             | 卡片、分页/搜索/状态/客户筛选、新建/编辑、共享 ClientSelect 服务端搜索分页、详情任务树/平铺、项目内组合筛选与分页、工时、人工笔记、受控附件、产出聚合、7 天/30 天/本月 Focus 报告与终态历史、归档恢复和活动时间线已交付                     | 已覆盖 ClientSelect 搜索/分页/跨页保留/反馈/键盘接线、项目 Focus canonical UUID/400/404、归档/空态、当前 Task 归属重分类、completed-only DST 报告、终态分页与独立反馈，以及事件/笔记/附件/产出/完成来源/客户动态原子投影/任务浏览器；后续验收真实浏览器、大数据量、项目产出→拆分→分派→验收         |
