...
> **v8.2 更新说明**：Focus D2b 周期报告新增标签时间分布。API 按 Session 绑定 Task 的查询时当前 `task_tags` 归类 completed interval；多标签为非互斥覆盖，同一秒可分别计入多个标签，删除或改绑后历史 Session 随当前关系重新归类；无任务或无当前标签统一进入“未加标签”。Focus 页明确展示该口径、标签颜色、分钟和块数；schema 保持 v28，原生反馈仍待后续。
...
> 状态截止：2026-08-28。当前版本是可运行、可扩展的 v0.1 基座；T-18A/B/C/D D1/D2、T-12 Focus Core A+B+C/D1/D2a/D2b 项目/标签/小时分布与二维热力图、T-11A1/A2/A3/B/C/F、T-11E follow-up Artifact/Task 阻塞/Task 临期/Project 完成来源、备份四类操作失败、数据库启动/迁移和 Sidecar 启动失败投影、T-06A–H、T-07A–D 和 T-04B 一致性备份完整闭环/迁移前自动回滚包/桌面安全重启/基础业务 JSON 导出已经交付，但这不代表第 9.1 节的完整 MVP、Focus 原生反馈、运行期数据库等其他系统故障来源、完整日志/恢复页、Agent、数据导入、含文件导出包或恢复诊断已经交付。
