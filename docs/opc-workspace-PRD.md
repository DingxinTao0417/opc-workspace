...
> **v9.58 更新说明**：内容日历 CC5-B2 已接入审核/发布到期的启动补偿与 15 秒周期扫描、版本化幂等投影、变更/发布/归档终结、删除协调和 Inbox 来源详情。单批 100 条可连续推进；不调用外部发布平台。schema 仍为 v38，无 AI/LLM、外发或 demo 数据。
...
> **当前状态**：路线图 `/roadmap` 已完成基础真实本地界面，R2 的 schema v36/API 提供 CRUD、筛选、项目关联、只读派生进度、乐观锁、归档恢复与重排；当前界面已接季度/状态浏览、新建、Project 深链和归档恢复，编辑、删除、拖拽与 Inbox/Reminder 自动化仍待。内容日历已交付 schema v37 数据/API、六周月格、新建/编辑、IANA/DST 安全改期、人工发布确认、Task 关联及 schema v38 Inbox 来源契约，并完成审核/发布到期的启动/周期扫描、版本化去重、变更终结、删除协调与 Inbox 来源详情。目标版本为 v0.3。
...
| 前端测试                           | 通过         | 全量 88 个文件 / 566 项通过；新增 Content Item Inbox 严格解析、来源快照和删除后链接降级覆盖，既有内容日历、路线图、启动进度、Agent Adapter、Automation、Reminder、内部回滚容量、业务与连接世代回归继续通过。组件测试不替代真实 WebView/休眠或父崩溃验收                                      |
...
| 统一源码质量门禁                   | 通过         | 本轮直接完成 `pnpm check:source` 全部门禁：Prettier、188 个 Go 文件与 Rust 格式、26 份 Markdown/本地链接、Web 类型/88 文件 566 项测试/构建、Go 无缓存测试/vet 与 x86_64-pc-windows-msvc Sidecar 构建                                                                                         |
