...
> 文档日期：2026-08-29
> 实现基线：app v0.1.0 / API v1 / SQLite schema v34

> **v9.26 更新说明**：交付数据库打开前的桌面启动进度纵切。Sidecar stdout 在 ready 前只输出固定 stage code，覆盖数据库运行锁、pending restore 检查/验证/应用/复验/收尾、数据库迁移、工作区初始化和本地 API 启动；Tauri 严格校验协议并把受控 `startupStage` 映射为恢复页文案。该链路不发送路径、备份 ID、令牌、原始错误或用户输入，也不增加 schema/API 路由。启动前备份选择、真实父进程崩溃/进程树、三平台和安装包仍待后续。app v0.1.0 / API v1 / SQLite schema v34 不变。
...
> **当前状态**：核心本地搜索、本地最近使用、设置运行诊断入口、脱敏诊断包 v1、Sidecar/Tauri 壳脱敏轮转日志、桌面打开日志目录、WebView→Sidecar request ID、全局渲染错误恢复和包含数据库打开前白名单进度的全局 Sidecar 启动故障恢复页已完成。支持已交付页面/安全操作命令、真实 Task/Project/Client/活动 Inbox 统一搜索、精确可刷新详情路由和完整键盘交互；空查询可展示有上限的非敏感最近命令/资源，并在确认资源 404 后清理。OS 全局快捷键和启动前备份选择仍待开发。
...
schema v31 同样是普通非破坏性迁移：只为 `client_activities` 的 Project Workflow Event 系统来源增加部分唯一索引，不改现有行、不增加 `source_id` 外键，也不回填历史 Project 事件。schema v32 继续以普通加法列、索引和 trigger 扩展 Reminder；既有行只回填 `series_id=id / none / 1 / UTC / occurrence=1`，不改变标题、状态、触发时间、Inbox 引用或事件。schema v33 只新增空的 `automation_rules/automation_runs` 及身份、形状、去重、重试和不可变约束；迁移不插入预设或 Run，Sidecar 启动幂等登记代码所有的默认禁用规则。schema v34 继续以普通加法迁移新增空的 `agent_adapters`，只保存代码所有清单、启停和脱敏诊断状态，不创建 Adapter/Actor/Assignment/Run。后续迁移从 `035_*` 继续。
...
| 数据持久化       | Tauri `appDataDir`、SQLite 迁移、受控文件、手动/迁移前一致性备份、基础 JSON 导入导出与原子恢复 | **部分完成**：正式/开发隔离、WAL、外键、schema v34、受控文件、版本化设置、数据库身份强绑定、备份/自动回滚包容量准入、恢复诊断，以及业务 JSON/含文件 ZIP 的空工作区同 schema 安全导入导出已交付；v32 扩展 Reminder，v33 新增空 Automation Rule/Run 表并由 Sidecar 幂等登记默认禁用预设，v34 新增空 Agent Adapter 表且迁移不登记 Adapter；数据库打开前实时进度页和高级冲突导入仍待后续                                                               |
...
> 状态截止：2026-08-29。当前版本是可运行、可扩展的 v0.1 基座，并已前置交付 v0.2 首个受限预设自动化纵切。schema v34、Project 完成→本地核对 Inbox、daily/weekly 本地 Reminder、设置预览/保存/启停、Run 历史/重试、IANA/DST、离线折叠和导入导出以及 Adapter 登记/安全诊断已经接通；这不代表完整 v0.2、Agent Runtime、发票/客户回访、自由规则、Task 自动化动作、Focus 原生反馈、复杂通知、数据库打开前恢复进度或冲突合并导入已经交付。
...
| API / Schema | API v1；SQLite schema v34                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| T-04 SQLite 初始化与迁移             | 已完成                           | schema v34、PRAGMA、嵌入迁移、demo 清理、Project/Task/Actor/Client/Focus/Inbox/Reminder、设置/保存视图、三类业务附件、Workspace Avatar、存储阈值、Submission origin、父任务 rollup guards、Project→Client Activity 唯一来源、Reminder 系列/occurrence、Automation Rule/Run、Agent Adapter 诊断事实与其他来源 guards                                                                                                                                                                        |
...
| T-13 命令面板与基础反馈              | 核心搜索、诊断与恢复反馈完成     | WebView 快捷键、已交付页面命令、Task/Project/Client/活动 Inbox 统一本地搜索、稳定详情路由、设置模块直达、combobox/listbox、焦点圈闭/恢复、IME 保护、加载/错误/重试/空状态、非敏感最近使用/404 资源清理、脱敏运行诊断/诊断包 v1、Sidecar/Tauri 壳脱敏轮转日志、打开日志目录、WebView→Sidecar request ID、全局渲染错误恢复与数据库打开前白名单进度的全局启动故障恢复页；OS 全局快捷键和启动前备份选择待后续                                                                                  |
...
- **实现方法**：schema v11 以 `focus_sessions`、`focus_session_intervals`、`task_focus_totals` 保存事实。API 快照统一返回 `session/server_now/elapsed_seconds/remaining_seconds` 并为 Session 返回 ETag；现有 Session 命令强制 `If-Match`，create/stop/cancel 支持 `Idempotency-Key`。Sidecar 启动把 active 转为 recovery_pending，每 15 秒刷新 heartbeat 且不递增 version。stop→completed 在一个事务内结算 interval、累计精确秒数、只把新增完整分钟写入 Task并每次递增 Task version；只有 `actual_minutes` 实际增加时既有 trigger 才递增 Project 聚合版本。cancel/interrupted 不入账。终态历史按 `ended_at DESC, id ASC` 分页；周期统计按显式 IANA 时区将 completed 的闭合正时长 interval overlap 切到 1–93 个本地日，并派生 distinct Session、秒/分钟和 Streak。历史与周期接口可选 canonical UUID `project_id`，不存在 404、归档可读，并按 Task 查询时当前 Project 归属分类；无 Task/Task 已删除/当前无项目不进入项目过滤。前端 TanStack Query 共享活动、历史与报告快照，并以 Project/日期/页码隔离 key；本地 persist store 只保存 work/break/cycle 编排与绝对休息截止时间。该 Focus 扩展没有新增自己的迁移；当前整体基线为 API v1/schema v34。
...
| schema v31→v32 与重复 Reminder     | 通过         | 定向数据库/API 测试覆盖既有一次性事实保留、系列与 occurrence 唯一约束、daily/weekly 创建编辑、IANA 时区、DST 当地钟点、离线跳过积压、重复扫描幂等及导入预检；最终继续迁移至 schema=34                                                                                                        |
...
| 前端测试                           | 通过         | 全量 83 个文件 / 537 项通过；新增严格 `startupStage` 运行期契约与恢复页白名单进度文案覆盖，既有 Agent Adapter、Automation、Reminder、内部回滚容量、业务与连接世代回归继续通过。组件测试不替代真实 WebView/休眠或父崩溃验收                                                                   |
| Web 生产构建                       | 通过         | `pnpm build:web` 的 TypeScript 与 Vite 构建通过；当前主入口 minified 846.70 kB、gzip 228.96 kB，仍有非阻断的 500 kB chunk 警告，后续按页面拆包                                                                                                                                               |
| 前端格式                           | 通过         | Web 范围 Prettier format check 通过                                                                                                                                                                                                                                                          |
| Go 全量测试与静态检查              | 通过         | `go test ./... -count=1` 与 `go vet ./...` 均通过；不虚构未统计的用例总数                                                                                                                                                                                                                    |
| 统一源码质量门禁                   | 通过         | 本轮以等价分项完成 `check:source` 全部门禁：Prettier、171 个 Go 文件与 Rust 格式、26 份 Markdown/本地链接、Web 类型/83 文件 537 项测试/构建、Go 无缓存测试/vet 与 x86_64-pc-windows-msvc Sidecar 构建                                                                                        |
...
| 8    | 设置与命令面板            | Actor API、health/version、Tauri Sidecar 生命周期、schema v16 app_settings、schema v27 受控头像、Query committed、旧 localStorage 迁移、手动备份/恢复/业务导出、诊断包、Sidecar/Tauri 壳轮转日志、打开日志目录、WebView→Sidecar request ID、全局启动故障恢复页 v1、schema v33 自动化 API、schema v34 Agent Adapter 诊断 API 及统一 search API 已接入                                                  | “人员与责任”支持 owner/person；个人资料支持名称与头像即时预览/保存/取消；设置支持 10 个左栏模块，本地 Agent 可登记内置清单、检查安全闸门且未就绪时保持禁用，自动化在模块内独立预览/保存/启停/重试，运行诊断可重查、复制脱敏摘要、下载诊断包并打开日志目录；命令面板支持多实体搜索和稳定详情路由 | 已覆盖设置/头像/备份/导出、自动化、Agent Adapter 登记/诊断、诊断包白名单与隐私、双进程日志、request ID、启动门禁、搜索契约、IME、焦点恢复、health 与桌面状态脱敏；仍需真实 WebView/窄屏验收，通知、OS 快捷键、平台 Agent 隔离验证和数据库打开前恢复进度仍为后续                                                                      |
