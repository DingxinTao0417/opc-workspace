...
> **v4.1 更新说明**：交付 T-06H Today 行内编辑/删除入口：编辑按钮直达共享完整任务详情，不复制第二套字段不完整的编辑器；删除按钮先显示不可恢复的独立二次确认，再以当前 Task 版本调用既有确认删除 API。成功刷新 Task/Project/Today/Inbox，版本冲突刷新事实并要求按最新内容重试，活动 Inbox 关系保护会明确引导先解除关联。
...
> **状态**：未开始；不属于 v0.1，目标版本和本地模型/运行时待单独评审。当前仓库没有模型 SDK、密钥、AI API、会话表或助手页面；PRD v4.1 当前阶段不接入线上模型服务。
...
首次启动不下载业务运行时或后端镜像。PRD v4.1 当前阶段不提供线上更新或第三方连接；安装、升级和核心使用均可在离线环境完成。
...
- **建议开发流程**：
  1. 先完成本地运行时、资源预算、上下文权限和质量评测 ADR；PRD v4.1 当前阶段不评审远程 Provider。
