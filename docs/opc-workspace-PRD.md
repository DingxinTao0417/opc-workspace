...
> 文档日期：2026-08-28
> 实现基线：app v0.1.0 / API v1 / SQLite schema v19

> **v5.3 更新说明**：交付 v0.1 Client 受控附件纵切。schema v19 新增 `client_attachments` 与不可变删除墓碑；公开 API 支持严格 metadata-first multipart 上传、稳定分页、完整性校验下载、带原因软删除和幂等重放，附件变化传播 Client 聚合版本。客户详情接入真实附件上传、下载、删除与历史；Task Artifact 与 Client Attachment 共用受控对象存储、启动协调、备份/恢复和业务导出边界。外部来源投影、Actor 显式关联和 v0.4 回访/财务仍未实现。
...
> **当前状态**：部分完成。当前 schema v19 保留并验证了由 schema v3–v5 交付的 Project model、CRUD、分页/搜索/筛选、快照式创建幂等、覆盖聚合事实的 `If-Match` 乐观锁、受控状态流转、归档恢复、确认硬删除，以及真实卡片/详情/任务聚合；v12–v19 的 Inbox、Reminder、编排、设置、保存视图、客户活动与客户附件迁移均不改 Project 结构。Client CRUD、客户选择/改绑/解除和客户筛选已接通。Focus 完成会更新绑定 Task 工时并沿既有聚合链刷新 Project；项目级产出/附件/事件与 Inbox 来源投影仍未实现。Task 六状态上线后 Project 仍只把 done 计为完成，cancelled 留在总数/剩余口径。
...
> **当前状态**：部分完成。schema v10 的 Client 基础资料和 Project 客户关联、schema v18 的本地人工活动时间线、schema v19 的受控附件均已交付；外部来源投影、Actor 显式关联、回访、发票详情和财务聚合仍未实现。
...
- 待实现：外部活动来源投影、Actor 显式关联、关联发票详情和财务聚合；线上下载或邮件事件只有在未来明确集成后才能出现。
...
> **状态**：未开始；不属于 v0.1，目标版本和本地模型/运行时待单独评审。当前仓库没有模型 SDK、密钥、AI API、会话表或助手页面；PRD v5.3 当前阶段不接入线上模型服务。
...
首次启动不下载业务运行时或后端镜像。PRD v5.3 当前阶段不提供线上更新或第三方连接；安装、升级和核心使用均可在离线环境完成。
...
| 客户管理         | 客户列表表格、客户详情、基本 CRUD                                                          | **部分完成**：基础资料 CRUD、分页/搜索/状态筛选/排序、创建幂等、并发控制、基础详情、受约束删除、Project 关联、本地活动和受控附件已实现；外部来源投影、Actor 关联、回访和财务待实现                                                                                                                            |
...
| 数据持久化       | Tauri `appDataDir`、SQLite 迁移、受控文件、手动/迁移前一致性备份、基础 JSON 导出与原子恢复 | **部分完成**：正式/开发隔离、WAL、外键、schema v19、重建迁移安全、设置/保存视图/客户活动/附件事实、数据库身份强绑定和共享受控文件 store 已实现；手动 SQLite+受控文件备份完整闭环、桌面一键安全重启及基础业务 JSON 导出已交付；迁移前备份、恢复诊断、数据导入和含文件导出包仍待实现；每日计划和高级导入归 v0.3 |
...
| API / Schema | API v1；SQLite schema v19                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 数据默认值   | 开发数据库默认空白，不自动注入 demo 业务数据                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 明确边界     | 当前代码不使用 Docker；已实现 Task 事实层、Actor/Assignment、D1 生命周期、Task 时间线、D2 manual Submission/Artifact/受控文件、Client 基础资料/Project 客户关联/人工活动时间线/受控附件、Focus Core A+B+C、手工 Inbox 受理/分诊、Reminder、Inbox Task 批量拆分/分派/自动结清、基础备份闭环及业务 JSON 导出；未实现 Client 外部活动来源/Actor 显式关联、Focus D、非 Reminder 来源投影、重复/原生通知、Agent 执行、数据导入/含文件导出、AI 助手、知识库、客户回访或收入/支出/发票业务；person 只做本地责任记录，线上账号、云同步和远程协作均不在当前范围 |
...
| T-04 SQLite 初始化与迁移             | 已完成            | schema v19、PRAGMA、嵌入迁移、demo 清理、Project/Task/Actor、六状态、workspace identity、Submission/Artifact、Client/Activity/Attachment、Focus Session/interval/精确 Task ledger、Inbox、Reminder、app_settings 与 task_saved_views                                                                  |
...
| T-09 客户管理                        | 部分完成          | 基础资料 CRUD、列表/详情、创建幂等、乐观锁、删除约束、项目数聚合、Project 客户关联、本地活动和受控附件已交付；外部来源投影、Actor 关联、回访/财务待实现                                                                                                                                               |
...
- **实现方法**：GORM 使用纯 Go SQLite 驱动和单物理连接，启用外键、WAL 与 5 秒 busy timeout。schema v1–v14 依次建立核心、Project、Task、Actor/Assignment/Event、Submission/Artifact、Client、Focus、Inbox、Inbox–Task 关系与 Reminder；schema v15 增加 Inbox 自动结清索引和数据库校验 trigger；schema v16 新增空的版本化 `app_settings`；schema v17 新增空的版本化 `task_saved_views`；schema v18 新增空的 `client_activities`、软删除审计、时间线索引和 Client 版本传播；schema v19 新增空的 `client_attachments`、同客户活动约束、软删除墓碑、对象 ID 跨表唯一和 Client 版本传播，均不改写既有模块事实或创建 demo 数据。不可变 `database_id`、一次性 `artifact_store_id` 与 marker 的 database/store ID 双向绑定，阻止错库接管或数据库静默换文件根。标有 foreign-keys-off 的迁移在同连接事务外关外键、事务内校验 `foreign_key_check`，再提交或整体回滚并恢复。开发数据库/受控文件位于 `.local/dev-data/`，生产数据位于 Tauri `appDataDir`。
...
- **需求映射**：5.4。
- **当前状态**：部分完成；Client 基础资料 CRUD、表格、详情、Project 客户关联、本地人工活动和受控附件已交付，外部来源投影、Actor 显式关联、回访和财务仍未实现。
- **用户流程**：用户可分页搜索/按状态筛选客户，新建或编辑名称、联系人、邮箱、电话、备注与状态；详情展示完整关联项目、真实活动时间线和受控附件。用户可选择本地文件、预览名称/大小后上传，下载时由 Sidecar 校验完整性，删除必须填写原因并可查看删除历史；停用后可恢复，inactive 状态可二次确认永久删除。列表、表单、活动和附件均覆盖加载、空、错误、重试与分页；版本冲突刷新最新事实但保留可恢复的本地输入。
- **实现方法**：schema v10 建立 Client 聚合版本和 Project 传播；schema v18 建立活动事实、审计与版本传播；schema v19 建立附件事实、同 Client 活动约束、软删除墓碑、对象 ID 跨表唯一保护与版本传播。Client CRUD 使用稳定分页、快照幂等、`ETag`/`If-Match` 和受约束删除。附件上传采用 metadata-first multipart、单文件 50 MiB/请求 100 MiB 限制及可选 `Idempotency-Key`；文件 staging、数据库事务、对象提交与失败补偿组成受控生命周期。下载逐字节验证 size/SHA-256；删除强制附件 `If-Match + confirm=true + reason`，Client 永久删除同步 trash/墓碑/清理全部附件。共享 store 启动协调、备份/恢复和业务导出同时覆盖 Task Artifact 与 Client Attachment。
- **关键路径**：`services/sidecar/internal/database/migrations/010_client_facts.sql`、`018_client_activities.sql`、`019_client_attachments.sql`、`internal/models/client.go`、`internal/models/client_activity.go`、`internal/models/client_attachment.go`、`internal/api/clients.go`、`client_activities.go`、`client_attachments.go`、`artifact_store.go`、`apps/web/src/pages/ClientsPage.tsx`、`pages/ClientDetailPage.tsx`、`components/ClientActivitiesSection.tsx`、`components/ClientAttachmentsSection.tsx`。
- **验证/剩余**：迁移、CRUD/校验/null、分页/过滤/排序、创建幂等、旧版本写入、删除确认/Invoice 强引用无副作用、Project 关联版本传播、活动版本/审计，以及附件上传/关联约束/完整性下载/软删除/历史/Client 硬删除/启动协调/备份恢复/业务导出均有自动化覆盖；前端覆盖 Client API/Query、表单、列表、详情、活动和附件交互。真实浏览器键盘/焦点/窄屏和大数据量性能仍需专项验收。累计收入在 v0.4 只从 `confirmed` Financial Entry 聚合；paid Invoice 只用于关联与对账，不在 Client 中重复存储。
...
- **建议开发流程**：
  1. 先完成本地运行时、资源预算、上下文权限和质量评测 ADR；PRD v5.3 当前阶段不评审远程 Provider。
...
3. **收口客户基础事实并推进真实扩展**：schema v10、Client CRUD/搜索/删除约束、基础详情和 Project 客户关联，schema v18 人工活动时间线，以及 schema v19 受控附件已交付；继续做真实浏览器/大数据量验收，再按独立纵切实现来源投影和 Actor 显式关联。回访与财务仍属于 v0.4。
...
| 4    | 客户                      | schema v10 基础事实、schema v18 活动事实和 schema v19 附件事实已交付：Client CRUD/搜索/状态/稳定排序、创建幂等、聚合版本、删除约束、项目数/最近活动派生，人工 note/meeting，以及受控附件上传/下载/软删除/历史；来源投影/Actor 关联及财务聚合待实现 | 表格、新建/编辑、基础详情、完整关联项目、停用/恢复、危险区、真实活动时间线和附件区已交付；回访/发票详情仍明确为后续且不伪造线上行为                                            | 已覆盖字段校验、迁移、分页筛选、并发、活动幂等/只读/软删除、附件生命周期/完整性/备份恢复、受约束 Client 删除和关联传播；剩余真实浏览器/大数据量、来源投影/Actor 关联，回访/财务保持 v0.4         |
...
已交付：Task 事实与项目关联 + Project 基础纵切
  ...
  → 已交付：Client 基础 CRUD + Project 客户选择/筛选 + Client 人工活动时间线 + 受控附件
  → Client 来源投影/Actor 关联 + Project 活动与事件增强
...
在对应纵切完成前，客户回访、发票“新建”和收入时间范围等无业务行为控件必须禁用或明确标记“后续版本”，不得用可点击外观暗示已经实现。Client 新建/编辑/基础详情/人工活动时间线/受控附件、Project 客户选择/筛选、Inbox“全部已读”/已有 Task 关系/批量拆分分派/自动结清，以及一次性 Reminder 已接入真实 API，不再属于占位清单；Client 外部来源投影、非 Reminder Inbox 来源投影、重复/原生通知、任务看板与生命周期批量操作仍未实现。
...
| DELETE               | /api/v1/client-attachments/:id         | 带原因软删除附件并审计                                                                   | 已实现；要求 `confirm=true`、附件 `If-Match` 和 1–1,000 字符原因，可选 `Idempotency-Key`；trash/墓碑/事务补偿后递增 Client 聚合版本                                                                                                                                   |
