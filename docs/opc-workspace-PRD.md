...
> **v3.8 更新说明**：交付 T-06E Today 明确改期：四个活动任务组均提供行内安排日期，支持任意日期、所选日期/后一天与未排期；单任务复用原子 batch `set_planned_date` 并携带版本，同日期不写入，冲突刷新事实，网络/超时仅在回读证明目标日期和版本前进后确认成功。跨分组拖拽及其他执行快捷操作继续延期。
...
> **当前状态**：部分完成。真实任务、持久化 Focus Session 和 Today Focus 汇总已接入，右栏读取真实活动 Session；任务已按所选本地日期完整分为逾期、当天、本周稍后和未排期，并支持日期导航。所选精确日期与未排期组支持按钮式/同组拖拽排序，所有活动行支持任意日期或未排期安排；跨分组拖拽、其他执行快捷操作、临期筛选、真实收入和客户动态尚未实现。
...
> **状态**：未开始；不属于 v0.1，目标版本和本地模型/运行时待单独评审。当前仓库没有模型 SDK、密钥、AI API、会话表或助手页面；PRD v3.8 当前阶段不接入线上模型服务。
...
首次启动不下载业务运行时或后端镜像。PRD v3.8 当前阶段不提供线上更新或第三方连接；安装、升级和核心使用均可在离线环境完成。
...
| ---------------- | ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 今日工作台       | 三栏布局、今日任务列表、手动排序、逾期及临期提示、任务详情操作、右侧概览面板                    | **部分完成**：日期导航、真实统计/四组任务、精确日期/未排期按钮式及同组拖拽排序、四组行内任意日期/未排期安排、共享任务详情、真实 Focus 概览与 completed-only 汇总已接通；跨分组拖拽、其他执行快捷操作、收入与客户动态待实现                                    |
...
- **当前限制**：跨分组直接拖拽、任务行内生命周期与开始专注快捷操作未实现。所选日期的周边界由浏览器本地时区计算；未来周任务不在当前所选周展示，需继续切换日期或从任务页查看。真实收入和客户动态未实现。
...
- **建议开发流程**：
  1. 先完成本地运行时、资源预算、上下文权限和质量评测 ADR；PRD v3.8 当前阶段不评审远程 Provider。
...
| 6    | 今日                      | 按本地日期、逾期和本周范围查询；完整计划组排序事务；版本化单任务改期；按 IANA 时区计算 UTC 边界；收件箱派生计数                                                                  | 日期导航、真实分组、按钮/同组拖拽排序及任意日期/未排期行内安排已交付；跨分组拖拽、生命周期/编辑/删除/专注快捷操作待开发；财务卡标后续                      | 已覆盖完整分页、排序集合/版本、终态槽位、乐观回滚、改期冲突和模糊响应验证；仍需真实浏览器验证日期控件、指针拖拽、键盘、窄屏及午夜/夏令时边界               |
