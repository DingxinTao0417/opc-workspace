...
> **v9.77 更新说明**：Tauri 托盘最小闭环已接入源码：托盘成功建立后，关闭主窗口隐藏但保留 Sidecar；左键或“显示”恢复窗口，“退出”进入现有优雅关闭链。托盘初始化失败时不拦截关闭，避免窗口不可恢复。当前主机缺 MSVC linker/Windows SDK，原生链接与三平台交互仍待验收。schema/API 不变。
...
- **验证/剩余**：托盘源码单元测试覆盖固定菜单白名单、完成的左键点击门禁、fallback RGBA 尺寸/透明度和 available 状态；Cargo metadata/lock 与 Rust 格式通过。当前主机缺 MSVC `link.exe`/Windows SDK，`cargo check/test` 在依赖 build script 链接前受阻，因此不能宣称托盘原生编译或交互通过。既有 Go 覆盖运行锁、父管道 EOF 和恢复阶段，Web 覆盖四态/generation。T-02 仍部分完成：托盘专注状态/业务动作/设置开关、真实 Tauri/Sidecar 父崩溃、OS 进程树、三平台和安装包未验收；没有 Job Object、进程组或孙进程治理，hard-hung orphan 只被锁挡住且不自动回收；启动前备份选择仍待实现。
...
| 前端测试                                           | 通过         | 全量 90 个文件 / 602 项通过；新增内容日历单条详情读取、跨月格 URL 指定详情、失败重试、关闭参数清理/保留上下文及 Inbox 精确链接回归，既有路线图 URL、目标日期排序、客户动态、启动进度、Agent Adapter、Automation、Reminder、内部回滚容量、业务与连接世代回归继续通过。组件测试不替代真实 WebView 验收 |
...
| 统一源码质量门禁                                   | 通过         | 本轮完成源码门禁：Prettier、193 个 Go 文件与 Rust 格式、26 份 Markdown/本地链接、Web 类型/90 文件 602 项测试/构建、Go 无缓存测试/vet 与 x86_64-pc-windows-msvc Sidecar 构建                                                                                                                          |
...
| 10   | 桌面与发布                | **部分完成**：generation-aware 两次有界重启、数据库运行锁、父管道 EOF、并发 shutdown、日志/request ID、全局恢复页、安全应用重启、固定命令面板/新建任务的原生快捷键注册/状态查询，以及托盘显示/隐藏/显式退出和不可用安全降级源码已交付；托盘原生链接/三平台交互、真实父崩溃/进程树、孤儿/孙进程治理、通知/自启/快捷键自定义/签名更新及三平台 CI 待实现                                                 | **部分完成**：starting/restarting/ready/error + generation、非 ready 连接/查询清理、状态重查、日志入口、安全重启、版本白名单和 fixed shortcut registered/unavailable 状态已交付；基础托盘交互不经 WebView，设置开关、专注状态/业务动作、启动前备份选择/恢复反馈与离线更新反馈待实现                                           | 本轮源码门禁已通过 Web 90 文件/602 测试、Go 无缓存全量测试/vet、文档、Sidecar 构建、Rust 格式与锁定 Cargo metadata。cargo check/test 受缺失 MSVC link.exe/Windows SDK 阻塞，托盘原生交互、真实父崩溃、进程树、签名、公证、干净机和三平台仍须验收                                                                                                                                     |
