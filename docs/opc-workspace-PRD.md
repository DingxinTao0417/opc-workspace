...
> 文档日期：2026-08-29
> 实现基线：app v0.1.0 / API v1 / SQLite schema v40

> **v9.70 更新说明**：T-11A3 本地 Reminder 已新增 monthly 规则。schema v40 在自动回滚包保护下重建 Reminder 约束，保留既有事实并增加服务端派生的当地日锚点；短月落在月末，后续长月恢复锚点日期。API/UI、导入预检和定向测试已同步；无 AI/LLM、外发或 demo 数据。
...
- `reminder_due`：用户创建的一次性或 daily/weekly/monthly Reminder occurrence 到期后生成收件箱项；Reminder occurrence 是调度事实，Inbox Item 是到期后的处理事实。重复系列在同一事务按 IANA 当地日历创建唯一下一 occurrence，monthly 以服务端派生的当地日为锚点。
...
- 当前阶段已实现的核心业务、Actor、任务分派、Submission/Artifact、手工 Inbox Item、Task 关系、一次性及 daily/weekly/monthly Reminder、已登记 Inbox 来源投影、批量拆分/分派、自动解决、设置、Focus、自动化预设与本地 Client Followup 均仅保存在本地，不主动访问线上服务；Agent Run 仍未实现
...
> 状态截止：2026-08-29。当前版本是可运行、可扩展的 v0.1 基座，并已前置交付 v0.2 首个受限预设自动化纵切。schema v40、Project 完成→本地核对 Inbox、daily/weekly/monthly 本地 Reminder、设置预览/保存/启停、Run 历史/重试、IANA/DST/月末、离线折叠和导入导出、Adapter 登记/安全诊断，以及客户回访的计划/终态数据契约已经接通；这不代表完整 v0.2、Agent Runtime、发票能力、自由规则、Task 自动化动作、Focus 原生反馈、复杂通知、数据库打开前备份选择或冲突合并导入已经交付。
...
迁移文件位于 `services/sidecar/internal/database/migrations/`，通过 Go `embed` 编入 Sidecar。当前 schema 为 v40，新增结构只能从 `041_*` 继续追加递增文件：
...
当前 `001_initial_schema.sql` 建立核心业务表和索引；002 清理旧固定 demo；003–031 依次增加核心 Project/Task/Actor/D2、Client、Focus、Inbox/Reminder、设置、受控文件和来源 guards；032 非破坏性增加 Reminder 系列、daily/weekly、IANA 时区和 occurrence 约束；033–039 增加预设 Automation、Agent Adapter 诊断、Client Followup、Roadmap/Content Item 及其 Inbox 来源；040 在自动回滚包保护下重建 Reminder 约束，保留已有事实并增加 monthly 与当地日锚点。迁移文件头可声明连续组合的 `-- migration: foreign_keys=off` 与 `-- migration: destructive`；迁移器在固定 `sql.Conn` 上于事务外关闭外键，事务内执行 SQL 与 `foreign_key_check`，提交/回滚后恢复外键，恢复失败与原错误一并返回；破坏性迁移还要求先生成并验证回滚包。数据库使用单物理连接，并启用外键、WAL 和 5000 ms `busy_timeout`；退出时执行 `wal_checkpoint(TRUNCATE)`。
...
| T-11 收件箱与工作编排中心            | 部分完成                         | T-11A1/A2/A3/B/C/F 已交付；A3 已含 schema v32 daily/weekly 与 schema v40 monthly Reminder、IANA/DST/月末推进、离线积压折叠和幂等下一 occurrence。拆分可继承/清除/改选可信来源 Project，写入独立完成条件并明确 person 本地责任，关系行可打开共享 Task；成功 mutation 会失效来源 Project，split 另失效 Task/Today/Project。T-11E 已交付既定来源和系统维护投影；工作日/自定义规则、原生通知、卷级趋势与 Agent 待实现                                                                          |
...
- **实现方法**：GORM 使用纯 Go SQLite 驱动和单物理连接，启用外键、WAL 与 5 秒 busy timeout。schema v1–v31 依次建立核心业务、Task/Actor/D2、Client、Focus、Inbox/Reminder、设置、受控文件与来源 guards；schema v32 为 Reminder 追加系列、daily/weekly、IANA 时区和 occurrence 约束；schema v33–v39 依次增加 Automation、Agent Adapter 诊断、Client Followup、Roadmap/Content Item 与本地 Inbox 来源；schema v40 在自动回滚包保护下关闭外键并重建 Reminder 表，复制既有事实、重建索引/trigger、执行 `foreign_key_check`，增加 monthly 与当地日锚点。迁移不创建 demo 数据。不可变 `database_id`、一次性 `artifact_store_id` 与 marker 双向绑定整个受控根。开发数据库/受控文件位于 `.local/dev-data/`，生产数据位于 Tauri `appDataDir`。
...
- **需求映射**：5.6。
- **当前状态**：部分完成。schema v12–v15 已交付手工受理分诊、已有 Task 关系、一次性 Reminder 和 T-11C 拆分编排；schema v32 已把 Reminder 扩展为 daily/weekly 系列与独立 occurrence，schema v40 又增加 monthly 与服务端派生当地日锚点，支持 IANA/DST/月末当地钟点、离线积压折叠、幂等下一 occurrence 和系列停止。T-11F 已交付实时运营计数、Today/Sidebar 展示和风险深链筛选；schema v23–v25 已交付显式 follow-up Task Artifact、Task 阻塞与提前 24 小时 Task 临期来源投影、稳定事件键、来源上下文与删除协调。系统维护已交付备份四类操作失败、数据库启动/迁移、Sidecar 启动、运行期数据库操作失败、可配置低空间投影、物理卷同卷去重和无路径手动容量检查。卷级趋势、工作日/自定义 Reminder、原生通知和 Agent Run 仍未实现；v0.1 不启用 AI/LLM/Agent。
...
- **分阶段纵切**：
  ...
  4. **T-11A3 Reminder（已完成）**：schema v14 一次性 `reminders` + schema v32 daily/weekly 系列/occurrence + schema v40 monthly/当地日锚点；创建/分页查询/详情、scheduled 编辑/改期/规则切换、带原因停止系列、ETag/幂等、启动补偿、15 秒扫描、IANA/DST/月末推进、离线积压折叠和稳定事件键 Inbox/下一 occurrence 投影。
...
- **验收要求**：当前纵切已覆盖纯离线手工受理、迁移保留、幂等/并发、事务回滚、全局未读、Task 关系/删除互锁、一次性及 daily/weekly/monthly Reminder、follow-up Artifact、Task 阻塞、Task 临期及安全系统维护来源；Reminder 专项覆盖 v31→v32 与 v39→v40 事实保留、系列/occurrence/锚点约束、API/前端规则切换、DST、短月夹取与恢复锚点、离线跨周期、重复扫描/重启、下一 occurrence 事务回滚与篡改导入预检。Go 金链覆盖一项直接 complete、一项 submit→waiting_review→accept，最终 Inbox automatic resolved/100%。真实浏览器/WebView、窄屏、焦点返回和大数据量仍待专项验收；Agent Run 是 v0.2 独立验收项，不能从本轮人工链路推断已实现。
...
| schema v31→v32 / v39→v40 与重复 Reminder | 通过         | 定向数据库/API 测试覆盖既有一次性/daily/weekly 事实保留、系列/occurrence/当地日锚点约束、daily/weekly/monthly 创建编辑、IANA 时区、DST 当地钟点、短月夹取后恢复锚点、离线跳过积压、重复扫描幂等及导入预检；最终继续迁移至 schema=40                                                          |
...
| 前端测试                                 | 通过         | 全量 89 个文件 / 590 项通过；新增路线图 Q4→下一年 Q1 键盘移动和 Q1→上一年 Q4 拖拽落点回归，既有路线图季度/精确日期/Inbox、内容日历预移与分页、启动进度、Agent Adapter、Automation、Reminder、内部回滚容量、业务与连接世代回归继续通过。组件测试不替代真实 WebView 验收                       |
| Web 生产构建                             | 通过         | `pnpm build:web` 的 TypeScript 与 Vite 构建通过；当前主入口 minified 931.91 kB、gzip 248.88 kB，仍有非阻断的 500 kB chunk 警告，后续按页面拆包                                                                                                                                               |
| 前端格式                                 | 通过         | Web 范围 Prettier format check 通过                                                                                                                                                                                                                                                          |
| Go 全量测试与静态检查                    | 通过         | `go test ./... -count=1` 与 `go vet ./...` 均通过；不虚构未统计的用例总数                                                                                                                                                                                                                    |
| 统一源码质量门禁                         | 通过         | 本轮完成源码门禁：Prettier、192 个 Go 文件与 Rust 格式、26 份 Markdown/本地链接、Web 类型/89 文件 590 项测试/构建、Go 无缓存测试/vet 与 x86_64-pc-windows-msvc Sidecar 构建                                                                                                                  |
