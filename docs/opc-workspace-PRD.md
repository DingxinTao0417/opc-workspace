...
> **v9.37 更新说明**：客户详情的回访时间线新增全部、待回访、已完成、已跳过和已取消筛选；筛选直接复用 `GET /api/v1/clients/:id/followups` 的 `status` 与严格分页，切换时回到第一页。无新 migration、无外发消息、无 AI/LLM。app v0.1.0 / API v1 / schema v35 不变。
