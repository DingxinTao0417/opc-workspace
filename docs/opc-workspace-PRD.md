...
| 右侧概览面板 | 右侧固定   | 280px        | 双形态：收起时仅 AI 助手页显示宽屏悬浮状态卡，展开时带页签停靠面板（概要/子智能体/文件/浏览器）；专注控制移至左侧边栏 |
...
- **工作区外壳**：亮/暗主题都以中性灰画布承托桌面工作区；主内容区在窗口四周保留间距，使用带边框和轻阴影的圆角工作面。左侧导航和右侧概览独立于主面；已启用的宽屏右侧概览可在当前会话收起/恢复，不改变业务状态或持久化默认值。
