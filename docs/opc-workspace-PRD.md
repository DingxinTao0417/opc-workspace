...
> **v9.15 更新说明**：交付 Project 生命周期到 Client Activity 的本地系统事实投影。已关联 Client 的 Project 每次 `complete / reopen` 都先写不可变 Workflow Event，再在同一 SQLite 事务中按该 Event ID 创建唯一 `system_reference` 客户动态；无 Client 不投影，迁移/启动不回填历史，后续改绑也不搬移旧事实。客户时间线显示“项目生命周期 / 项目状态变更 / 系统只读”，不暴露内部 source type 或事件 UUID；它只表示本地项目状态变化，不代表客户回访、邮件、外部沟通或线上行为。Project 状态、Workflow Event、Client Activity 及既有完成 Inbox 投影任一失败均整体回滚。API v1/app v0.1.0 不变；SQLite 升至 schema v31。
...
> **当前状态**：部分完成。当前 schema v31 保留并验证了由 schema v3–v5 交付的 Project model、CRUD、分页/搜索/筛选、快照式创建幂等、覆盖聚合事实的 `If-Match` 乐观锁、受控状态流转、归档恢复、确认硬删除，以及真实卡片/详情/任务聚合。schema v21 新增 Project Note，schema v22 新增 Project Attachment；schema v23–v25 已依次投影显式 `requires_followup` Artifact、所属 Task 阻塞事件和提前 24 小时 Task 临期，schema v28 已把 Project 完成周期投影为收尾 Inbox Item 并协调父项目删除，schema v31 又把已关联 Client 的 Project 完成/重开事件投影为客户系统动态。Client CRUD、客户选择/改绑/解除和客户筛选已接通。Focus 完成会更新绑定 Task 工时并沿既有聚合链刷新 Project；项目详情还可按 Task 查询时当前项目归属查看 7 天/30 天/本月 completed-only 报告与终态 Session 历史。Project 写命令继续同事务追加不可变 Workflow Event；项目笔记、附件和 Focus 读模型不混入命令审计。schema v31 不改变 Project 表，只增加 Client Activity 来源唯一性。独立验收/开票等没有真实 Project 状态的节点不提前投影。Task 六状态上线后 Project 仍只把 done 计为完成，cancelled 留在总数/剩余口径。
...
> **当前状态**：部分完成。schema v10 的 Client 基础资料和 Project 客户关联、schema v18 的本地人工活动时间线、schema v19 的受控附件、schema v20 的 person 显式关联均已交付；schema v31 已交付 Project 完成/重开的本地系统事实投影。外部来源集成、客户回访、发票详情和财务聚合仍未实现。
...
| 前端测试                           | 通过         | 本模块定向 4 个文件 / 11 项及全量 75 个文件 / 468 项通过，覆盖 Project transition 缓存、Client Activity 严格解析、项目生命周期可读/只读展示和客户详情契约；既有父任务、任务、项目、设置等回归继续通过。组件测试不替代浏览器视觉验收                     |
...
| 文档链接/过期措辞/diff             | 通过         | 本次 v9.15 文档更新后运行相对 Markdown 链接、绝对机器路径、过期状态和 `git diff --check` 审计                                                                                                                                                      |
| Rust / Tauri 链接                  | 环境受限     | 当前主机缺少 MSVC `link.exe`（Visual C++ Build Tools / Windows SDK）；无法把 `cargo check`/Rust tests 或完整桌面链接声明为通过                                                                                                                     |
| 浏览器渲染 / 窄屏视觉              | 本次未验收   | 父任务汇总与 Project 客户动态已由组件测试覆盖，尚无本轮真实浏览器键盘、焦点、长列表和窄屏视觉证据                                                                                                                                                   |
...
2. **收口项目基础纵切**：CRUD、schema v4 幂等快照、schema v5 聚合版本、乐观锁、归档关联约束、状态流转、归档恢复、确认硬删除、任务聚合、Client 选择/筛选、schema v21 项目笔记、schema v22 项目附件、只读 Task Artifact 聚合、活动时间线、项目级 Focus 报告/终态历史、schema v23–v25 Task 相关来源、schema v28 Project 完成节点→Inbox 和 schema v31 complete/reopen→Client Activity 已交付；下一纵切优先消除 Client 选择器全量串行拉取，再验证真实浏览器/窄屏/焦点。
3. **收口客户基础事实并推进真实扩展**：schema v10、Client CRUD/搜索/删除约束、基础详情和 Project 客户关联，schema v18 人工活动时间线、schema v19 受控附件、schema v20 person 显式关联，以及 schema v31 Project 生命周期本地系统动态已交付；下一纵切先做大客户量服务端搜索选择器和真实浏览器验收，再在具备真实集成来源时按独立纵切接入外部事实。回访与财务仍属于 v0.4。
...
| 3    | 项目                      | CRUD、schema v4 快照式创建幂等、schema v5 聚合版本、乐观锁、归档关联约束、状态流转、归档恢复、确认硬删除、任务派生进度/工时、Client 选择/筛选、schema v21 Project Note、schema v22 Project Attachment、Project Workflow Event、只读 Task Artifact 聚合、项目级 Focus 读模型、schema v23 follow-up、schema v28 Project 完成→Inbox 及 schema v31 complete/reopen→Client Activity 已交付 | 卡片、分页/搜索/状态/客户筛选、新建/编辑、Client 选择、详情任务树/平铺、项目内组合筛选与分页、工时、人工笔记、受控附件、产出聚合、7 天/30 天/本月 Focus 报告与终态历史、归档恢复和活动时间线已交付                                          | 已覆盖项目 Focus canonical UUID/400/404、归档/空态、当前 Task 归属重分类、completed-only DST 报告、终态分页与独立反馈，以及事件/笔记/附件/产出/完成来源/客户动态原子投影/任务浏览器；后续验收大数据量、项目产出→拆分→分派→验收                                            |
| 4    | 客户                      | schema v10 基础事实、schema v18 活动、schema v19 附件、schema v20 person 关联和 schema v31 Project 生命周期系统动态已交付：Client CRUD/搜索/状态/排序、聚合版本、删除约束、项目/活动/附件，以及显式 contact 关联/解除历史；外部来源集成及财务聚合待实现                                                                                                                                    | 表格、新建/编辑、基础详情、完整关联项目、停用/恢复、危险区、人工活动、只读项目状态动态、附件/本地联系人已交付；回访/发票详情仍明确为后续且不伪造线上行为                                                                                      | 已覆盖字段校验、迁移、分页/并发、活动审计、项目来源唯一/回滚/改绑历史归属、附件生命周期/备份恢复、person 原子创建/关联/解除、停用保护、Client 删除和版本传播；剩余真实浏览器/大数据量与外部来源集成，回访/财务保持 v0.4                                          |
