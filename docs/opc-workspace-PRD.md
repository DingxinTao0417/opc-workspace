...
> **v9.35 更新说明**：客户详情的“记录回访结果”可选展开下一次本地计划。`POST /api/v1/client-followups/:id/complete` 在同一 SQLite 事务写入完成事实、下一条计划和两条 Workflow Event；下一条负责人仍限制 active owner/person。创建失败（含负责人已停用）时本次完成也不会落库。无新 migration、无外发消息、无 AI/LLM。app v0.1.0 / API v1 / schema v35 不变。
