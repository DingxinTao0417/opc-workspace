...
> 文档日期：2026-09-03
> 实现基线：app v0.1.1 / API v1 / SQLite schema v56

> **v9.86 更新说明**：桌面宽屏左侧主导航新增显式收起/展开按钮，展开态 220 px、收起态 64 px；收起后保留带可访问名称和悬浮提示的模块图标及原位恢复入口。窄屏继续使用既有自动 64 px 图标栏，不重复显示手动按钮。状态属于当前前端运行期 UI 状态，不新增 SQLite/API/schema 事实。
...
> **状态**：首个纵向切片已交付（schema 052–056）。用户 2026-09-01 明确授权以 API key 配置式接入远程大模型并固化于 [ADR-004](adr/004-ai-assistant-provider-access.md)；用户 2026-09-03 授权 agent harness 运行时、本地大模型接入并固化于 [ADR-005](adr/005-agent-harness-and-local-models.md)，同日授权 harness 完整组件矩阵与自进化边界并固化于 [ADR-006](adr/006-harness-matrix-memory-evolution.md)；排期为独立轨道，不并入 v0.1–v0.4。已交付：Provider 登记/健康/密钥（OS 安全存储）、本地部署 Provider（OpenAI 兼容回环端点，无密钥）、多 Provider 管理与会话内切换、SSE 流式只读会话（含推理模型思考过程独立展示，schema 054）、取消/断连/超时/并发闸门、语义建任务建议卡片（确认后经既有任务 API 创建并跳转）、工具失败纠错重试与反思校验轮（ADR-006）、长期记忆（`ai_memories`，建议→人工确认→注入后续会话，schema 056）。Harness 运行循环/工具注册表机制已落地但生产不注册任何工具；上下文选择、知识库来源（仅预留扩展点）、多 Provider 自动路由与用量统计为后续纵切。
...
- Agent Harness 运行时：Sidecar `internal/harness` 承载调用/执行循环、工具注册表与执行器、预算与取消；生产注册表为空（单次调用，行为等同直连），为知识库检索等未来能力预留扩展点，启用任何真实工具须逐个评审授权。
...
- 模型输出只读，视为不可信预览；自动建任务、Shell/SQL/业务写、自主代理均禁止。Harness 的工具注册表生产为空——机制已建、能力未开，任何真实工具（含知识库检索）启用前必须逐个评审授权。
- 远程 Provider 外发代码所有系统提示、已确认长期记忆、所选会话预算内的最近完整回合和当前输入；不读取任务/项目/客户等业务数据。任务/项目/客户上下文选择与发送前预览、知识库检索与来源引用为后续纵切。
- 长程上下文管理设计固化于 [ADR-007](adr/007-session-context-compaction-and-memory-tools.md)：G1 的最新 200 条读取、完整回合窗口、历史助手控制块剥离、最终序列化请求预算与超限原子拒绝已交付；G2–G5 的摘要+关键事实、后台压缩快照及记忆工具仍待实施。其中记忆三工具将是对工具禁令的首次正式突破（仅限 memory_write/memory_propose/memory_search，其余工具仍须逐个评审授权）。
- 本地部署 Provider 仅限回环 OpenAI 兼容端点，不保存密钥；协议扩展与工具启用路径见 ADR-005。
- `ai_providers / ai_sessions / ai_generations / ai_messages / ai_memories` 五张 AI 表为操作态/隐私边界，排除出业务 JSON/ZIP 导出；一致性备份仍覆盖。
...
- 移动端伴侣 App
- 云同步（可选的端到端加密同步）
- AI 助手后续增强：业务上下文、摘要压缩、真实工具、知识库来源、自动路由与用量统计
- 本地知识库、文档索引与带来源检索
...
| **独立轨道（版本待定）** | AI 助手基础纵切与本地 Provider 已交付；本地知识库、文档索引/来源检索、会话摘要压缩、真实工具、自动路由与质量评测待开发 | 按 ADR 分阶段单独估算      |
...
| 明确边界     | 当前代码不使用 Docker；已实现 Task/Actor/D1/D2、Project、Client、Focus、Inbox/Reminder/Task 编排、已登记来源投影、首个受限预设自动化纵切、运行期数据库故障、可配置低空间安全投影、基础备份闭环、业务 JSON/ZIP 空目标及同 schema 零主键冲突追加、只读冲突/schema 方向预检、脱敏诊断、数据库打开前白名单启动进度与启动恢复；通用托盘最小闭环已接源码但原生链接未验收；未实现 Client 外部来源、Focus 专用原生反馈、启动前备份选择、复杂/原生通知、Agent、自由规则、真实冲突策略/UUID 重映射/跨 schema 升级、AI、知识库或财务；自动化没有 Shell/SQL/HTTP/外发能力，person 只做本地责任记录，线上账号、云同步和远程协作均不在当前范围 |
...
| T-15 AI 助手                         | 首个纵向切片完成                 | 远程/本地 Provider、OS 安全存储、流式会话与推理展示、Harness、自评修订、长期记忆确认注入、语义建任务确认卡片及最新完整回合窗口已交付；摘要压缩、生产工具、业务上下文、知识库来源、自动路由与步骤追踪待后续                                                                                                                                                                                                                                                                                                                                                        |
...
- **需求映射**：5.10、9.1、9.2。
- **当前状态**：首个纵向切片已交付。`/ai`、Provider/密钥/健康、远程双协议与本地回环模型、会话/消息/取消、独立 reasoning、Harness 预算/纠错/自评修订、长期记忆人工确认注入、语义建任务确认卡片和 G1 最新完整回合窗口均有代码与测试。
- **后续开发流程**：
  1. 按 ADR-007 G2–G5 实现摘要/关键事实压缩、两协议工具调用、受控记忆工具和前端透明化。
  2. 另行评审任务/项目/客户显式上下文、发送前范围预览与知识库来源；当前代码尚无 `ContextSource` 接口或检索实现。
  3. 增加固定问题集、无答案处理、提示注入、越权读取、本地模型真机、跨平台安全存储与质量评测。
...
- **版本条件**：独立于 v0.1–v0.4 持续交付；后续高风险工具和业务上下文逐项 ADR 授权。
...
| POST                 | /api/v1/ai/chat                           | 显式调用 AI 助手                                                                         | 已实现；`opc-ai-sse-v1` 返回 meta/delta/reasoning/可选 replace/done/error/cancelled，远程双协议与本地 OpenAI 兼容 Provider 共用 Harness、预算和取消门禁                                                                                                                                                                                                                                                         |
...
| v9.86    | 2026-09-03 | 桌面宽屏主导航新增品牌区收起按钮，220 px 展开栏可缩为 64 px 图标栏并原位恢复；收起态为搜索、模块与设置保留可访问名称/悬浮提示。窄屏沿用自动图标栏并隐藏重复按钮；状态仅属于 React 运行期，不新增数据库、API 或 schema。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
