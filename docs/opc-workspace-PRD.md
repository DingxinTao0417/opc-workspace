...
> **v6.4 更新说明**：交付 T-11E 第三项来源投影。schema v25 在 Sidecar ready 前补偿并每 15 秒扫描非终态 Task，将进入未来 24 小时窗口的截止事实以 `task:<task-id>:due:<due-at>` 稳定投影为 Inbox Item/system Event；每批最多 100 条并排除已投影事实，积压可持续推进。重复扫描/重启不重复，改期形成新的截止时点来源；详情展示临期/逾期分类、截止/投影时间、项目和 Task 入口。活动来源阻止 Task 删除，归档后删除协调保留快照与审计。系统故障来源仍待后续独立纵切。
...
> **状态**：未开始；不属于 v0.1，目标版本和本地模型/运行时待单独评审。当前仓库没有模型 SDK、密钥、AI API、会话表或助手页面；PRD v6.4 当前阶段不接入线上模型服务。
...
首次启动不下载业务运行时或后端镜像。PRD v6.4 当前阶段不提供线上更新或第三方连接；安装、升级和核心使用均可在离线环境完成。
...
| 明确边界     | 当前代码不使用 Docker；已实现 Task 事实层、Actor/Assignment、D1 生命周期、Task 时间线、D2 manual Submission/Artifact/受控文件、Project 笔记/附件/产出聚合/活动时间线、Client 基础资料/Project 客户关联/人工活动/附件/person 关联、Focus Core A+B+C、手工 Inbox/Reminder/Task 编排、follow-up Artifact/Task 阻塞/Task 临期来源投影与删除协调、基础备份闭环与业务 JSON 导出；未实现 Client 外部活动来源、Focus D2b、系统故障来源、重复/原生通知、Agent、导入/含文件导出、AI 助手、知识库、客户回访或收入/支出/发票业务；person 只做本地责任记录，线上账号、云同步和远程协作均不在当前范围 |
...
- **建议开发流程**：
  1. 先完成本地运行时、资源预算、上下文权限和质量评测 ADR；PRD v6.4 当前阶段不评审远程 Provider。
