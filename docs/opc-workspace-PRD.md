...
> **v9.74 更新说明**：今日右侧概览新增“临近项目节点”；路线图列表增加受控 `sort=target_date`，RightOverview 分别读取 planned/active 各 3 条、按目标日期与 ID 合并取最近 3 条，显示 Project 归属和派生 Task 完成数并进入路线图。schema 保持 v41，不复制项目/任务进度、不创建 demo 数据。
...
11. 临近项目节点分别读取 `status=planned/active&sort=target_date&page_size=3`，再以目标日期和 ID 合并取前 3 条；关联项目与任务完成数来自 Roadmap 响应的 Project/Task 派生事实，Today 不保存第二份进度
...
- **需求映射**：5.1。
- **用户流程**：进入首页后读取本地任务和今日统计，可新建任务、完成/恢复任务、打开详情编辑或删除，并从右侧概览开始/暂停专注、查看最近路线图节点或打开专注设置。
...
- **当前限制**：计划日期“逾期/本周”仍是多日期聚合，只有落到具体任务时才有不含猜测的目标日期；空组级落点仅对精确所选日期和未排期开放。截止日期风险视图是查询时动态读模型，不写第二份状态、不纳入静态保存视图，也不等同于可归档的 `task_due` Inbox 历史事项。跨日期的日期事实与两个顺序事务不是一个数据库事务，改期确认后排序失败不会反向覆盖日期，而是刷新并明确提示部分成功。需要原因的阻塞/取消以及 manual review 验收仍通过详情处理；快捷开始依赖活动负责人。路线图节点当前进入 `/roadmap` 列表，尚未用 URL 直接打开指定里程碑详情。真实收入未实现；客户动态只覆盖本地事实，不含外部线上行为。
...
| 前端测试                                           | 通过         | 全量 90 个文件 / 598 项通过；新增路线图目标日期排序契约及右栏 planned/active 合并、归属、加载/空/失败重试回归，既有客户动态、路线图、内容日历、启动进度、Agent Adapter、Automation、Reminder、内部回滚容量、业务与连接世代回归继续通过。组件测试不替代真实 WebView 验收                      |
...
| 统一源码质量门禁                                   | 通过         | 本轮完成源码门禁：Prettier、193 个 Go 文件与 Rust 格式、26 份 Markdown/本地链接、Web 类型/90 文件 598 项测试/构建、Go 无缓存测试/vet 与 x86_64-pc-windows-msvc Sidecar 构建                                                                                                                  |
...
| 13   | 路线图（v0.3）            | **R2/R5 已完成**：roadmap_milestones、季度/日期/状态/项目关联，进度从项目/任务派生；列表支持目标日期稳定排序；schema v39 增加本地到期/达成 Inbox 来源、扫描补偿、稳定去重、变更终结和删除协调                                                                                                                                                                                                         | **R3/R4 与 Today 读取联动已完成**：季度/年度、状态/Project 筛选、稳定分页、新增/编辑/按 ID 详情、项目关联/跳转、归档恢复和二次确认删除已交付；完整同季度集合可原子排序，年度 Q1–Q4 时间线支持跨季度/跨年度移动，季度三月日历支持精确日期拖拽和键盘日期输入；Today 右栏合并最近 3 条 planned/active 节点并显示 Project/Task 归属 | API 已覆盖目标日期排序/分页、季度/跨年边界、目标季度尾部排序、进度口径、项目删除约束、并发冲突、本地日期到期、达成转换、重扫去重和删除协调；Web 已覆盖 Today 合并/归属/状态、筛选分页、详情最新事实、编辑载荷、版本头、保护性删除、同季度重排、跨季度/跨年度/精确日期移动、失败回读、容量保护及 Inbox 来源解析。真实浏览器拖拽/焦点、窄屏及长列表仍待专项验收 |
