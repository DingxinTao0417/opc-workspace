...
> **v4.8 更新说明**：交付 T-04B 重启前安全恢复，不增加 SQLite schema。`POST /backups/:id/restore` 要求明确确认，在维护写锁内重复恢复演练并创建当前状态的完整自动回滚包，原子发布私有 pending package/plan 后冻结普通 API 和后台写入。下一次 Sidecar 启动在打开 live SQLite 与 Artifact lease 前验证目标/回滚包、迁移副本、交换数据库（含 WAL/SHM 恢复路径）与完整 objects，最终复验失败则回滚并隔离计划，成功先推进 applied 提交点再清理旧副本，避免重复应用。设置页提供二次确认、目标/回滚 ID 与关闭重开指引；桌面自动重启、备份删除、JSON 导出和迁移前自动备份继续独立实现。
...
> **状态**：未开始；不属于 v0.1，目标版本和本地模型/运行时待单独评审。当前仓库没有模型 SDK、密钥、AI API、会话表或助手页面；PRD v4.8 当前阶段不接入线上模型服务。
...
├─ backups/                  # 已实现创建/列表/校验/演练/重启恢复；删除/导出待实现
...
版本边界：手动一致性备份的创建、内部列表、完整校验、隔离恢复演练、恢复前自动回滚包和下一次 Sidecar 启动原子替换已经交付；v0.1 仍须交付迁移前备份、基础 JSON 导出、备份删除和桌面一键重启/恢复诊断。v0.3 只增加可配置计划、外部目录、保留策略、CSV 映射和高级导入工具。
...
首次启动不下载业务运行时或后端镜像。PRD v4.8 当前阶段不提供线上更新或第三方连接；安装、升级和核心使用均可在离线环境完成。
...
| 数据持久化       | Tauri `appDataDir`、SQLite 迁移、受控 Artifact、手动/迁移前一致性备份、基础 JSON 导出与原子恢复 | **部分完成**：正式/开发隔离、WAL、外键、schema v17、重建迁移安全、设置/保存视图事实、数据库身份强绑定和 Artifact store 已实现；手动 SQLite+Artifact 备份创建/列表/完整校验、隔离演练、自动回滚点及重启原子恢复已交付；迁移前备份、删除、导入导出和桌面一键重启仍待实现；每日计划和高级导入归 v0.3 |
...
> 状态截止：2026-08-28。当前版本是可运行、可扩展的 v0.1 基座；T-18A/B/C/D D1/D2、T-12 Focus Core A+B+C、T-11A1/A2/A3/B/C/F、T-06A–H、T-07A–D 和 T-04B 手动备份创建/列表/校验/隔离演练/重启恢复已经交付，但这不代表第 9.1 节的完整 MVP、Focus D、非 Reminder 来源投影、Agent、备份删除/导出、桌面一键重启或迁移前自动备份已经交付。
...
| 明确边界     | 当前代码不使用 Docker；已实现 Task 事实层、Actor/Assignment、D1 生命周期、Task 时间线、D2 manual Submission/Artifact/受控文件、Client 基础资料/Project 客户关联、Focus Core A+B+C、手工 Inbox 受理/分诊、Reminder、Inbox Task 批量拆分/分派/自动结清及基础备份恢复；未实现 Client 活动/附件/Actor 显式关联、Focus D、非 Reminder 来源投影、重复/原生通知、Agent 执行、备份删除/导出、AI 助手、知识库、客户回访或收入/支出/发票业务；person 只做本地责任记录，线上账号、云同步和远程协作均不在当前范围 |
...
| T-04B 手动一致性备份                 | 基础恢复闭环完成  | 专用 backup root、维护写锁、SQLite `VACUUM INTO`、active Artifact+marker+manifest、完整校验、同卷原子发布、创建幂等、列表/重新校验、隔离演练、自动回滚点、pending/applied 提交点、重启原子恢复及设置 UI 已交付；删除/导出、桌面一键重启和迁移前自动备份待后续纵切 |
...
| T-14 设置持久化                      | 核心闭环完成      | schema v16、四模块严格 schema、原子 PATCH、乐观锁、Query committed、启动门禁、旧值迁移和数据备份/恢复模块已交付；受控头像文件、备份删除/导出待开发                                                                                                                |
...
- **用户流程**：设置 → 数据与备份读取真实本地包；用户可填写说明并创建，成功后查看创建/校验时间、schema、Artifact 数和总大小，也可重新校验或隔离演练。点击“恢复此备份”后必须二次确认；成功挂起会显示目标/自动回滚短 ID，并要求关闭重开应用。
...
- **关键路径**：`services/sidecar/internal/api/backups.go`、`backup_drill.go`、`backup_restore.go`、`backups_test.go`、`cmd/server/main.go`、`internal/config/config.go`、`scripts/dev.mjs`、`apps/web/src/components/BackupSettings.tsx`、`api/client.ts`、`api/hooks.ts`。
- **验证/剩余**：Go 集成测试覆盖含真实 file Artifact 的快照边界、包结构、列表、幂等重放/冲突、显式校验、隔离演练、自动回滚点、pending 同目标重放/异目标冲突、业务冻结、启动恢复、目标/回滚内容、WAL 部分移动补偿、临时清理、篡改拒绝及输入校验；前端覆盖严格契约、确认请求序列化和设置入口交互。备份删除、JSON 导出、迁移前备份、大数据量进度/取消、桌面一键重启和真实磁盘故障仍未实现。
...
- **建议开发流程**：
  1. 先完成本地运行时、资源预算、上下文权限和质量评测 ADR；PRD v4.8 当前阶段不评审远程 Provider。
...
6. **补数据安全链路**：手动一致性备份的创建、列表、完整校验、维护写锁、原子发布、隔离演练、恢复前回滚点与重启原子恢复已经交付；下一步依次实现确认删除、基础 JSON 导出、迁移前自动备份、桌面一键重启和诊断包内容选择。
...
| 9    | 数据安全                  | 已交付 v0.1 手动一致性快照、manifest/SHA-256、维护写锁、临时库完整验证、同卷原子发布、隔离演练、自动回滚点和重启原子恢复；迁移前备份、删除和基础 JSON 导出待实现；v0.3 增加计划/映射 | 已交付手动创建、列表、重新校验、恢复演练、二次确认与关闭重开指引；删除和导出入口将在对应安全纵切实现时开放                                                                                 | 已覆盖 WAL 活跃、真实 Artifact、幂等、篡改拒绝、临时演练、pending 重放/冲突、写入冻结、启动替换、失败回滚和清理；仍须覆盖低磁盘、桌面一键重启和真实磁盘故障                                 |
...
已交付：Task 事实与项目关联 + Project 基础纵切
  ...
  → 已交付手动备份创建/列表/完整校验/隔离演练/重启恢复；继续删除/导出、桌面日志和故障恢复
  → 本地 Agent
  → 预设自动化与后续业务事件源
```
