...
> **v9.66 更新说明**：路线图 R5 已通过 schema v39 接入本地到期/达成 Inbox 事件；Sidecar 按本地日历日启动补偿和周期扫描，版本化事件键与目标日期二次判重避免无关编辑/排序重复提醒，并补齐变更终结、归档、删除协调和 Inbox 来源详情。当前提醒由 Inbox Item 承载，不额外创建 Reminder 行或原生通知；精确日期拖拽仍待，无 AI/LLM、外发或 demo 数据。
...
> **当前状态**：路线图 `/roadmap` 的 R3 基础界面已完成；R4 已交付完整同季度集合的安全卡片排序、年度 Q1–Q4 时间线、跨季度拖拽/键盘移动，以及 PATCH 跨季度追加到目标季度尾部的事实规则。R2 的 schema v36/API 提供 CRUD、筛选、项目关联、只读派生进度、乐观锁、归档恢复与重排；R5 的 schema v39 已交付本地到期/达成 Inbox 来源、启动/周期补偿、稳定去重、变更终结、删除协调与来源详情。当前界面已接季度/年度、状态/Project 浏览、稳定分页、新建、基础事实编辑、按 ID 详情、Project 深链、归档恢复、保护性永久删除，以及安全排序/季度移动；精确目标日期拖拽仍待。内容日历已交付 schema v37 数据/API、六周月格、新建/编辑、IANA/DST 安全改期、人工发布确认、Task 关联及 schema v38 Inbox 来源契约，并完成审核/发布到期的启动/周期扫描、版本化去重、变更终结、删除协调、Inbox 来源详情与六周范围自动分页聚合。目标版本为 v0.3。
...
| 前端测试                           | 通过         | 全量 89 个文件 / 584 项通过；新增路线图 Inbox 来源严格解析与目标日期/季度/计划版本快照展示，既有路线图年度跨季度移动、同季度原子排序/容量保护、筛选分页/编辑删除/按 ID 详情、内容日历、启动进度、Agent Adapter、Automation、Reminder、内部回滚容量、业务与连接世代回归继续通过。组件测试不替代真实 WebView 验收 |
...
9. **扩展本地规划与预设自动化**：预设自动化首个纵切已交付 Project 完成 Inbox 与 daily/weekly Reminder；路线图已完成数据/API、R3 基础界面、R4 同季度/跨季度排期和 R5 本地到期/达成 Inbox 来源，内容日历已完成 CC1–CC5-B。下一步完成路线图精确目标日期拖拽、内容日历大数据量与真实 WebView 专项，并在发票/Agent 事实上线后分别启用对应预设；Task 动作和自由规则仍待后续，任何规则都不得自动对外发送。
...
| 13   | 路线图（v0.3）            | **R2/R5 已完成**：roadmap_milestones、季度/日期/状态/项目关联，进度从项目/任务派生；schema v39 增加本地到期/达成 Inbox 来源、扫描补偿、稳定去重、变更终结和删除协调                                                                                                                                                                                                                                   | **R3 已完成；R4 部分完成**：季度/年度、状态/Project 筛选、稳定分页、新增/编辑/按 ID 详情、项目关联/跳转、归档恢复和二次确认删除已交付；完整同季度集合可原子排序，年度 Q1–Q4 时间线支持拖拽或上一/下一季度按钮移动，目标日期明确设为季度末；Inbox 可查看里程碑快照并返回路线图；精确目标日期拖拽待补                     | API 已覆盖季度边界、跨季度尾部排序、进度口径、项目删除约束、并发冲突、本地日期到期、达成转换、重扫去重和删除协调；Web 已覆盖筛选分页、详情最新事实、编辑载荷、版本头、保护性删除、同季度重排、跨季度移动及 Inbox 来源解析。真实浏览器拖拽/焦点、窄屏、超过 100 条提示及长列表仍待专项验收                                            |
