...
> **状态**：部分完成，不属于 v0.1。schema v35 已提供本地回访计划和终态的安全数据契约；回访 API、提醒、结果交互、客户时间线、Today 与 Inbox 投影仍在后续 C3–C6 实现。
...
| 客户管理         | 客户列表表格、客户详情、基本 CRUD                                                              | **部分完成**：基础资料 CRUD、分页/搜索/状态筛选/排序、创建幂等、并发控制、基础详情、受约束删除、Project 关联、本地活动、受控附件和 person 显式关联已实现；schema v35 已交付客户回访计划/终态的本地数据契约，CRUD、提醒和界面仍待后续；外部来源投影和财务待实现                                                                                                                                                                                                                                                           |
...
- AI 助手与模型运行时接入
- 本地知识库、文档索引与带来源检索
- 客户回访的 CRUD、执行闭环和提醒（schema v35 数据契约已完成）
