...
> **v7.3 更新说明**：将 Project 完成节点接入 T-11E。schema v28 新增 `project_completion` Inbox 来源；Project `complete` 与 `project_completed` Workflow Event、P1 收尾事项及 system `source_projected` Event 同事务提交。稳定键 `project:<project_id>:completed:<completion_version>` 区分 reopen 后的再次完成周期，payload 只保存项目 ID/名称、完成时间、完成后 version 和当时未结任务数，不自动建 Task、不伪造验收/开票/收入。活动来源阻止 Project 永久删除；全部 resolve/dismiss 后删除事务原子标记来源已删除并保留快照。前端展示完成上下文并直达 Project。
...
| 项目管理         | 项目卡片、状态流转、项目进度、项目详情（任务列表）                                         | **部分完成**：CRUD、分页/搜索/状态筛选、创建幂等、乐观锁、受控状态、归档恢复、确认硬删除、卡片/详情、任务派生进度/工时、客户选择/筛选、可编辑人工笔记、受控附件、产出聚合、活动时间线、显式 follow-up、Task 阻塞/临期与 Project 完成节点→Inbox 已实现；任务树和高级分析待实现                              |
...
| T-08 项目管理                        | 部分完成                         | CRUD、分页/搜索/筛选、创建幂等、乐观锁、受控状态、归档恢复、确认硬删除、任务聚合、客户选择/筛选、笔记、项目附件、Task Artifact 聚合、活动时间线、显式 follow-up、Task 阻塞/临期与 Project 完成节点→Inbox；任务树和高级分析待实现                                                                                                                                         |
...
- **需求映射**：5.3。
- **当前状态**：部分完成；项目资料、基础生命周期、任务聚合、Client 客户关联、可编辑人工笔记、受控项目附件、只读 Task Artifact 产出聚合、追加式项目活动时间线、显式 follow-up、Task 阻塞/临期和 Project 完成节点→Inbox 已接通；任务树和高级分析仍未完成。
...
| 3    | 项目                      | CRUD、schema v4 快照式创建幂等、schema v5 聚合版本、乐观锁、归档关联约束、状态流转、归档恢复、确认硬删除、任务派生进度/工时、Client 选择/筛选、schema v21 Project Note、schema v22 Project Attachment、Project Workflow Event、只读 Task Artifact 聚合、schema v23 follow-up 及 schema v28 Project 完成→Inbox 已交付         | 卡片、分页/搜索/状态/客户筛选、新建/编辑、Client 选择、详情任务列表/工时、人工笔记、受控附件、产出聚合、归档恢复和追加式活动时间线已交付；任务树待实现                   | 已覆盖事件同事务/不可变、笔记与附件幂等/版本/软删除/归档只读、附件完整性/备份/父删除、产出范围/分页/删除历史、完成周期来源上下文/删除协调及独立 UI 状态；后续验收项目产出→拆分→分派→验收                                    |
