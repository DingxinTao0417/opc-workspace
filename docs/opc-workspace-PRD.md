...
> 文档日期：2026-08-28
> 实现基线：app v0.1.0 / API v1 / SQLite schema v21

> **v6.0 更新说明**：交付 Project 产出聚合纵切。新增只读 `GET /projects/:id/artifacts`，在同一只读事务中按 Project 聚合现有 Task Artifact，稳定分页并可显式查看删除历史，返回所属 Task 标题/当前状态、Submission 序号和 Project 聚合版本；不复制产出正文或验收状态。项目详情新增真实产出区，可查看来源任务、提交批次、类型、跟进/删除状态并直达共享任务详情完成查看、下载和验收。项目附件与 Inbox 来源投影仍待开发。
...
> **当前状态**：部分完成。当前 schema v21 保留并验证了由 schema v3–v5 交付的 Project model、CRUD、分页/搜索/筛选、快照式创建幂等、覆盖聚合事实的 `If-Match` 乐观锁、受控状态流转、归档恢复、确认硬删除，以及真实卡片/详情/任务聚合。schema v21 新增独立、版本化 Project Note，支持幂等创建、稳定分页、乐观编辑、带原因软删除、删除历史和归档只读，并原子传播 Project 聚合版本。项目详情现已只读聚合所属 Task Artifact 和来源 Task/Submission 上下文，不复制产出或验收事实。Client CRUD、客户选择/改绑/解除和客户筛选已接通。Focus 完成会更新绑定 Task 工时并沿既有聚合链刷新 Project。Project 写命令继续同事务追加不可变 Workflow Event；项目笔记不混入命令审计。项目附件和 Inbox 来源投影仍未实现。Task 六状态上线后 Project 仍只把 done 计为完成，cancelled 留在总数/剩余口径。
...
当前工时聚合任务表 `actual_minutes`；完成的绑定 Focus Session 已在同一事务中按精确秒数 ledger 折算新增完整分钟写回 Task，并通过既有 trigger 递增 Project 聚合版本。项目新建/编辑可从真实 Client API 选择、改绑或解除客户，项目列表支持客户筛选；默认项目列表排除归档项，Client 详情为完整关联历史显式包含归档项目。项目详情已接系统写命令活动时间线、可编辑人工笔记和所属任务的产出聚合；项目附件、发票明细、收入/成本和 Focus 高级分析仍未接入。
...
> **状态**：未开始；不属于 v0.1，目标版本和本地模型/运行时待单独评审。当前仓库没有模型 SDK、密钥、AI API、会话表或助手页面；PRD v6.0 当前阶段不接入线上模型服务。
...
首次启动不下载业务运行时或后端镜像。PRD v6.0 当前阶段不提供线上更新或第三方连接；安装、升级和核心使用均可在离线环境完成。
...
| 项目管理         | 项目卡片、状态流转、项目进度、项目详情（任务列表）                                         | **部分完成**：CRUD、分页/搜索/状态筛选、创建幂等、乐观锁、受控状态、归档恢复、确认硬删除、卡片/详情、任务派生进度/工时、客户选择/筛选、可编辑人工笔记和追加式活动时间线已实现；产出/附件与 Inbox 来源投影待实现                                                                                                                    |
...
| 数据持久化       | Tauri `appDataDir`、SQLite 迁移、受控文件、手动/迁移前一致性备份、基础 JSON 导出与原子恢复 | **部分完成**：正式/开发隔离、WAL、外键、schema v21、重建迁移安全、设置/保存视图/客户活动/附件/person 关联/项目笔记事实、数据库身份强绑定和共享受控文件 store 已实现；手动 SQLite+受控文件备份完整闭环、桌面一键安全重启及基础业务 JSON 导出已交付；迁移前备份、恢复诊断、数据导入和含文件导出包仍待实现；每日计划和高级导入归 v0.3 |
...
| API / Schema | API v1；SQLite schema v21                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 数据默认值   | 开发数据库默认空白，不自动注入 demo 业务数据                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 明确边界     | 当前代码不使用 Docker；已实现 Task 事实层、Actor/Assignment、D1 生命周期、Task 时间线、D2 manual Submission/Artifact/受控文件、Project 可编辑人工笔记/追加式活动时间线、Client 基础资料/Project 客户关联/人工活动时间线/受控附件/person 显式关联、Focus Core A+B+C、手工 Inbox 受理/分诊、Reminder、Inbox Task 批量拆分/分派/自动结清、基础备份闭环及业务 JSON 导出；未实现 Client 外部活动来源、Focus D、Project 产出/附件、非 Reminder 来源投影、重复/原生通知、Agent 执行、数据导入/含文件导出、AI 助手、知识库、客户回访或收入/支出/发票业务；person 只做本地责任记录，线上账号、云同步和远程协作均不在当前范围 |
...
迁移文件位于 `services/sidecar/internal/database/migrations/`，通过 Go `embed` 编入 Sidecar。当前 schema 为 v21，新增结构只能从 `022_*` 继续追加递增文件：
...
| T-04 SQLite 初始化与迁移             | 已完成           | schema v21、PRAGMA、嵌入迁移、demo 清理、Project/Project Note/Task/Actor、六状态、workspace identity、Submission/Artifact、Client/Activity/Attachment/Actor Link、Focus Session/interval/精确 Task ledger、Inbox、Reminder、app_settings 与 task_saved_views                                          |
...
| T-08 项目管理                        | 部分完成         | CRUD、分页/搜索/筛选、创建幂等、乐观锁、受控状态、归档恢复、确认硬删除、卡片/详情、任务聚合、客户选择/筛选、可编辑人工笔记、Task Artifact 产出聚合和追加式项目活动时间线；项目附件与 Inbox 来源投影待实现                                                                                             |
...
- **实现方法**：GORM 使用纯 Go SQLite 驱动和单物理连接，启用外键、WAL 与 5 秒 busy timeout。schema v1–v14 依次建立核心、Project、Task、Actor/Assignment/Event、Submission/Artifact、Client、Focus、Inbox、Inbox–Task 关系与 Reminder；schema v15–v17 增加 Inbox 编排、版本化设置和任务保存视图；schema v18 新增客户活动；schema v19 新增受控客户附件；schema v20 新增 Client–person contact；schema v21 新增 Project Note、不可变身份/删除历史和 Project 版本传播，均不改写既有模块事实或创建 demo 数据。不可变 `database_id`、一次性 `artifact_store_id` 与 marker 的 database/store ID 双向绑定，阻止错库接管或数据库静默换文件根。标有 foreign-keys-off 的迁移在同连接事务外关外键、事务内校验 `foreign_key_check`，再提交或整体回滚并恢复。开发数据库/受控文件位于 `.local/dev-data/`，生产数据位于 Tauri `appDataDir`。
...
- **需求映射**：5.3。
- **当前状态**：部分完成；项目资料、基础生命周期、任务聚合、Client 客户关联、可编辑人工笔记、只读 Task Artifact 产出聚合和追加式项目活动时间线已接通，项目附件与 Inbox 来源投影仍未完成。
...
- **建议开发流程**：
  1. 先完成本地运行时、资源预算、上下文权限和质量评测 ADR；PRD v6.0 当前阶段不评审远程 Provider。
...
2. **收口项目基础纵切**：CRUD、schema v4 幂等快照、schema v5 聚合版本、乐观锁、归档关联约束、状态流转、归档恢复、确认硬删除、任务聚合、Client 选择/筛选、schema v21 可编辑人工笔记、只读 Task Artifact 产出聚合和追加式活动时间线已交付；继续验证大数据量串行分页、真实浏览器/窄屏/焦点，并保留项目附件/Inbox 来源投影为明确缺口。
...
| 3    | 项目                      | CRUD、schema v4 快照式创建幂等、schema v5 聚合版本、乐观锁、归档关联约束、状态流转、归档恢复、确认硬删除、任务派生进度/工时、Client 选择/筛选、schema v21 Project Note、Project Workflow Event 和只读 Task Artifact 聚合已交付；附件/Inbox 来源投影待开发 | 卡片、分页/搜索/状态/客户筛选、新建/编辑、Client 选择、详情任务列表/工时、人工笔记、产出聚合、归档恢复和追加式活动时间线已交付；任务树与项目附件待实现                   | 已覆盖事件同事务/不可变、笔记幂等/版本/软删除/归档只读、产出范围/分页/删除历史/来源上下文、稳定分页/快照/删除审计及独立 UI 状态；后续验收项目产出→拆分→分派→验收和来源事件去重              |
