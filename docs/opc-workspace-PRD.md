...
> **v9.6 更新说明**：交付全局 Sidecar 启动故障恢复页 v1。React 根节点在设置 bootstrap 与业务路由之前读取白名单 `sidecar_status`：浏览器开发模式直接放行；桌面 `starting` 显示安全加载页，`ready` 放行业务并每 3 秒继续观察，`error` 或状态读取失败则拦截全部业务页。恢复页支持立即重查、无路径打开脱敏日志目录和调用既有受管 Sidecar 安全重启；只显示 app/API/schema 白名单版本，不展示 Tauri 原始 message、路径、Token 或业务数据。当前不在原进程中直接拉起第二个 Sidecar，也不提供数据库打开前备份选择/实时恢复进度。API v1/schema v29 不变。
...
- **需求映射**：5.6。
- **当前状态**：部分完成。schema v12–v15 已交付手工受理分诊、已有 Task 关系、一次性 Reminder 和 T-11C 拆分编排；T-11F 已交付实时运营计数、Today/Sidebar 展示和风险深链筛选；schema v23–v25 已交付显式 follow-up Task Artifact、Task 阻塞与提前 24 小时 Task 临期来源投影、稳定事件键、来源上下文与删除协调；系统维护已交付备份四类操作失败、数据库启动/迁移、Sidecar 启动、运行期数据库操作失败和固定低空间投影。阈值配置、重复/原生通知和 Agent Run 仍未实现。
