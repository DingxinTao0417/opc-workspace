...
> **v9.78 更新说明**：新增严格白名单 `desktop_capabilities` Tauri command；设置“运行诊断”现可显示托盘运行时可用/不可用，并在复制摘要中只加入枚举状态。通知、自启、原生文件对话框和离线更新明确返回 `not_implemented`，浏览器模式不伪造桌面能力；任一能力读取失败不阻断 Sidecar 恢复诊断。schema/API 不变。
...
| 前端测试                                           | 通过         | 全量 90 个文件 / 603 项通过；新增桌面能力严格解析、畸形能力降级、设置托盘状态展示与共享恢复 fixture 回归，既有内容日历/路线图 URL、目标日期排序、客户动态、启动进度、Agent Adapter、Automation、Reminder、内部回滚容量、业务与连接世代回归继续通过。组件测试不替代真实 WebView 验收          |
...
| 统一源码质量门禁                                   | 通过         | 本轮完成源码门禁：Prettier、193 个 Go 文件与 Rust 格式、26 份 Markdown/本地链接、Web 类型/90 文件 603 项测试/构建、Go 无缓存测试/vet 与 x86_64-pc-windows-msvc Sidecar 构建                                                                                                                  |
...
| 10   | 桌面与发布                | **部分完成**：generation-aware 两次有界重启、数据库运行锁、父管道 EOF、并发 shutdown、日志/request ID、全局恢复页、安全应用重启、固定命令面板/新建任务的原生快捷键注册/状态查询、托盘显示/隐藏/显式退出和不可用安全降级源码，以及白名单 `desktop_capabilities` 已交付；托盘原生链接/三平台交互、真实父崩溃/进程树、孤儿/孙进程治理、通知/自启/快捷键自定义/签名更新及三平台 CI 待实现                 | **部分完成**：starting/restarting/ready/error + generation、非 ready 连接/查询清理、状态重查、日志入口、安全重启、版本白名单、fixed shortcut 状态和托盘运行时能力诊断已交付；基础托盘交互不经 WebView，设置开关、专注状态/业务动作、启动前备份选择/恢复反馈与离线更新反馈待实现                                               | 本轮源码门禁已通过 Web 90 文件/603 测试、Go 无缓存全量测试/vet、文档、Sidecar 构建、Rust 格式与锁定 Cargo metadata。cargo check/test 受缺失 MSVC link.exe/Windows SDK 阻塞，托盘原生交互、真实父崩溃、进程树、签名、公证、干净机和三平台仍须验收                                                                                                                                     |
