...
> **v9.38 更新说明**：客户详情的回访时间线新增 active owner/person 负责人筛选，可与全部、待回访、已完成、已跳过和已取消状态组合；筛选直接复用 `GET /api/v1/clients/:id/followups` 的 `status` / `assigned_actor_id` 与严格分页，切换时回到第一页。无新 migration、无外发消息、无 AI/LLM。app v0.1.0 / API v1 / schema v35 不变。
