...
| T-19 本地 Agent 执行                 | v0.2-B 首片完成                  | ADR-003 冻结边界；ADR-027 分层 builtin/external 信任并交付 v0.2-B 首片：schema 071 agent_runs、Run 状态机（含 interrupted 恢复）、单次管道协议 `opc-agent-pipe-v1`、builtin 本地模型文本执行器（回环限定）、启用幂等创建 agent Actor、Assignment 门控、创建/列表/详情/取消/重试 API 与事件。Windows 生命周期矩阵（管道往返、Job Object 子树回收、超时/取消）已实测；产出暂只入 Run 记录，Artifact/验收接入（v0.2-C）、Web UI 与 macOS/Linux 矩阵待后续，external 执行保持关闭                                                                                     |
...
- **当前状态**：v0.2-A Adapter 登记与安全诊断、v0.2-B Runner 首片均已完成。[ADR-027](adr/027-builtin-agent-executor-and-run-lifecycle.md) 把信任分为 builtin（代码所有，Windows 生命周期矩阵实测后可 execution_ready）与 external（保持 ADR-003 全闸门，验证前永不启用）两级。schema 071 新增 `agent_runs` 与 `actors.agent_adapter_id`；Sidecar 已交付 Run 状态机、`opc-agent-pipe-v1` 管道协议、自再执行内置文本执行器（回环模型端点）、启用幂等创建 agent Actor、Assignment 门控、创建/列表/详情/取消/重试 API 与追加事件。Windows 上管道往返、Job Object 子树回收（含孙进程）、超时/取消已实测。Run 结果暂存于 Run 记录（≤64 KiB 文本）；Artifact/验收接入、Web UI、macOS/Linux 矩阵仍待，external 执行保持关闭。
