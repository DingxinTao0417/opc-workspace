...
> **v9.17 更新说明**：交付共享的服务端分页搜索 `ProjectSelect`。任务新建、任务详情编辑、Tasks 项目筛选、批量移动项目及 Inbox 批量拆分任务五个入口统一每次读取 20 条，输入经 250 ms 防抖后复用既有 `GET /api/v1/projects?q=...&sort=name`，支持请求取消、按 ID 去重、稳定上一页/下一页、跨页或读取失败时保留当前选择、显式清除，以及归档当前选择的只读保留；已移除串行拉取全部 Project 的 `getAllProjects`。Project 列表的默认及全部白名单排序追加 `projects.id ASC`，Count 与当前页 Scan 在同一 SQLite 只读事务中执行，保证选择器分页具有确定性读快照。组件测试覆盖加载、等待、空、错误重试、更多结果、状态/客户上下文和 combobox/listbox 键盘语义；真实浏览器、窄屏及 1,000/10,000 条 Project 性能专项仍未验收。app v0.1.0 / API v1 / SQLite schema v31 不变，无迁移；AI、知识库、客户回访和财务仍保持后续版本边界。
...
当前卡片工时聚合任务表 `actual_minutes`；完成的绑定 Focus Session 已在同一事务中按精确秒数 ledger 折算新增完整分钟写回 Task，并通过既有 trigger 递增 Project 聚合版本。项目新建/编辑和项目列表客户筛选共用真实 `ClientSelect`：每次从既有 Client API 读取 20 条，以 250 ms 防抖做服务端搜索并稳定翻页，不再串行拉取全部客户；跨页、inactive 或读取失败时保留当前选择，只有用户显式点击“清除客户”才提交 `null` 并解除关联。任务新建/编辑、Tasks 项目筛选、批量移动项目和 Inbox 批量拆分共用真实 `ProjectSelect`：固定每页 20 条并按名称服务端搜索，当前选择通过详情、当前页或名称快照保留；默认候选排除归档项目，但既有归档选择仍可辨认且不会因失败被改写。默认项目列表排除归档项，Client 详情为完整关联历史显式包含归档项目。项目详情已接系统写命令活动时间线、可编辑人工笔记、受控项目附件、所属任务产出聚合，以及独立加载的 7 天/30 天/本月 Focus 趋势与终态历史。Focus 报告只计算 completed Session 的闭合正时长 interval，终态历史另包含 cancelled/interrupted 审计记录；两者按 Task 查询时当前 Project 归属过滤，不复制历史项目快照。归档 Project 可读；Task 改绑会重分类，无 Task/Task 已删除/当前无项目不进入项目过滤结果。发票明细和收入/成本仍未接入。
...
| T-08 项目管理                        | 部分完成                         | CRUD、稳定分页/搜索/筛选、同事务 Count/Scan、创建幂等、乐观锁、受控状态、归档恢复、确认硬删除、任务聚合与树/平铺/项目内检索分页、服务端分页搜索客户与项目选择器、笔记、项目附件、Task Artifact 聚合、活动时间线、项目级 Focus 分析/终态历史、显式 follow-up、Task 阻塞/临期与 Project 完成节点→Inbox；财务与真实里程碑增强待实现                                                                                                                                                                                                                |
...
- **关键路径**：`services/sidecar/internal/database/migrations/003_project_lifecycle.sql`、`migrations/004_idempotency_snapshots.sql`、`migrations/005_project_aggregate_versions.sql`、`migrations/011_focus_sessions.sql`、`migrations/028_project_completion_inbox_projection.sql`、`migrations/031_client_project_activity_projection.sql`、`internal/models/project.go`、`internal/api/projects.go`、`internal/api/projects_test.go`、`internal/api/focus_history.go`、`internal/api/inbox_source_projections.go`、`internal/api/project_client_activity_projection_test.go`、`apps/web/src/api/client.ts`、`api/hooks.ts`、`components/ClientSelect.tsx`、`components/ProjectSelect.tsx`、`components/ProjectFormModal.tsx`、`components/NewTaskModal.tsx`、`components/TaskDetailModal.tsx`、`components/InboxTaskOrchestrationModal.tsx`、`pages/ProjectsPage.tsx`、`pages/TasksPage.tsx`、`api/projectHooks.test.tsx`、`components/ClientActivitiesSection.tsx`。
- **验证/剩余**：代码审查确认 Project Count/Scan 共用请求上下文内的 SQLite 只读事务；Go 自动化覆盖所有排序字段的 `id ASC` 后备键、同名项目 `page_size=1` 跨页无重无漏与重复请求一致、搜索/状态/客户/归档过滤，以及事件 producer/分页/快照/Actor/幂等不重复/事务回滚、删除审计、聚合版本、客户关联、Project complete/reopen 客户动态完整行契约、无客户、陈旧版本不重复、Client version/latest activity、改绑不搬移、下游失败全回滚，Project Note/Attachment/Artifact/Inbox、Focus→Task→Project 版本传播及项目 Focus canonical UUID/400/404/归档空态/当前归属重分类/稳定分页/completed-only IANA-DST 报告；前端覆盖两类共享选择器首屏、搜索、分页、跨页保留、状态/客户上下文、归档当前项、反馈、键盘语义及五处 ProjectSelect 接线，以及客户动态可读文案/隐藏内部 ID、只读操作和 Project/Client 缓存刷新。真实浏览器窄屏浮层、完整键盘焦点、1,000/10,000 条客户/项目及 Focus 大账本性能仍需专项验收。内嵌 Assignment/Submission 写控件仍未实现。
...
| `pnpm --filter @opc/web typecheck` | 通过         | 本次覆盖 Project 单页查询、AbortSignal、共享 ProjectSelect 及任务五处接线；既有 Task D2、Client/Project/Inbox 契约继续通过                                                                                                                                                   |
| 前端测试                           | 通过         | 本模块定向 7 个文件 / 96 项及全量 77 个文件 / 494 项通过，覆盖 ProjectSelect 首屏/搜索/分页/跨页保留、项目状态/客户上下文、归档当前项、反馈与键盘语义、单页 Query/实际取消，以及任务五处接线；既有父任务、任务、客户、项目、设置等回归继续通过。组件测试不替代浏览器视觉验收 |
...
| 文档链接/过期措辞/diff             | 通过         | 本次 v9.17 文档更新后运行相对 Markdown 链接、绝对机器路径、过期状态和 `git diff --check` 审计                                                                                                                                                                                |
| Rust / Tauri 链接                  | 环境受限     | 当前主机缺少 MSVC `link.exe`（Visual C++ Build Tools / Windows SDK）；无法把 `cargo check`/Rust tests 或完整桌面链接声明为通过                                                                                                                                               |
| 浏览器渲染 / 窄屏视觉              | 本次未验收   | ClientSelect/ProjectSelect 已由组件测试覆盖，尚无本轮真实浏览器键盘、焦点、Portal 窄屏浮层及 1,000/10,000 条客户/项目性能证据                                                                                                                                                |
...
2. **收口项目基础纵切**：CRUD、schema v4 幂等快照、schema v5 聚合版本、乐观锁、归档关联约束、状态流转、归档恢复、确认硬删除、任务聚合、共享服务端分页搜索 Client 与 Project 选择/筛选、Project 列表确定性排序与同事务 Count/Scan、schema v21 项目笔记、schema v22 项目附件、只读 Task Artifact 聚合、活动时间线、项目级 Focus 报告/终态历史、schema v23–v25 Task 相关来源、schema v28 Project 完成节点→Inbox 和 schema v31 complete/reopen→Client Activity 已交付；下一步验证两类选择器的真实浏览器/窄屏/焦点与 1,000/10,000 条数据，并继续项目产出→拆分→分派→验收的端到端体验收口。
...
| 3    | 项目                      | CRUD、schema v4 快照式创建幂等、schema v5 聚合版本、乐观锁、归档关联约束、状态流转、归档恢复、确认硬删除、任务派生进度/工时、Client 选择/筛选、稳定 ID 后备排序与同事务 Count/Scan、schema v21 Project Note、schema v22 Project Attachment、Project Workflow Event、只读 Task Artifact 聚合、项目级 Focus 读模型、schema v23 follow-up、schema v28 Project 完成→Inbox 及 schema v31 complete/reopen→Client Activity 已交付 | 卡片、分页/搜索/状态/客户筛选、新建/编辑、共享 ClientSelect 与 ProjectSelect 服务端搜索分页、详情任务树/平铺、项目内组合筛选与分页、工时、人工笔记、受控附件、产出聚合、7 天/30 天/本月 Focus 报告与终态历史、归档恢复和活动时间线已交付      | 已覆盖两类选择器搜索/分页/跨页保留/反馈/键盘接线、同名 Project 确定性分页、项目 Focus canonical UUID/400/404、归档/空态、当前 Task 归属重分类、completed-only DST 报告、终态分页与独立反馈，以及事件/笔记/附件/产出/完成来源/客户动态原子投影/任务浏览器；后续验收真实浏览器、大数据量、项目产出→拆分→分派→验收                      |
