...
> **v9.67 更新说明**：路线图 R4 已补齐季度内精确目标日期调整；在完整未归档季度集合不超过 100 条且无 Project 筛选时，页面展示三个月星期对齐日历，卡片可拖到具体日期，也可用卡片内日期输入作为键盘替代。每次 PATCH 携带当前版本，失败保留原日期并回读服务端；schema 仍为 v39，无 AI/LLM、外发或 demo 数据。
...
| 前端测试                           | 通过         | 全量 89 个文件 / 586 项通过；新增路线图精确日期拖拽、键盘日期输入及容量门禁回归，既有路线图 Inbox 来源严格解析、年度跨季度移动、同季度原子排序/容量保护、筛选分页/编辑删除/按 ID 详情、内容日历、启动进度、Agent Adapter、Automation、Reminder、内部回滚容量、业务与连接世代回归继续通过。组件测试不替代真实 WebView 验收 |
...
9. **扩展本地规划与预设自动化**：预设自动化首个纵切已交付 Project 完成 Inbox 与 daily/weekly Reminder；路线图已完成数据/API、R3 基础界面、R4 同季度/跨季度/精确日期排期和 R5 本地到期/达成 Inbox 来源，内容日历已完成 CC1–CC5-B。下一步完成内容日历大数据量与真实 WebView 专项，并在发票/Agent 事实上线后分别启用对应预设；Task 动作和自由规则仍待后续，任何规则都不得自动对外发送。
