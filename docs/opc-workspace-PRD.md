...
> **v9.64 更新说明**：路线图跨季度移动的服务端顺序语义已补齐：PATCH 改变 `year`/`quarter` 时，会在同一事务把里程碑追加到目标季度尾部并递增版本，不再沿用源季度 `manual_order` 造成碰撞；API 回归覆盖目标/源季度结果。多季度拖拽界面仍待，schema 仍为 v38，无 AI/LLM、外发或 demo 数据。
...
> **当前状态**：路线图 `/roadmap` 的 R3 基础界面已完成，R4 已交付完整同季度集合的安全卡片排序，并补齐 PATCH 跨季度时追加到目标季度尾部的服务端事实规则；R2 的 schema v36/API 提供 CRUD、筛选、项目关联、只读派生进度、乐观锁、归档恢复与重排。当前界面已接季度/状态/Project 浏览、稳定分页、新建、基础事实编辑、按 ID 详情、Project 深链、归档恢复、保护性永久删除，以及 2–100 条完整未归档季度集合的拖拽/键盘顺序预览与原子保存；目标日期/跨季度拖拽和 Inbox/Reminder 自动化仍待。内容日历已交付 schema v37 数据/API、六周月格、新建/编辑、IANA/DST 安全改期、人工发布确认、Task 关联及 schema v38 Inbox 来源契约，并完成审核/发布到期的启动/周期扫描、版本化去重、变更终结、删除协调、Inbox 来源详情与六周范围自动分页聚合。目标版本为 v0.3。
...
| 前端测试                           | 通过         | 全量 89 个文件 / 580 项通过；新增路线图原子重排序列化、完整季度读取、键盘替代、拖拽保存和超过 100 条安全禁用覆盖，既有路线图筛选分页/编辑删除/按 ID 详情、内容日历、启动进度、Agent Adapter、Automation、Reminder、内部回滚容量、业务与连接世代回归继续通过。组件测试不替代真实 WebView 验收 |
| Web 生产构建                       | 通过         | `pnpm build:web` 的 TypeScript 与 Vite 构建通过；当前主入口 minified 916.00 kB、gzip 244.61 kB，仍有非阻断的 500 kB chunk 警告，后续按页面拆包                                                                                                                                               |
| 前端格式                           | 通过         | Web 范围 Prettier format check 通过                                                                                                                                                                                                                                                          |
| Go 全量测试与静态检查              | 通过         | `go test ./... -count=1` 与 `go vet ./...` 均通过；不虚构未统计的用例总数                                                                                                                                                                                                                    |
| 统一源码质量门禁                   | 通过         | 本轮完成源码门禁：Prettier、188 个 Go 文件与 Rust 格式、26 份 Markdown/本地链接、Web 类型/89 文件 576 项测试/构建、Go 无缓存测试/vet 与 x86_64-pc-windows-msvc Sidecar 构建                                                                                                                  |
...
| 13   | 路线图（v0.3）            | **R2 已完成**：roadmap_milestones、季度/日期/状态/项目关联，进度从项目/任务派生，编辑/归档/恢复/删除使用乐观版本                                                                                                                                                                                                                                                                                      | **R3 已完成；R4 部分完成**：季度/状态/Project 筛选、稳定分页、新增/编辑/按 ID 详情、日期与状态调整、项目关联/跳转、归档恢复和二次确认删除已交付；完整同季度集合支持拖拽或上移/下移预览并携带全量版本原子保存，超过 100 条或存在细分筛选时安全禁用；目标日期/跨季度拖拽待补                                              | API 已覆盖季度边界、进度口径、项目删除约束和并发冲突；Web 已覆盖筛选分页、详情最新事实、编辑载荷、版本头、保护性删除和同季度重排。真实浏览器拖拽/焦点、窄屏、超过 100 条提示及长列表仍待专项验收                                                                                                                                     |
