...
> **v3.4 更新说明**：交付 T-06C Today 按钮式持久排序基线：所选精确日期与未排期活动 Task 支持上移/下移和恢复默认；写入始终携带完整计划组及版本，活动 Task 可跨活动状态移动且保留 done/cancelled 槽位，集合变化、版本冲突、超限或网络失败不提前改变可见顺序并显示错误。拖拽、跨日期改期和行内编辑/删除/专注快捷操作仍未交付。
...
> **当前状态**：部分完成。真实任务、持久化 Focus Session 和 Today Focus 汇总已接入，右栏读取真实活动 Session；任务已按所选本地日期完整分为逾期、当天、本周稍后和未排期，并支持前后切换及回到今天。所选精确日期与未排期组已支持按钮式持久排序/恢复默认；拖拽、跨日期改期、临期筛选、真实收入和客户动态尚未实现。
...
2. 目标交互为拖拽调整并立即持久化，且提供“恢复默认排序”；当前任务页及 Today 所选精确日期/未排期活动组已接入上移/下移与恢复默认，拖拽和跨日期改期仍待实现
...
> **状态**：未开始；不属于 v0.1，目标版本和本地模型/运行时待单独评审。当前仓库没有模型 SDK、密钥、AI API、会话表或助手页面；PRD v3.4 当前阶段不接入线上模型服务。
...
首次启动不下载业务运行时或后端镜像。PRD v3.4 当前阶段不提供线上更新或第三方连接；安装、升级和核心使用均可在离线环境完成。
...
| ---------------- | ----------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 今日工作台       | 三栏布局、今日任务列表、手动排序、逾期及临期提示、任务详情操作、右侧概览面板                    | **部分完成**：布局、日期切换/回到今天、真实任务/Today 统计、四组完整任务、精确日期/未排期按钮式持久排序、共享任务详情、真实活动 Focus 概览与 completed-only Focus 汇总已接通；拖拽/跨日期改期、收入与客户动态待实现                     |
| 任务管理         | 完整 CRUD、父子任务、状态流转、标签、项目关联、完成条件、人工验收、列表视图、搜索和快捷键       | **部分完成**：schema v6–v9 事实、责任、六状态、时间线与 manual Submission/Artifact 验收，快照幂等、`ETag`/`If-Match`、分页筛选、层级、批量/排序和受控文件均已实现；schema v11 Focus 工时已接入；Today 已消费按钮式排序，拖拽与看板仍待实现       |
...
- **建议开发流程**：
  1. 先完成本地运行时、资源预算、上下文权限和质量评测 ADR；PRD v3.4 当前阶段不评审远程 Provider。
...
| 6    | 今日                      | 按本地日期、逾期和本周范围查询；完整计划组排序事务；按 IANA 时区计算 UTC 边界；增加收件箱派生计数                                                                                                                      | 日期切换、真实分组、精确日期/未排期上移下移与恢复默认已交付；拖拽/跨日期改期、编辑/删除/专注快捷操作待开发；未上线财务卡标后续                                       | 已覆盖超过 100 项分页、排序集合/版本、活动状态跨状态移动和终态槽位保留；仍需真实浏览器验证拖拽、键盘、窄屏及午夜/夏令时边界                                                     |
