...
> **v7.6 更新说明**：设置新增只读“运行诊断”模块和命令面板直达入口，同时读取严格 `/health` 与 Tauri `sidecar_status`；桌面状态进入 UI 前只保留环境、生命周期及 app/API/schema 版本，主动丢弃会话令牌、监听地址和原始错误。页面显示浏览器开发/桌面差异、版本兼容、加载/错误/重试/最近成功状态，并可复制不含路径、令牌、地址、底层错误或业务正文的脱敏摘要。schema 保持 v28；完整日志落盘、轮转、诊断包、恢复页和 OS 全局快捷键仍未实现。
...
> **当前状态**：核心本地搜索、本地最近使用与设置运行诊断入口已完成。支持已交付页面/安全操作命令、真实 Task/Project/Client/活动 Inbox 统一搜索、精确可刷新详情路由和完整键盘交互；空查询可展示有上限的非敏感最近命令/资源，并在确认资源 404 后清理。完整日志/诊断包、恢复页和 OS 全局快捷键仍待开发。
...
| T-13 命令面板与基础反馈              | 核心搜索、最近使用与运行诊断完成 | WebView 快捷键、已交付页面命令、Task/Project/Client/活动 Inbox 统一本地搜索、稳定详情路由、设置模块直达、combobox/listbox、焦点圈闭/恢复、IME 保护、加载/错误/重试/空状态、有上限的非敏感最近使用/404 资源清理，以及脱敏运行诊断；完整日志/诊断包和 OS 全局快捷键待后续                                                                                                  |
...
- **验证/当前限制**：Go 契约测试覆盖四类型结果、归档/终态过滤、类型去重、稳定分页、文字 `%/_`、非法查询和只读空库；前端覆盖 strict 契约、防抖、错误重试、精确导航及 Task/Inbox 直达路由、最近使用的容量/去重/过期/非敏感存储/404 清理，以及 Sidecar 状态白名单规范化和诊断模块。当前不是 FTS/模糊搜索，没有高亮、桌面启动恢复页、完整日志/诊断包、全局错误边界或 OS 全局快捷键；真实浏览器和桌面 WebView 焦点/视觉仍需人工验收。
