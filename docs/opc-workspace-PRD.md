...
> **v9.57 更新说明**：内容日历 CC5-B1 已交付 schema v38 的版本化 Inbox 来源契约：审核/发布到期 payload 必须匹配当前内容状态、版本、排期和 IANA 时区，来源键唯一且删除须协调。迁移不创建投影；运行时扫描仍待。无 AI/LLM、外发或 demo 数据。
...
> **当前状态**：路线图 `/roadmap` 已完成基础真实本地界面，R2 的 schema v36/API 提供 CRUD、筛选、项目关联、只读派生进度、乐观锁、归档恢复与重排；当前界面已接季度/状态浏览、新建、Project 深链和归档恢复，编辑、删除、拖拽与 Inbox/Reminder 自动化仍待。内容日历已交付 schema v37 数据/API、六周月格、新建/编辑、IANA/DST 安全改期、人工发布确认、Task 关联及 schema v38 Inbox 来源契约；运行时 Inbox 扫描/终结仍待。目标版本为 v0.3。
