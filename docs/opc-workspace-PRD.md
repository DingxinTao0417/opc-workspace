...
> **v9.32 更新说明**：交付 T-17/C5 客户详情回访时间线读取纵切。客户详情通过版本化 API 严格校验并分页显示本地回访计划、终态结果、下一步、负责人、优先级和派生逾期状态，具备加载、空和错误重试状态；创建/编辑/执行页面入口、Today 展示与 Inbox 反向执行仍待。app v0.1.0 / API v1 / schema v35 不变。
...
> **当前状态**：部分完成。schema v10 的 Client 基础资料和 Project 客户关联、schema v18 的本地人工活动时间线、schema v19 的受控附件、schema v20 的 person 显式关联均已交付；schema v31 已交付 Project 完成/重开的本地系统事实投影。Project/Task 三个入口共用的服务端分页搜索 ClientSelect 已交付，真实浏览器/窄屏/大数据量专项仍待验收。客户回访已具备本地计划/终态 API、到期 Inbox 投影和客户详情只读时间线；执行表单、Today 展示、财务与外部来源集成仍待。
...
- 已实现回访只读时间线：客户详情使用 `GET /api/v1/clients/:id/followups` 分页读取并严格校验响应，显示计划/完成/跳过/取消、优先级、负责人、渠道、结果与下一步；`overdue` 只由 planned 的计划时间在页面派生。页面同时提供加载、空、失败重试和分页，不把尚未接入的创建/执行操作伪装为可用。
- 待实现：外部活动来源集成、关联发票详情和财务聚合；回访创建、编辑、完成、跳过、取消、重排的页面交互，Today 展示与 Inbox 反向处理入口；线上下载、邮件或其他客户沟通只有在未来明确集成并记录真实事实后才能出现。

#### 客户回访（后续版本）

> **状态**：部分完成，不属于 v0.1。schema v35 已提供本地回访计划和终态的安全数据契约，C3/C4 已提供计划与执行 API，C5 已提供到期 Inbox 投影及客户详情只读时间线；创建/执行页面交互、Today 展示和 Inbox 反向入口仍在后续纵切。
...
| 客户管理         | 客户列表表格、客户详情、基本 CRUD                                                              | **部分完成**：基础资料 CRUD、分页/搜索/状态筛选/排序、创建幂等、并发控制、基础详情、受约束删除、Project 关联、本地活动、受控附件和 person 显式关联已实现；schema v35 回访计划/终态、C3/C4 API、C5 到期 Inbox 投影及客户详情只读时间线已交付，创建/执行页、Today 和 Inbox 反向入口仍待；外部来源投影和财务待实现                                                                                                                                    |
...
- AI 助手与模型运行时接入
- 本地知识库、文档索引与带来源检索
- 客户回访创建/执行页面、Today 展示及 Inbox 反向处理入口（本地 CRUD/终态 API、到期 Inbox 投影和客户详情只读时间线已完成）
...
> 状态截止：2026-08-29。当前版本是可运行、可扩展的 v0.1 基座，并已前置交付 v0.2 首个受限预设自动化纵切。schema v35、Project 完成→本地核对 Inbox、daily/weekly 本地 Reminder、设置预览/保存/启停、Run 历史/重试、IANA/DST、离线折叠和导入导出、Adapter 登记/安全诊断，以及客户回访的计划/终态数据契约已经接通；这不代表完整 v0.2、Agent Runtime、发票/客户回访 CRUD 和提醒、自由规则、Task 自动化动作、Focus 原生反馈、复杂通知、数据库打开前恢复进度或冲突合并导入已经交付。
...
| 前端测试                           | 通过         | 全量 83 个文件 / 539 项通过；新增原生快捷键 action、注册状态白名单与运行诊断降级覆盖，既有启动进度、Agent Adapter、Automation、Reminder、内部回滚容量、业务与连接世代回归继续通过。组件测试不替代真实 WebView/休眠或父崩溃验收                                                               |
| Web 生产构建                       | 通过         | `pnpm build:web` 的 TypeScript 与 Vite 构建通过；当前主入口 minified 848.11 kB、gzip 229.52 kB，仍有非阻断的 500 kB chunk 警告，后续按页面拆包                                                                                                                                               |
| 前端格式                           | 通过         | Web 范围 Prettier format check 通过                                                                                                                                                                                                                                                          |
| Go 全量测试与静态检查              | 通过         | `go test ./... -count=1` 与 `go vet ./...` 均通过；不虚构未统计的用例总数                                                                                                                                                                                                                    |
| 统一源码质量门禁                   | 通过         | 本轮以等价分项完成 `check:source` 全部门禁：Prettier、171 个 Go 文件与 Rust 格式、26 份 Markdown/本地链接、Web 类型/83 文件 539 项测试/构建、Go 无缓存测试/vet 与 x86_64-pc-windows-msvc Sidecar 构建                                                                                        |
