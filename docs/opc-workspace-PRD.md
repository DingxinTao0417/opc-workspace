...
> 文档日期：2026-08-29
> 实现基线：app v0.1.0 / API v1 / SQLite schema v42

> **v9.81 更新说明**：业务 JSON 与含文件 ZIP 的 preview 新增只读导入冲突事实。同 schema 非空目标按真实 SQLite 复合主键返回逐表源/目标行数与重叠数，不返回 ID 或正文；跨 schema 包分类为源版本更旧/更新。设置页展示清单与方向，确认按钮保持禁用；apply 仍只接受同 schema 空目标，并在任何容量探测、回滚包或业务写入前拒绝 blocker。app v0.1.0、API v1、schema v42 均不变。
...
> 状态截止：2026-08-29。当前版本是可运行、可扩展的 v0.1 基座，并已前置交付 v0.2 首个受限预设自动化纵切。schema v42、Project 完成→本地核对 Inbox、daily/weekly/weekdays/monthly 本地 Reminder、设置预览/保存/启停、持久化关闭到托盘偏好、Run 历史/重试、IANA/DST/月末、离线折叠和导入导出、Adapter 登记/安全诊断，以及客户回访的计划/终态数据契约已经接通；这不代表完整 v0.2、Agent Runtime、发票能力、自由规则、Task 自动化动作、Focus 原生反馈、复杂通知、数据库打开前备份选择或冲突合并导入已经交付。
...
| 前端测试                                           | 通过         | 全量 90 个文件 / 611 项通过；新增业务导入冲突元数据严格解析、聚合校验、跨 schema 方向、逐表清单和禁用确认，以及既有桌面能力、设置模块可见性、内容日历/路线图、客户动态、启动进度、Agent Adapter、Automation、Reminder、内部回滚容量、关闭到托盘与连接世代回归。组件测试不替代真实 WebView 验收 |
| Web 生产构建                                       | 通过         | `pnpm build:web` 的 TypeScript 与 Vite 构建通过；当前主入口 minified 944.36 kB、gzip 252.68 kB，仍有非阻断的 500 kB chunk 警告，后续按页面拆包                                                                                                                                                 |
| 前端格式                                           | 通过         | Web 范围 Prettier format check 通过                                                                                                                                                                                                                                                            |
| Go 全量测试与静态检查                              | 通过         | `go test ./... -count=1` 与 `go vet ./...` 均通过；不虚构未统计的用例总数                                                                                                                                                                                                                      |
| 统一源码质量门禁                                   | 通过         | 本轮完成源码门禁：Prettier、194 个 Go 文件与 Rust 格式、26 份 Markdown/本地链接、Web 类型/90 文件 611 项测试/构建、Go 无缓存测试/vet 与 x86_64-pc-windows-msvc Sidecar 构建                                                                                                                    |
