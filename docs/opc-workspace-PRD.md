...
> **v9.69 更新说明**：路线图 R4 已补齐跨年度连续排期；年度视图首尾提供上一年 Q4/下一年 Q1 拖拽落点，卡片的上一季度/下一季度按钮在 Q1/Q4 边界跨年。每次移动继续携带当前版本并把目标日期设为目标季度末；schema 仍为 v39，无 AI/LLM、外发或 demo 数据。
...
| 前端测试                           | 通过         | 全量 89 个文件 / 589 项通过；新增路线图 Q4→下一年 Q1 键盘移动和 Q1→上一年 Q4 拖拽落点回归，既有路线图季度/精确日期/Inbox、内容日历预移与分页、启动进度、Agent Adapter、Automation、Reminder、内部回滚容量、业务与连接世代回归继续通过。组件测试不替代真实 WebView 验收                       |
