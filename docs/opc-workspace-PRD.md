...
> **v9.62 更新说明**：路线图 R3 已接入按 ID 最新详情读取，展示说明、Project/Task 派生事实、期间、版本和审计时间；错误可重试，非归档详情以最新版本衔接编辑，归档详情保持只读并提示先恢复。R3 基础界面完成，schema 仍为 v38，无 AI/LLM、外发或 demo 数据。
...
> **当前状态**：路线图 `/roadmap` 的 R3 基础界面已完成，R2 的 schema v36/API 提供 CRUD、筛选、项目关联、只读派生进度、乐观锁、归档恢复与重排；当前界面已接季度/状态/Project 浏览、稳定分页、新建、基础事实编辑、按 ID 详情、Project 深链、归档恢复和保护性永久删除，拖拽与 Inbox/Reminder 自动化仍待。内容日历已交付 schema v37 数据/API、六周月格、新建/编辑、IANA/DST 安全改期、人工发布确认、Task 关联及 schema v38 Inbox 来源契约，并完成审核/发布到期的启动/周期扫描、版本化去重、变更终结、删除协调、Inbox 来源详情与六周范围自动分页聚合。目标版本为 v0.3。
...
| 前端测试                           | 通过         | 全量 89 个文件 / 576 项通过；新增路线图按 ID 详情解析、Project/Task 派生事实展示和详情最新版本编辑衔接覆盖，既有路线图筛选分页/编辑删除、内容日历、启动进度、Agent Adapter、Automation、Reminder、内部回滚容量、业务与连接世代回归继续通过。组件测试不替代真实 WebView 验收                  |
...
9. **扩展本地规划与预设自动化**：预设自动化首个纵切已交付 Project 完成 Inbox 与 daily/weekly Reminder；路线图已完成数据/API 和首个页面纵切，内容日历已完成 CC1–CC5-B。下一步完成路线图编辑/拖拽/来源投影、内容日历大数据量与真实 WebView 专项，并在发票/Agent 事实上线后分别启用对应预设；Task 动作和自由规则仍待后续，任何规则都不得自动对外发送。
...
| 13   | 路线图（v0.3）            | **R2 已完成**：roadmap_milestones、季度/日期/状态/项目关联，进度从项目/任务派生，编辑/归档/恢复/删除使用乐观版本                                                                                                                                                                                                                                                                                      | **R3 已完成**：季度/状态/Project 筛选、稳定分页、新增/编辑/按 ID 详情、日期与状态调整、项目关联/跳转、归档恢复和二次确认删除已交付；R4 拖拽待补                                                                                                                                                                         | API 已覆盖季度边界、进度口径、项目删除约束和并发冲突；Web 已覆盖筛选分页、详情最新事实、编辑载荷、版本头和保护性删除。拖拽回滚、真实浏览器焦点/窄屏及长列表仍待                                                                                                                                                                      |
