...
> **v9.36 更新说明**：客户回访计划表单把 `datetime-local` 的墙上时间按填写的 IANA 时区转换为 UTC；编辑同一计划时也按其保存时区回显，避免浏览器所在时区改变原计划。夏令时前跳的不存在时刻在前端拒绝，秋季回拨的重复时刻稳定选择较早物理时刻。无新 migration、无外发消息、无 AI/LLM。app v0.1.0 / API v1 / schema v35 不变。
