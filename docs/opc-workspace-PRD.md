...
> 文档日期：2026-08-29
> 实现基线：app v0.1.0 / API v1 / SQLite schema v39

> **v9.68 更新说明**：内容日历 CC4 拖拽现会立即把卡片预移到目标日期，并按条目 IANA 排期时区归日；保存期间冻结重复写入，失败或版本冲突立即回滚原日期并回读服务端，成功后由新版本事实收敛。schema 仍为 v39，无 AI/LLM、外发或 demo 数据。
...
> **当前状态**：路线图 `/roadmap` 的 R3 基础界面已完成；R4 已交付完整同季度集合的安全卡片排序、年度 Q1–Q4 时间线、跨季度拖拽/键盘移动、季度内三月日历精确日期拖拽/键盘日期输入，以及 PATCH 跨季度追加到目标季度尾部的事实规则。R2 的 schema v36/API 提供 CRUD、筛选、项目关联、只读派生进度、乐观锁、归档恢复与重排；R5 的 schema v39 已交付本地到期/达成 Inbox 来源、启动/周期补偿、稳定去重、变更终结、删除协调与来源详情。当前界面已接季度/年度、状态/Project 浏览、稳定分页、新建、基础事实编辑、按 ID 详情、Project 深链、归档恢复、保护性永久删除，以及安全排序/季度/精确日期移动。内容日历已交付 schema v37 数据/API、六周月格、新建/编辑、IANA/DST 安全改期、人工发布确认、Task 关联及 schema v38 Inbox 来源契约，并完成审核/发布到期的启动/周期扫描、版本化去重、变更终结、删除协调、Inbox 来源详情与六周范围自动分页聚合；拖拽按条目排期时区即时预移，失败/冲突回滚并回读服务端。目标版本为 v0.3。
...
- 当前阶段已实现的核心业务、Actor、任务分派、Submission/Artifact、手工 Inbox Item、Task 关系、一次性及 daily/weekly Reminder、已登记 Inbox 来源投影、批量拆分/分派、自动解决、设置、Focus、自动化预设与本地 Client Followup 均仅保存在本地，不主动访问线上服务；Agent Run 仍未实现
...
| 前端测试                           | 通过         | 全量 89 个文件 / 587 项通过；新增内容日历拖拽即时预移、IANA 时区归日、失败回滚与回读回归，既有路线图精确日期/Inbox、内容日历分页、启动进度、Agent Adapter、Automation、Reminder、内部回滚容量、业务与连接世代回归继续通过。组件测试不替代真实 WebView 验收                                   |
| Web 生产构建                       | 通过         | `pnpm build:web` 的 TypeScript 与 Vite 构建通过；当前主入口 minified 923.40 kB、gzip 246.56 kB，仍有非阻断的 500 kB chunk 警告，后续按页面拆包                                                                                                                                               |
| 前端格式                           | 通过         | Web 范围 Prettier format check 通过                                                                                                                                                                                                                                                          |
| Go 全量测试与静态检查              | 通过         | `go test ./... -count=1` 与 `go vet ./...` 均通过；不虚构未统计的用例总数                                                                                                                                                                                                                    |
| 统一源码质量门禁                   | 通过         | 本轮完成源码门禁：Prettier、188 个 Go 文件与 Rust 格式、26 份 Markdown/本地链接、Web 类型/89 文件 582 项测试/构建、Go 无缓存测试/vet 与 x86_64-pc-windows-msvc Sidecar 构建                                                                                                                  |
...
9. **扩展本地规划与预设自动化**：预设自动化首个纵切已交付 Project 完成 Inbox 与 daily/weekly Reminder；路线图已完成数据/API、R3 基础界面、R4 同季度/跨季度/精确日期排期和 R5 本地到期/达成 Inbox 来源，内容日历已完成 CC1–CC5-B、CC6-A 分页聚合和拖拽即时预移/失败回滚。下一步完成内容日历真实 WebView 大数据专项，并在发票/Agent 事实上线后分别启用对应预设；Task 动作和自由规则仍待后续，任何规则都不得自动对外发送。
...
| 13   | 路线图（v0.3）            | **R2/R5 已完成**：roadmap_milestones、季度/日期/状态/项目关联，进度从项目/任务派生；schema v39 增加本地到期/达成 Inbox 来源、扫描补偿、稳定去重、变更终结和删除协调                                                                                                                                                                                                                                   | **R3 已完成；R4 当前范围已完成**：季度/年度、状态/Project 筛选、稳定分页、新增/编辑/按 ID 详情、项目关联/跳转、归档恢复和二次确认删除已交付；完整同季度集合可原子排序，年度 Q1–Q4 时间线支持拖拽或上一/下一季度按钮移动，季度三月日历支持精确日期拖拽和键盘日期输入；Inbox 可查看里程碑快照并返回路线图；跨年度移动待后续 | API 已覆盖季度边界、跨季度尾部排序、进度口径、项目删除约束、并发冲突、本地日期到期、达成转换、重扫去重和删除协调；Web 已覆盖筛选分页、详情最新事实、编辑载荷、版本头、保护性删除、同季度重排、跨季度/精确日期移动、失败回读、容量保护及 Inbox 来源解析。真实浏览器拖拽/焦点、窄屏及长列表仍待专项验收                                |
| 14   | 内容日历（v0.3）          | **CC1/CC2/CC5-B 已完成**：content_items、平台/状态/发布时间/项目与任务关联，审核/发布时间幂等生成 Inbox Item                                                                                                                                                                                                                                                                                          | **CC3/CC4/CC5-A/CC6-A 已完成当前范围**：月视图、月份切换、详情、新建/编辑、IANA/DST 安全拖拽、即时预移/失败回滚、准备任务和六周范围自动分页聚合                                                                                                                                                                           | 组件测试已覆盖跨月、时区转换、即时预移、拖拽回滚、关联任务和重复提醒；不自动发布外部平台。真实 WebView 指针/键盘/窄屏与大数据性能仍待专项                                                                                                                                                                                            |
