...
> **v6.5 更新说明**：交付 Focus D2b 的本地日期范围回顾。Focus 页面复用既有 IANA 时区、completed-only 的 1–93 日统计 API，支持切换最近 7 天、最近 30 天、本月与自定义范围；前端拒绝倒置、无效或超过 93 天的范围，较长周期图表支持横向浏览。活动 Session、休息循环与历史分页保持独立；项目/标签分布、热力图、最佳时段和原生反馈仍待后续独立纵切。
...
> **当前状态**：核心本地搜索已完成。支持已交付页面/安全操作命令、真实 Task/Project/Client/活动 Inbox 统一搜索、精确可刷新详情路由和完整键盘交互；最近使用、健康诊断和 OS 全局快捷键仍待开发。
...
- **需求映射**：5.9。
- **用户流程**：`Ctrl/Cmd+K` 打开命令面板，`Ctrl/Cmd+N` 打开新建任务；面板可搜索已交付页面、真实本地 Task/Project/Client/活动 Inbox、新建任务和设置，支持方向键、Enter、Esc 与 Tab。选择业务结果进入带精确 ID 的详情路由，刷新仍定位同一资源；设置命令直接打开指定模块，关闭后焦点返回原触发元素。
...
- **验证/当前限制**：Go 契约测试覆盖四类型结果、归档/终态过滤、类型去重、稳定分页、文字 `%/_`、非法查询和只读空库；前端覆盖 strict 契约、防抖、错误重试、精确导航及 Task/Inbox 直达路由。当前不是 FTS/模糊搜索，没有高亮、最近使用、Sidecar 诊断入口、桌面启动恢复页、全局错误边界或 OS 全局快捷键；真实浏览器和桌面 WebView 焦点/视觉仍需人工验收。
