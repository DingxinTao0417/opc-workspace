...
> **v9.80 更新说明**：通用设置新增“关闭窗口时隐藏到托盘”。点击后通过固定布尔 Tauri command 即时预览，保存写入设置 schema v2，取消按顺序恢复 committed；schema v42 将既有通用设置补为默认启用并保留行版本，空库不创建默认数据。托盘不可用或偏好关闭时，主窗口关闭不再被拦截。API 路由仍为 v1。
...
| 明确边界     | 当前代码不使用 Docker；已实现 Task/Actor/D1/D2、Project、Client、Focus、Inbox/Reminder/Task 编排、已登记来源投影、首个受限预设自动化纵切、运行期数据库故障、可配置低空间安全投影、基础备份闭环、业务 JSON/ZIP 空工作区导入导出、脱敏诊断、数据库打开前白名单启动进度与启动恢复；通用托盘最小闭环已接源码但原生链接未验收；未实现 Client 外部来源、Focus 专用原生反馈、卷级趋势、启动前备份选择、复杂/原生通知、Agent、自由规则、非空目标/跨 schema 合并、AI、知识库或财务；自动化没有 Shell/SQL/HTTP/外发能力，person 只做本地责任记录，线上账号、云同步和远程协作均不在当前范围 |
...
| 前端测试                                           | 通过         | 全量 90 个文件 / 607 项通过；新增关闭到托盘命令映射、启动同步、即时预览/取消恢复/保存持久化，以及既有桌面能力、设置模块可见性、内容日历/路线图、客户动态、启动进度、Agent Adapter、Automation、Reminder、内部回滚容量、业务与连接世代回归。组件测试不替代真实 WebView 验收                   |
| Web 生产构建                                       | 通过         | `pnpm build:web` 的 TypeScript 与 Vite 构建通过；当前主入口 minified 940.58 kB、gzip 251.58 kB，仍有非阻断的 500 kB chunk 警告，后续按页面拆包                                                                                                                                               |
| 前端格式                                           | 通过         | Web 范围 Prettier format check 通过                                                                                                                                                                                                                                                          |
| Go 全量测试与静态检查                              | 通过         | `go test ./... -count=1` 与 `go vet ./...` 均通过；不虚构未统计的用例总数                                                                                                                                                                                                                    |
| 统一源码质量门禁                                   | 通过         | 本轮完成源码门禁：Prettier、194 个 Go 文件与 Rust 格式、26 份 Markdown/本地链接、Web 类型/90 文件 607 项测试/构建、Go 无缓存测试/vet 与 x86_64-pc-windows-msvc Sidecar 构建                                                                                                                  |
