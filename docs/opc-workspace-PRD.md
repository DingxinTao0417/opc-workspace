...
> 文档日期：2026-09-10
> 实现基线：app v0.1.1 / API v1 / SQLite schema v69
...
- 十张 AI 表（providers/sessions/generations/messages/memories/memory_entries/run_steps/evaluation_runs/evaluation_results/evaluation_reviews）继续排除便携业务 JSON/ZIP，但一致性 SQLite 备份覆盖；兼容图包含 v49/v63/v64/v65/v66/v67/v68→69，未知未来 schema 拒绝。
...
### 5.11 本地知识库（TXT/Markdown 基线已交付）

> **状态**：独立轨道持续交付，不属于 v0.1。ADR-009、schema 059、TXT/Markdown 受控导入、本地 Actor 异步索引/取消/retry/启动恢复、FTS5/中文辅助检索、来源定位、版本化重建、删除清理、来源清单和知识库页面已完成；ADR-010 AI6 只读片段集成也已交付。PDF、授权引用与来源变化未完成。

#### 目标能力

- 当前支持导入 Markdown/TXT 本地资料并记录来源、大小、更新时间、索引状态、版本和可删除范围；PDF 为下一阶段。
...
| **独立轨道（版本待定）** | AI 助手 AI5/AI6、AI7-Q1、Q2 离线 scorer/8-case smoke/四类 6-case topic/24-case full Actor/suite 趋势/category/failure/Wilson/候选/人工决定审计、Q3 steps/Provider token/本地聚合/UTC 趋势与知识库文本/Actor/FTS5 已交付；PDF、授权引用、费用与自动路由待开发 | 按 ADR 分阶段单独估算      |
...
| T-04 SQLite 初始化与迁移             | 已完成                           | schema v69（含 AI/知识库、配置身份与可靠确认迁移）、PRAGMA、嵌入迁移、demo 清理、Project/Task/Actor/Client/Focus/Inbox/Reminder、设置/保存视图、三类业务附件、Workspace Avatar、存储阈值、Submission origin、父任务 rollup guards、Project→Client Activity 唯一来源、Reminder 系列/occurrence、Automation Rule/Run、Agent Adapter 诊断、Client Followup 计划/终态事实与其他来源 guards                                                                                                                                                                            |
...
| T-16 本地知识库                      | K1–K6 文本基线完成               | ADR-009、schema 059、TXT/Markdown 受控导入、Actor 异步索引/取消/retry/启动恢复、FTS5/中文辅助检索、来源/版本/行号/字符引用、重建、单文档/来源删除、全库清理、metadata-only 清单、Web 页面及 ADR-010 AI6 已交付；PDF、授权引用与变化检测待后续                                                                                                                                                                                                                                                                                                                     |
