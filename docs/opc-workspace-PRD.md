...
| 区域         | 位置       | 宽度         | 职责                                                                                                                                    |
| ------------ | ---------- | ------------ | --------------------------------------------------------------------------------------------------------------------------------------- |
| 左侧导航栏   | 左侧固定   | 220px / 64px | 工作台模式：用户信息、全局搜索、模块导航、本周效率，宽屏可手动收起/展开；智能体模式：该列改为 AI 会话列表，两种模式的切换入口固定在列顶 |
...
| 右侧概览面板 | 无         | —            | 工作台布局不渲染任何右侧概览列。右侧工作栏只属于智能体模式（`/ai`）：展开为带页签停靠面板，收起为悬浮状态卡；专注控制移至左侧边栏       |
...
5. 用户从左侧边栏底部极简专注小组件进入/控制专注（时长由设置决定）；右侧概览收起时仅 AI 助手页显示悬浮状态卡、展开时为带页签面板，主内容区不再重复放置专注卡片
6. 专注期间：当前已记录绑定任务的有效区间并在左侧专注小组件显示剩余时间；暂停本应用通知与系统级勿扰引导仍按平台能力延后
...
| ---------------- | ---------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 今日工作台       | 三栏布局、今日任务列表、手动排序、逾期及临期提示、任务详情操作、右侧概览面板                   | **部分完成**：日期导航、真实统计/四组任务、精确日期/未排期按钮式排序、四组共享同日/跨日期拖拽、空精确日期/未排期落点、任意日期/未排期安排、安全执行快捷操作、编辑/版本化确认删除入口、共享任务详情、服务端逾期/未来 24 小时临期快捷筛选、真实 Focus 概览移至左侧极简小组件、completed-only 汇总保留；客户动态/路线图节点/本月收入改为左侧导航角标提醒，右侧概览为双形态（收起仅 AI 页悬浮状态卡 / 展开带页签面板：概要/子智能体/文件/浏览器）；外部线上客户动态不在当前边界 |
...
- **需求映射**：5.1–5.9、附录 A/B。
- **用户流程**：用户通过左侧导航访问今日、任务、项目、客户、收入、发票、收件箱和专注；左侧底部显示极简专注小组件与导航提醒角标，右侧概览为双形态（收起仅 AI 页悬浮状态卡 / 展开带页签面板）。
...
- **关键路径**：`apps/web/src/App.tsx`、`components/AppShell.tsx`、`Sidebar.tsx`、`FocusMiniCard.tsx`、`RightOverview.tsx`、`styles.css`。
...
- **工作区外壳**：亮/暗主题都以中性灰画布承托桌面工作区；主内容区在窗口四周保留间距，使用带边框和轻阴影的圆角工作面。左侧导航独立于主面，工作台模式的主工作面横向占满剩余宽度，不渲染右侧概览列。智能体模式下右侧概览可与主工作面同处一个框架（共用外边框、圆角与阴影，内部以 1px 发丝线分隔，宽度 240–480px 可拖拽），收起时回落为独立悬浮状态卡（18px 圆角、发丝描边、低强度阴影）。两种形态都按“分组标题 + 图标行 + 右对齐取值”排布，停靠面板的页签为纯文字下划线样式。
- **模式切换**：左列顶部固定一个工作台 / 智能体双态切换。模式即路由，不额外持久化：工作台模式保持业务导航与全部工作区页面，且不渲染任何右侧概览列；进入 `/ai` 即为智能体模式，左列整体替换为智能体侧栏——顶部依次是「会话」标题与隐藏开关、「新会话」「定时任务」「项目」三行入口，随后是会话搜索、`对话` 分组标题与按今天/昨天/更早分组的会话行，底部固定「AI 助手设置」。切回工作台会返回进入智能体前最后停留的页面。对话页头右上角是右侧工作栏开关：打开为带页签的停靠面板（概要/子智能体/文件/浏览器），关闭后回落为右侧悬浮状态卡；面板宽度可在 240–480px 之间拖拽或键盘调整（方向键 ±12px、Shift ±48px、Home 恢复 280px 默认值）。左右两个开关的状态都只作用于当前会话，不写持久化偏好。窄屏下左列与停靠面板收起，会话选择与模式切换移入对话页头；`/ai` 页面另有一层作用域限定的令牌覆盖，详见 [AI 助手模块](modules/ai-assistant.md)。
