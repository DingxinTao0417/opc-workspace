...
> **v3.3 更新说明**：交付设置“关于与诊断”健康事实基线：按需读取并严格校验 `/health`，展示真实 Sidecar 状态、应用名/运行版本/commit、API 版本、schema 和 SQLite 可用性；构建脚本注入 Git 短提交并标识 dirty 工作树；补齐加载、错误/request ID、重试、手动重新检查、最近成功结果降级及只读页脚。统一搜索、全局状态条、Sidecar 手动恢复、数据/日志目录和桌面能力诊断仍未交付。
...
> **当前状态**：部分完成。真实任务、持久化 Focus Session 和 Today Focus 汇总已接入，右栏读取真实活动 Session；任务已按所选本地日期完整分为逾期、当天、本周稍后和未排期，并支持前后切换及回到今天。Today 持久排序/拖拽、临期筛选、真实收入和客户动态尚未实现。
...
> **状态**：未开始；不属于 v0.1，目标版本和本地模型/运行时待单独评审。当前仓库没有模型 SDK、密钥、AI API、会话表或助手页面；PRD v3.3 当前阶段不接入线上模型服务。
...
首次启动不下载业务运行时或后端镜像。PRD v3.3 当前阶段不提供线上更新或第三方连接；安装、升级和核心使用均可在离线环境完成。
...
| ---------------- | ----------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 今日工作台       | 三栏布局、今日任务列表、手动排序、逾期及临期提示、任务详情操作、右侧概览面板                    | **部分完成**：布局、日期切换/回到今天、真实任务/Today 统计、逾期/当天/本周/未排期完整分组、共享任务详情、真实活动 Focus 概览与 completed-only Focus 汇总已接通；Today 持久排序/拖拽、收入与客户动态待实现                         |
...
- **建议开发流程**：
  1. 先完成本地运行时、资源预算、上下文权限和质量评测 ADR；PRD v3.3 当前阶段不评审远程 Provider。
