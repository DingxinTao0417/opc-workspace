...
> **v7.9 更新说明**：Focus D2b 周期报告新增项目时间分布。`/stats/focus` 在既有 completed-only、IANA 当地日和 interval overlap 窗口内，按 Session 绑定 Task 的查询时当前项目归属聚合秒数、分钟和 distinct Session；未绑定、Task 已删除或当前无项目归入“未归项目”。Focus 页展示项目占比与用时，不新增历史归属快照、不修改 Task/Project 事实；schema 保持 v28，标签分布、热力图、最佳时段及原生反馈仍待后续。
...
> **当前状态**：Focus Core A+B+C、D1、D2a，以及 D2b 日期范围回顾与项目时间分布已完成。schema v11、Session API、任务绑定、绝对时间快照、刷新/启动恢复、15 秒心跳、暂停/继续/停止/取消、精确工时记账、Today 当地日统计、终态历史、7/30 天/本月/自定义趋势、Streak、项目分布和 Task 详情记录已接通；标签分布、热力图、最佳时段以及原生通知、托盘和勿扰集成未实现。
...
| 专注模式         | 番茄钟、环形进度、工时记录、连续天数统计、暂停本应用通知、系统专注模式引导                 | **Core A+B+C、D1、D2a 与 D2b 日期范围回顾/项目分布已完成**：schema v11 Session/interval、任务绑定、绝对时间、心跳/恢复、并发/幂等、精确工时、Today 汇总、终态历史、7/30 天/本月/自定义趋势、Streak、项目分布与 Task 详情记录已实现；标签分布、热力图、最佳时段及原生通知/托盘/DND 待实现                             |
...
> 状态截止：2026-08-28。当前版本是可运行、可扩展的 v0.1 基座；T-18A/B/C/D D1/D2、T-12 Focus Core A+B+C/D1/D2a/D2b 项目分布、T-11A1/A2/A3/B/C/F、T-11E follow-up Artifact/Task 阻塞/Task 临期/Project 完成来源、备份四类操作失败、数据库启动/迁移和 Sidecar 启动失败投影、T-06A–H、T-07A–D 和 T-04B 一致性备份完整闭环/迁移前自动回滚包/桌面安全重启/基础业务 JSON 导出已经交付，但这不代表第 9.1 节的完整 MVP、Focus 标签分布/热力图/最佳时段/原生反馈、运行期数据库等其他系统故障来源、完整日志/恢复页、Agent、数据导入、含文件导出包或恢复诊断已经交付。
...
| 明确边界     | 当前代码不使用 Docker；已实现 Task 事实层、Actor/Assignment、D1 生命周期、Task 时间线、D2 manual Submission/Artifact/受控文件、Project 笔记/附件/产出聚合/活动时间线、Client 基础资料/Project 客户关联/人工活动/附件/person 关联、Focus Core A+B+C 与 D2b 日期范围回顾/项目分布、手工 Inbox/Reminder/Task 编排、follow-up Artifact/Task 阻塞/Task 临期来源投影与删除协调、备份四类操作失败、数据库启动/迁移和 Sidecar 启动失败的系统维护 Inbox 投影、基础备份闭环与业务 JSON 导出；未实现 Client 外部活动来源、Focus 标签分布/热力图/最佳时段/原生反馈、运行期数据库等其他系统故障来源、完整日志/恢复页、重复/原生通知、Agent、导入/含文件导出、AI 助手、知识库、客户回访或收入/支出/发票业务；person 只做本地责任记录，线上账号、云同步和远程协作均不在当前范围 |
...
| T-12 专注设置与全局计时              | Core+D1+D2a+D2b 项目分布完成     | A+B+C：schema v11 Session/interval、任务绑定、绝对时间、心跳/恢复、状态命令、幂等/并发、精确工时与 Today 汇总；D1/D2b：终态历史分页、7/30 天/本月/最多 93 天自定义本地日趋势、当前/最长 Streak 与按 Task 当前归属的项目分布；D2a：Task 详情按需专注记录已交付；标签/热力图/最佳时段及通知/托盘/DND 延后                                                                  |
...
- **需求映射**：5.7。
- **完成范围**：Core A 事实迁移、Core B API/状态机/事务、Core C 前端接入与恢复、D1 终态历史/七日报告、D2a Task 详情记录，以及 D2b 7/30 天、本月、自定义日期范围回顾和项目时间分布已完成；标签分布、热力图、最佳时段和系统集成延后。Today 与周期 completed-only 统计属于已交付闭环。
...
- **验证/当前限制**：自动化测试覆盖迁移、状态机、绝对时间、心跳、恢复、并发创建/停止、幂等重放、事务回滚、跨 Session 余秒、Task/Project 版本、历史筛选/分页、Task 详情按需读取/翻页/错误隔离，以及 IANA/DST/跨午夜 completed-only 日与周期统计、Streak 和项目/未归项目分布。Focus 参数已从 SQLite committed 读取；标签分布、热力图、最佳时段、原生通知、托盘和系统勿扰仍未实现，真实三平台后台/睡眠场景仍需桌面验收。
...
5. **扩展 Focus D2b**：Core A+B+C、D1、D2a 与日期范围回顾的持久化、恢复、精确工时、Today 统计、终态历史、7/30 天/本月/自定义趋势、Streak、项目时间分布和 Task 详情记录已交付；后续独立实现标签分布、热力图、最佳时段和经平台验收的原生通知、托盘、DND 引导。
