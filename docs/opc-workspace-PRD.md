...
> **v4.0 更新说明**：交付 T-06G Today 跨分组拖拽：四个可见活动分组共享拖拽状态，可落到具体目标任务并按其真实 `planned_date` 排序；空的所选日期和未排期组提供明确组级落点。跨日期先以版本化 batch 确认改期，再分别重建源/目标完整计划组顺序；排序部分失败会明确显示“已改期”及失败组并刷新事实，不把后续排序失败伪装成改期失败。行内编辑/删除继续延期。
...
> **当前状态**：部分完成。真实任务、持久化 Focus Session 和 Today Focus 汇总已接入，右栏读取真实活动 Session；任务已按所选本地日期完整分为逾期、当天、本周稍后和未排期，并支持日期导航。所选精确日期与未排期组支持按钮式/同组拖拽排序，四个可见分组支持按真实日期跨组拖拽，所有活动行支持任意日期或未排期安排，以及按状态/验收策略约束的开始、完成和开始专注快捷操作；行内编辑/删除、临期筛选、真实收入和客户动态尚未实现。
...
> **状态**：未开始；不属于 v0.1，目标版本和本地模型/运行时待单独评审。当前仓库没有模型 SDK、密钥、AI API、会话表或助手页面；PRD v4.0 当前阶段不接入线上模型服务。
...
首次启动不下载业务运行时或后端镜像。PRD v4.0 当前阶段不提供线上更新或第三方连接；安装、升级和核心使用均可在离线环境完成。
...
| ---------------- | ----------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 今日工作台       | 三栏布局、今日任务列表、手动排序、逾期及临期提示、任务详情操作、右侧概览面板                    | **部分完成**：日期导航、真实统计/四组任务、精确日期/未排期按钮式排序、四组共享同日/跨日期拖拽、空精确日期/未排期落点、任意日期/未排期安排、安全的开始/完成/开始专注快捷操作、共享任务详情、真实 Focus 概览与 completed-only 汇总已接通；行内编辑/删除、收入与客户动态待实现 |
| 任务管理         | 完整 CRUD、父子任务、状态流转、标签、项目关联、完成条件、人工验收、列表视图、搜索和快捷键       | **部分完成**：schema v6–v9 事实、责任、六状态、时间线与 manual Submission/Artifact 验收，快照幂等、`ETag`/`If-Match`、分页筛选、层级、批量/排序和受控文件均已实现；schema v11 Focus 工时已接入；Today 已消费按钮式与同日/跨日期拖拽排序，任务页拖拽与看板仍待实现           |
...
| T-07 任务管理纵向闭环                | 部分完成          | 任务事实、关系/标签、版本/ETag、分页筛选、批量/排序、Assignment、六状态/时间线、D2 manual Submission/Artifact/受控文件及 Focus 自动工时已交付；Today 已消费同日/跨日期拖拽排序，任务页拖拽与看板待实现 |
...
- **验证/剩余**：Go 测试覆盖 D1/D2 状态、JSON/multipart、限制、Actor 归属、并发/幂等、补偿、软/硬删除与文件完整性；数据库迁移套件通过。前端测试覆盖 manual 前置、混合提交、接受/返工、冲突保留 File、下载错误和软删确认。Focus 自动工时已由 schema v11/T-12 接入；任务页拖拽/看板以及 Today 指针拖拽、日期控件和窄屏专项真实浏览器验收仍未完成。
...
- **建议开发流程**：
  1. 先完成本地运行时、资源预算、上下文权限和质量评测 ADR；PRD v4.0 当前阶段不评审远程 Provider。
...
1. **D1/D2 已交付，收口任务体验**：schema v8 生命周期与 schema v9 manual Submission/Artifact 已完成；Today 已按 `planned_date` 完整查询并接入按钮、同日/跨日期拖拽和安全快捷操作。继续做真实浏览器键盘/焦点/窄屏验收、长历史性能和错误恢复；任务页仍只有上移/下移，不是拖拽，也没有看板。
...
| 6    | 今日                      | 按本地日期、逾期和本周范围查询；完整计划组排序事务；版本化单任务改期/生命周期命令；Focus Session 幂等创建；按 IANA 时区计算 UTC 边界；收件箱派生计数                             | 日期导航、真实分组、按钮/同日/跨日期拖拽、空精确日期/未排期落点、任意日期安排及安全的开始/完成/开始专注已交付；行内编辑/删除待开发；财务卡标后续           | 已覆盖完整分页、排序集合/版本、终态槽位、跨组乐观回滚、空组、改期冲突/模糊响应、排序部分成功和快捷策略；仍需真实浏览器验证 hover/focus、日期控件、指针拖拽、窄屏及午夜/夏令时边界 |
...
已交付：Task 事实与项目关联 + Project 基础纵切
  ...
  → 已交付：Focus Core 持久化/工时/Today 统计与今日完整日期编排；行内编辑/删除和真实浏览器验收继续
  → 备份恢复、桌面日志和故障恢复
  → 本地 Agent
  → 预设自动化与后续业务事件源
```
