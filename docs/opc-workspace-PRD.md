...
> **v9.27 更新说明**：交付 T-13 的首个原生全局快捷键纵切。Tauri 只尝试注册 `⌘/Ctrl+Shift+K`（命令面板）和 `⌘/Ctrl+Shift+N`（新建任务）；触发时显示/聚焦主窗口并向 WebView 发出固定 action。注册冲突、权限或平台失败只返回 `registered/unavailable` 白名单状态并保留现有应用内快捷键，不携带原始平台错误。设置运行诊断展示结果；自定义、专注、页面切换及真实三平台验收仍待后续。app v0.1.0 / API v1 / SQLite schema v34 不变。
