...
> **v7.2 更新说明**：交付数据库与 Sidecar 启动失败补偿。数据库尚未可写时，将 `database_startup / database_migration / sidecar_startup` 白名单 kind、稳定 UUID 和 UTC 时间原子写入独立 `OPC_LOG_DIR/startup-incidents-v1.json`；不记录 raw error、路径、Token、请求正文或业务数据。下一次成功打开并迁移数据库后、Router ready 前补偿为 `database:startup` / `database:migration` / `sidecar:startup` 系统维护 Inbox Item。稳定 event key 覆盖“数据库已提交但 journal 清理不确定”的重放，损坏/未知/超限 journal 隔离且不读取。`OPC_LOG_DIR`/`--logs` 默认数据库同级 `logs/`，并与 Artifact/backup root 隔离；完整日志、轮转和恢复页仍待开发。schema 保持 v27。
...
| 数据目录     | Tauri Path API（`appDataDir` / `appLogDir`） | 已实现目录初始化与启动故障 journal；完整日志管线未接入   |
...
└─ opc-workspace.log         # 规划；完整日志落盘管线尚未完成
```
...
> 状态截止：2026-08-28。当前版本是可运行、可扩展的 v0.1 基座；T-18A/B/C/D D1/D2、T-12 Focus Core A+B+C、T-11A1/A2/A3/B/C/F、T-11E follow-up Artifact/Task 阻塞/Task 临期来源、备份四类操作失败、数据库启动/迁移和 Sidecar 启动失败的系统维护投影、T-06A–H、T-07A–D 和 T-04B 一致性备份完整闭环/迁移前自动回滚包/桌面安全重启/基础业务 JSON 导出已经交付，但这不代表第 9.1 节的完整 MVP、Focus 高级分析、运行期数据库等其他系统故障来源、完整日志/恢复页、Agent、数据导入、含文件导出包或恢复诊断已经交付。
...
| 明确边界     | 当前代码不使用 Docker；已实现 Task 事实层、Actor/Assignment、D1 生命周期、Task 时间线、D2 manual Submission/Artifact/受控文件、Project 笔记/附件/产出聚合/活动时间线、Client 基础资料/Project 客户关联/人工活动/附件/person 关联、Focus Core A+B+C 与 D2b 日期范围回顾、手工 Inbox/Reminder/Task 编排、follow-up Artifact/Task 阻塞/Task 临期来源投影与删除协调、备份四类操作失败、数据库启动/迁移和 Sidecar 启动失败的系统维护 Inbox 投影、基础备份闭环与业务 JSON 导出；未实现 Client 外部活动来源、Focus 高级分析/原生反馈、运行期数据库等其他系统故障来源、完整日志/恢复页、重复/原生通知、Agent、导入/含文件导出、AI 助手、知识库、客户回访或收入/支出/发票业务；person 只做本地责任记录，线上账号、云同步和远程协作均不在当前范围 |
...
| T-11 收件箱与工作编排中心            | 部分完成                         | T-11A1/A2/A3/B/C/F 已交付；T-11E 已交付 follow-up Artifact、Task 阻塞、提前 24 小时 Task 临期、备份四类操作失败、数据库启动/迁移和 Sidecar 启动失败投影；运行期数据库等其他系统故障与 Agent 待实现                                                                                                                                                                       |
