...
> **v5.9 更新说明**：交付 Project 人工笔记纵切。schema v21 新增独立 `project_notes`、稳定时间线、身份/删除历史保护和 Project 聚合版本传播；API 支持幂等创建、分页读取、笔记级 `If-Match` 编辑、带原因软删除及归档只读，业务 JSON 纳入笔记事实。项目详情提供完整加载/空/错误、新增/编辑/删除历史交互，并明确永久删除 Project 会级联删除笔记；项目笔记不混入不可变 Workflow Event。项目产出/附件与 Inbox 来源投影仍待开发。
...
> **当前状态**：部分完成。当前 schema v21 保留并验证了由 schema v3–v5 交付的 Project model、CRUD、分页/搜索/筛选、快照式创建幂等、覆盖聚合事实的 `If-Match` 乐观锁、受控状态流转、归档恢复、确认硬删除，以及真实卡片/详情/任务聚合。schema v21 新增独立、版本化 Project Note，支持幂等创建、稳定分页、乐观编辑、带原因软删除、删除历史和归档只读，并原子传播 Project 聚合版本。Client CRUD、客户选择/改绑/解除和客户筛选已接通。Focus 完成会更新绑定 Task 工时并沿既有聚合链刷新 Project。Project 写命令继续同事务追加不可变 Workflow Event；项目笔记不混入命令审计。项目级产出/附件和 Inbox 来源投影仍未实现。Task 六状态上线后 Project 仍只把 done 计为完成，cancelled 留在总数/剩余口径。
...
当前工时聚合任务表 `actual_minutes`；完成的绑定 Focus Session 已在同一事务中按精确秒数 ledger 折算新增完整分钟写回 Task，并通过既有 trigger 递增 Project 聚合版本。项目新建/编辑可从真实 Client API 选择、改绑或解除客户，项目列表支持客户筛选；默认项目列表排除归档项，Client 详情为完整关联历史显式包含归档项目。项目详情已接系统写命令活动时间线。项目产出、附件、可编辑笔记、发票明细、收入/成本和 Focus 高级分析仍未接入。
...
> **状态**：未开始；不属于 v0.1，目标版本和本地模型/运行时待单独评审。当前仓库没有模型 SDK、密钥、AI API、会话表或助手页面；PRD v5.9 当前阶段不接入线上模型服务。
...
首次启动不下载业务运行时或后端镜像。PRD v5.9 当前阶段不提供线上更新或第三方连接；安装、升级和核心使用均可在离线环境完成。
...
| T-08 项目管理                        | 部分完成         | CRUD、分页/搜索/筛选、创建幂等、乐观锁、受控状态、归档恢复、确认硬删除、卡片/详情、任务聚合、客户选择/筛选、可编辑人工笔记和追加式项目活动时间线；项目产出/附件与 Inbox 来源投影待实现                                                                                                                |
...
- **需求映射**：5.3。
- **当前状态**：部分完成；项目资料、基础生命周期、任务聚合、Client 客户关联、可编辑人工笔记和追加式项目活动时间线已接通，项目产出/附件与 Inbox 来源投影仍未完成。
...
- **建议开发流程**：
  1. 先完成本地运行时、资源预算、上下文权限和质量评测 ADR；PRD v5.9 当前阶段不评审远程 Provider。
...
2. **收口项目基础纵切**：CRUD、schema v4 幂等快照、schema v5 聚合版本、乐观锁、归档关联约束、状态流转、归档恢复、确认硬删除、任务聚合、Client 选择/筛选、schema v21 可编辑人工笔记和追加式活动时间线已交付；继续验证大数据量串行分页、真实浏览器/窄屏/焦点，并保留项目产出/附件/Inbox 来源投影为明确缺口。
...
| 3    | 项目                      | CRUD、schema v4 快照式创建幂等、schema v5 聚合版本、乐观锁、归档关联约束、状态流转、归档恢复、确认硬删除、任务派生进度/工时、Client 选择/筛选、schema v21 Project Note 及 Project Workflow Event 已交付；产出/Inbox 来源投影待开发 | 卡片、分页/搜索/状态/客户筛选、新建/编辑、Client 选择、详情任务列表/工时、人工笔记、归档恢复和追加式活动时间线已交付；任务树、产出与附件待实现                           | 已覆盖事件同事务/不可变、笔记幂等/版本/软删除/归档只读、稳定分页/快照/删除审计及独立 UI 状态；继续验收聚合并发、分页拉全性能，后续验收项目产出→拆分→分派→验收和来源事件去重                 |
