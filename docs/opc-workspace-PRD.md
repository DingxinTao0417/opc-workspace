...
> **v4.9 更新说明**：交付 T-04B 备份确认删除，不增加 SQLite schema。`DELETE /backups/:id?confirm=true` 仅接受 canonical UUID，递归拒绝 symlink/reparse 和非普通文件；有效或损坏包都会先在同一 backup root 原子重命名为 `.deleting-<id>` 并同步目录，再永久清理，中断后同一请求可从隐藏态续删。设置页为每个包提供独立二次确认、进行中/成功/失败反馈，删除不会改变当前 SQLite 或 Artifact；pending 恢复期间删除与普通 API 一并冻结。JSON 导出、迁移前自动备份、桌面一键重启和高级导入继续独立实现。
...
> **状态**：未开始；不属于 v0.1，目标版本和本地模型/运行时待单独评审。当前仓库没有模型 SDK、密钥、AI API、会话表或助手页面；PRD v4.9 当前阶段不接入线上模型服务。
...
├─ backups/                  # 已实现创建/列表/校验/演练/重启恢复/确认删除；导出待实现
...
版本边界：手动一致性备份的创建、内部列表、完整校验、隔离恢复演练、恢复前自动回滚包、下一次 Sidecar 启动原子替换和确认删除已经交付；v0.1 仍须交付迁移前备份、基础 JSON 导出和桌面一键重启/恢复诊断。v0.3 只增加可配置计划、外部目录、保留策略、CSV 映射和高级导入工具。
...
首次启动不下载业务运行时或后端镜像。PRD v4.9 当前阶段不提供线上更新或第三方连接；安装、升级和核心使用均可在离线环境完成。
...
| 数据持久化       | Tauri `appDataDir`、SQLite 迁移、受控 Artifact、手动/迁移前一致性备份、基础 JSON 导出与原子恢复 | **部分完成**：正式/开发隔离、WAL、外键、schema v17、重建迁移安全、设置/保存视图事实、数据库身份强绑定和 Artifact store 已实现；手动 SQLite+Artifact 备份创建/列表/完整校验、隔离演练、自动回滚点、重启原子恢复及确认删除已交付；迁移前备份、导入导出和桌面一键重启仍待实现；每日计划和高级导入归 v0.3 |
...
> 状态截止：2026-08-28。当前版本是可运行、可扩展的 v0.1 基座；T-18A/B/C/D D1/D2、T-12 Focus Core A+B+C、T-11A1/A2/A3/B/C/F、T-06A–H、T-07A–D 和 T-04B 手动备份创建/列表/校验/隔离演练/重启恢复/确认删除已经交付，但这不代表第 9.1 节的完整 MVP、Focus D、非 Reminder 来源投影、Agent、备份导出、桌面一键重启或迁移前自动备份已经交付。
...
| 明确边界     | 当前代码不使用 Docker；已实现 Task 事实层、Actor/Assignment、D1 生命周期、Task 时间线、D2 manual Submission/Artifact/受控文件、Client 基础资料/Project 客户关联、Focus Core A+B+C、手工 Inbox 受理/分诊、Reminder、Inbox Task 批量拆分/分派/自动结清及基础备份恢复/删除；未实现 Client 活动/附件/Actor 显式关联、Focus D、非 Reminder 来源投影、重复/原生通知、Agent 执行、备份导出、AI 助手、知识库、客户回访或收入/支出/发票业务；person 只做本地责任记录，线上账号、云同步和远程协作均不在当前范围 |
...
| T-04B 手动一致性备份                 | 基础安全闭环完成  | 专用 backup root、维护写锁、SQLite `VACUUM INTO`、active Artifact+marker+manifest、完整校验、同卷原子发布、创建幂等、列表/重新校验、隔离演练、自动回滚点、pending/applied 提交点、重启原子恢复、确认删除及设置 UI 已交付；导出、桌面一键重启和迁移前自动备份待后续纵切 |
...
| T-14 设置持久化                      | 核心闭环完成      | schema v16、四模块严格 schema、原子 PATCH、乐观锁、Query committed、启动门禁、旧值迁移和数据备份/恢复/删除模块已交付；受控头像文件、备份导出待开发                                                                                                                     |
...
- **验证/剩余**：迁移测试覆盖连续旧版本数据保留、Task/Project/Client 版本与关系、Submission/Artifact 约束、Client 空值/索引/trigger、Focus interval/ledger，以及 v16→v17 保存视图事实保留和数据库约束。手动一致性快照、隔离恢复演练和重启原子恢复见 T-04B；迁移前自动备份和导入导出仍未实现。

#### 10.4.4A T-04B 手动一致性备份

- **需求映射**：7.1–7.4、9.1、10.3.3。
- **用户流程**：设置 → 数据与备份读取真实本地包；用户可填写说明并创建，成功后查看创建/校验时间、schema、Artifact 数和总大小，也可重新校验或隔离演练。点击“恢复此备份”后必须二次确认；成功挂起会显示目标/自动回滚短 ID，并要求关闭重开应用。点击“删除备份”会进入独立永久删除确认，成功后从真实列表移除，不改变当前数据。
- **实现方法**：`--backups` / `OPC_BACKUP_DIR` 声明与数据库同级且不和 Artifact 重叠的安全 root。所有普通 API、Focus heartbeat 与 Reminder 扫描共享维护读锁，创建备份和安排恢复取得写锁；用 `VACUUM INTO` 创建 WAL 一致性 SQLite 快照，复制数据库绑定 marker 与数据库当前全部 active file Artifact，在同卷 `.staging-<uuid>` 中校验预期文件全集、size/SHA-256、`quick_check`、`foreign_key_check`、schema、database/store identity 和 Artifact 元数据，再原子重命名为 UUID 包。创建可选 Idempotency-Key 只以 hash 写 manifest，模糊响应可安全重放。恢复演练在 `.restore-drill-*` 完成迁移和复验后清理。实际恢复严格确认后再次演练目标、创建回滚包并发布 `.restore-pending-v1` 私有副本；下一进程在数据库/Artifact lease 打开前验证目标和回滚包、准备迁移副本，通过同父目录 new/old 路径交换 SQLite/WAL/SHM 与完整 objects，失败回滚并隔离计划，成功将 pending 原子重命名为 `.restore-applied-*` 提交点后清理。删除要求 canonical UUID 与 `confirm=true`，有效或损坏包均递归拒绝 symlink/reparse/非普通文件，随后原子重命名为 `.deleting-<id>`、同步并永久清理；中断后同一删除从隐藏态恢复。
- **关键路径**：`services/sidecar/internal/api/backups.go`、`backup_drill.go`、`backup_restore.go`、`backup_delete.go`、`backups_test.go`、`cmd/server/main.go`、`internal/config/config.go`、`scripts/dev.mjs`、`apps/web/src/components/BackupSettings.tsx`、`api/client.ts`、`api/hooks.ts`。
- **验证/剩余**：Go 集成测试覆盖含真实 file Artifact 的快照边界、包结构、列表、幂等重放/冲突、显式校验、隔离演练、自动回滚点、pending 同目标重放/异目标冲突、业务冻结、启动恢复、目标/回滚内容、WAL 部分移动补偿、临时清理、有效/损坏包确认删除、篡改拒绝及输入校验；前端覆盖严格契约、恢复/删除确认请求序列化和设置入口交互。JSON 导出、迁移前备份、大数据量进度/取消、桌面一键重启和真实磁盘故障仍未实现。
...
- **建议开发流程**：
  1. 先完成本地运行时、资源预算、上下文权限和质量评测 ADR；PRD v4.9 当前阶段不评审远程 Provider。
...
6. **补数据安全链路**：手动一致性备份的创建、列表、完整校验、维护写锁、原子发布、隔离演练、恢复前回滚点、重启原子恢复与确认删除已经交付；下一步依次实现基础 JSON 导出、迁移前自动备份、桌面一键重启和诊断包内容选择。
...
| 8    | 设置与命令面板            | Actor API、health/version、schema v16 app_settings、GET/PATCH、Query committed、旧 localStorage 缺失模块迁移和手动备份/恢复 API 已接入；头像文件引用和统一 search API 待实现             | “人员与责任”支持 owner/person；设置支持 7 个左栏模块、启动加载、即时预览、按变化保存、取消恢复和错误保留；“关于”展示真实运行事实，“数据与备份”支持创建、列表、重新校验、演练和二次确认恢复 | 已覆盖严格契约、原子保存/回滚、乐观锁、启动门禁、兼容迁移、失败草稿、Focus 隔离、备份确认交互、Task 搜索、IME、焦点恢复和 health；仍需真实 WebView/窄屏验收，通知/快捷键/完整诊断仍为说明态 |
| 9    | 数据安全                  | 已交付 v0.1 手动一致性快照、manifest/SHA-256、维护写锁、临时库完整验证、同卷原子发布、隔离演练、自动回滚点、重启原子恢复和确认删除；迁移前备份和基础 JSON 导出待实现；v0.3 增加计划/映射 | 已交付手动创建、列表、重新校验、恢复演练、恢复/删除二次确认与关闭重开指引；导出入口将在对应安全纵切实现时开放                                                                              | 已覆盖 WAL 活跃、真实 Artifact、幂等、篡改拒绝、临时演练、pending 重放/冲突、写入冻结、启动替换、失败回滚、有效/损坏包删除和清理；仍须覆盖低磁盘、桌面一键重启和真实磁盘故障                |
...
已交付：Task 事实与项目关联 + Project 基础纵切
  ...
  → 已交付手动备份创建/列表/完整校验/隔离演练/重启恢复/确认删除；继续导出、桌面日志和故障恢复
  → 本地 Agent
  → 预设自动化与后续业务事件源
```
