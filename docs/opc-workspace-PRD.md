...
> **v3.0 更新说明**：交付 T-06B Today 日期导航：日期栏提供前一天、后一天和条件显示的“回到今天”，切换后任务四分组与 Today 统计使用同一所选日期重新查询；非当天视图明确标记“所选日期”，避免把过去或未来日期误称为今天。Today 组内持久排序/拖拽、快捷操作和项目/客户/财务概览仍未交付。
...
> **状态**：未开始；不属于 v0.1，目标版本和本地模型/运行时待单独评审。当前仓库没有模型 SDK、密钥、AI API、会话表或助手页面；PRD v3.0 当前阶段不接入线上模型服务。
...
首次启动不下载业务运行时或后端镜像。PRD v3.0 当前阶段不提供线上更新或第三方连接；安装、升级和核心使用均可在离线环境完成。
...
- **建议开发流程**：
  1. 先完成本地运行时、资源预算、上下文权限和质量评测 ADR；PRD v3.0 当前阶段不评审远程 Provider。
