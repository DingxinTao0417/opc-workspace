...
> **v9.34 更新说明**：补齐 T-17/C5 客户回访的 Today 与 Inbox 入口。Sidecar 的到期 `client_followup` Inbox 投影现在由前端严格校验；Inbox 来源上下文只提供回到客户详情的本地深链，Today 通过受限 `source_entity_type=client_followup` 筛选展示前五项待办和总数。更新、完成、跳过、取消和重排与旧到期投影的归档在同一事务完成，并失效 Today/Inbox 缓存；终态命令仍只在客户详情执行，不会外发消息或伪造沟通结果。app v0.1.0 / API v1 / schema v35 不变。
...
| 客户管理         | 客户列表表格、客户详情、基本 CRUD                                                              | **部分完成**：基础资料 CRUD、分页/搜索/状态筛选/排序、创建幂等、并发控制、基础详情、受约束删除、Project 关联、本地活动、受控附件和 person 显式关联已实现；schema v35 回访计划/终态、C3/C4 API、C5 到期 Inbox 投影、客户详情时间线/创建/执行表单及 Today/Inbox 入口已交付；外部来源投影和财务待实现                                                                                                                                                                                                         |
