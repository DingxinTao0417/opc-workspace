...
> 文档日期：2026-09-10
> 实现基线：app v0.1.1 / API v1 / SQLite schema v71

> **v10.9 更新说明**：[ADR-025](adr/025-ai-reliability-confirmations-and-evaluation-identity.md) 收口 AI 运行隔离与总预算、流终态、应用级恢复、任务原子确认、记忆决定、临时会话隐私和分段压缩；schema 068–069 增加配置/请求/确认身份与水位线，dataset v4 增加有界事实回归并按配置隔离质量证据。保留既有来源、用量与评测能力；完整验证集中于 AI7 计划，自动化不替代真实模型和原生故障验收。
...
| 区域         | 位置       | 宽度         | 职责                                                                 |
| ------------ | ---------- | ------------ | -------------------------------------------------------------------- |
| 左侧导航栏   | 左侧固定   | 220px / 64px | 用户信息、全局搜索、模块导航、本周效率；宽屏可手动收起/展开          |
...
| 右侧概览面板 | 右侧固定   | 280px        | 专注模式环形进度、临近路线图节点、本月收入迷你图、客户动态           |
...
10. 客户动态通过跨客户只读分页 API 获取，按发生时间和 ID 稳定倒序；右栏只取最近 3 条并深链客户详情，不复制活动或生成 Today Task
11. 临近项目节点分别读取 `status=planned/active&sort=target_date&page_size=3`，再以目标日期和 ID 合并取前 3 条；关联项目与任务完成数来自 Roadmap 响应的 Project/Task 派生事实，Today 不保存第二份进度。每条通过 `/roadmap?milestone=<id>` 打开指定详情；关闭或进入编辑仅移除 `milestone`，保留其他 URL 查询上下文
...
> **当前状态**：部分完成。T-11A1/A2/A3/B/C/F、既定来源与系统维护投影已交付；设置页已交付物理卷同卷去重、无路径手动容量检查和 30 天本地历史/7 天趋势。重复/原生通知和 Agent Run 仍是规划，v0.1 不启用 AI/LLM/Agent。
...
> **状态**：独立轨道持续交付（schema 052–069）。Provider、双协议会话、Harness、AI5/AI6、回答级 citation、dataset v4 的 smoke/topic/full 配置身份评测与人工决定审计、无正文 steps/用量/UTC 趋势，以及 [ADR-025](adr/025-ai-reliability-confirmations-and-evaluation-identity.md) 的运行隔离、总预算、确认与恢复已交付。生产只注册记忆三工具；费用、句子级证据、自动路由和真实模型质量验收仍为后续工作。
...
- Harness 运行时：`internal/harness` 承载工具 allowlist、跨帧调用聚合、执行循环与取消。一次 generation 的全部模型轮、工具输出和自检修订共享 10 分钟/累计 1 MiB，另有限制 8 轮、32 次工具、单工具 30 秒/64 KiB。生产仅注册记忆三工具，其他工具须逐个评审。
...
- 模型输出是不可信预览，自动建任务、Shell/SQL/业务写工具、自主代理均禁止；用户确认的共享 Task 事务不属于模型自主写入。Registry 只含记忆三工具，AI6 仅注入显式静态知识，不自动检索整库。
- 远程外发包括代码系统提示、已确认长期记忆、持久会话摘要/事实和最近回合、当前输入、允许的 memory_search 命中、逐字段确认的业务快照和最多 3 个逐段确认的知识 chunk。不扩展未选关联对象，preview 展示正文、位置、版本与字节数。
...
- 十张 AI 表（providers/sessions/generations/messages/memories/memory_entries/run_steps/evaluation_runs/evaluation_results/evaluation_reviews）继续排除便携业务 JSON/ZIP，但一致性 SQLite 备份覆盖；兼容图包含 v49/v63–v69→70，未知未来 schema 拒绝。
- 费用、非记忆工具、自动路由、句子级证据和更广泛语义质量需单独评审。真实模型质量、原生 WebView 输入法、真实断网/父进程崩溃与跨平台安全存储仍需实机验收，完整门禁记录集中于 [AI7 计划](plans/ai-quality-gates.md)。

### 5.11 本地知识库（TXT/Markdown/PDF 基线已交付）

> **状态**：独立轨道持续交付，不属于 v0.1。ADR-009、schema 059、TXT/Markdown 受控导入、本地 Actor 异步索引/取消/retry/启动恢复、FTS5/中文辅助检索、来源定位、版本化重建、删除清理、来源清单和知识库页面已完成；ADR-010 AI6 只读片段集成与 ADR-026（schema 070）PDF 提取、页码定位、损坏/加密/无文本页防护也已交付。授权引用与来源变化检测未完成。
...
跨客户只读端点 `GET /api/v1/client-activities?page=&page_size=&kind=` 只读取未删除活动，默认 5/页、最大 20/页，复用规范 UTC 纳秒排序并以 ID 打破同刻并列；响应在活动严格契约外附加当前 `client_name / client_status`。RightOverview 固定取最近 3 条并深链客户详情；该端点不提供写入、不生成第二份活动或 Inbox，也不推断线上行为。
...
| ---------------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 今日工作台       | 三栏布局、今日任务列表、手动排序、逾期及临期提示、任务详情操作、右侧概览面板                   | **部分完成**：日期导航、真实统计/四组任务、精确日期/未排期按钮式排序、四组共享同日/跨日期拖拽、空精确日期/未排期落点、任意日期/未排期安排、安全执行快捷操作、编辑/版本化确认删除入口、共享任务详情、服务端逾期/未来 24 小时临期快捷筛选、真实 Focus 概览、completed-only 汇总、最近 3 条本地客户动态及按目标日期聚合的 3 条 planned/active 路线图节点已接通；收入待实现，外部线上客户动态不在当前边界                                 |
...
| API / Schema | API v1；SQLite schema v70                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 数据默认值   | 开发数据库默认空白，不自动注入 demo 业务数据                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 明确边界     | 当前代码不使用 Docker；已实现 Task/Actor/D1/D2、Project、Client、Focus、Inbox/Reminder/Task 编排、已登记来源投影、首个受限预设自动化纵切、运行期数据库故障、可配置低空间安全投影、基础备份闭环、业务 JSON/ZIP 空目标及同 schema 零主键冲突追加、只读冲突/schema 方向预检、脱敏诊断、数据库打开前白名单启动进度与启动恢复、AI5/AI6、回答级 citation、dataset v4 配置身份评测、ADR-025 运行可靠性/确认恢复与知识库 Actor/文本检索基线；通用托盘最小闭环已接源码但原生链接未验收；未实现 Client 外部来源、Focus 专用原生反馈、启动前备份选择、复杂/原生通知、完整 Agent Runtime、自由规则、真实冲突策略/UUID 重映射/跨 schema 升级、知识库授权引用/变化检测或句子级 AI 引用支持验证；自动化没有 Shell/SQL/HTTP/外发能力，person 只做本地责任记录，线上账号、云同步和远程协作均不在当前范围 |
...
| T-04 SQLite 初始化与迁移             | 已完成                           | schema v71（含 AI/知识库、PDF 页码、Agent Run、配置身份与可靠确认迁移）、PRAGMA、嵌入迁移、demo 清理、Project/Task/Actor/Client/Focus/Inbox/Reminder、设置/保存视图、三类业务附件、Workspace Avatar、存储阈值、Submission origin、父任务 rollup guards、Project→Client Activity 唯一来源、Reminder 系列/occurrence、Automation Rule/Run、Agent Adapter 诊断、Client Followup 计划/终态事实与其他来源 guards                                                                                                                                                       |
...
| T-11 收件箱与工作编排中心            | 部分完成                         | T-11A1/A2/A3/B/C/F 已交付；A3 已含 schema v32 daily/weekly、schema v40 monthly 与 schema v41 weekdays Reminder、IANA/DST/月末推进、离线积压折叠和幂等下一 occurrence。拆分可继承/清除/改选可信来源 Project，写入独立完成条件并明确 person 本地责任，关系行可打开共享 Task；成功 mutation 会失效来源 Project，split 另失效 Task/Today/Project。T-11E 已交付既定来源和系统维护投影；法定节假日/自定义规则、原生通知、Agent 待实现                                                                                                                                   |
...
| T-19 本地 Agent 执行                 | v0.2 Windows 受控切片已交付      | schema 71 的 builtin Adapter/Actor/Run、管道文本执行、任务详情运行界面和条件式 manual-review 提交已实现；设置显式启停，任务启动/重试查询真实 Adapter、Actor、Assignment 和模型就绪状态，不自动初始化或静默改派。支持本地回环/在线 OpenAI 兼容模型；external、macOS/Linux、受控文件产出和 Inbox 联动仍待，执行器剩余差距见 ADR-027 与本地 Agent 模块                                                                                                                                                                                                               |
...
- **后续开发流程**：
  ...
  4. ADR-010 AI6 已完成，只允许用户显式选择的片段进入发送预览和模型上下文；保持无 knowledge tool 回归。
...
- **需求映射**：5.6、6、9.2。
- **版本归属**：v0.2 本地编排版的受控切片；当前应用版本仍为 0.1.1、API v1、schema 71。本轮初始化修复不新增迁移、不补种实际数据库、不自动启用 Agent。
- **当前状态**：v0.2-A Adapter 登记与安全诊断、v0.2-B Windows builtin Runner，以及任务详情 Run 界面和 v0.2-C 条件式文本验收提交已实现。[ADR-027](adr/027-builtin-agent-executor-and-run-lifecycle.md) 区分 builtin 与 external 信任：已验证 Windows builtin 可就绪，external 与未验证平台保持关闭。schema 071 已有 `agent_runs` 与 `actors.agent_adapter_id`；Run 提供创建/列表/详情/取消/重试、追加事件、文本查看和下载。执行器支持本地回环及显式选择的在线 OpenAI 兼容模型；不支持所有 Provider 协议。受控文件产出、Inbox 联动和 macOS/Linux 矩阵仍待。
...
- **任务启动流程**：启动前刷新 Adapter、Actor、Assignment、Provider、Run。仅 `todo/in_progress` Task，且 Adapter 为 enabled/healthy/execution_ready、真实 active agent Actor 的 `agent_adapter_id` 匹配时允许启动；缺失或读取失败禁用并引导设置。无 assignee 时先明确告知将分派，再通过现有 Assignment 领域命令及其 `task_version` 分派；已有其他 assignee 不静默改派。模型只展示 ready/healthy 的 `openai_chat` Provider；Sidecar 仍独立校验执行前置，固化快照并通过 `opc-agent-pipe-v1` 运行。
- **产出与验收**：Run 记录保存 ≤64 KiB 文本。成功后仅当 Task 满足 manual review、可提交状态、匹配 agent assignee 和 owner reviewer 等条件时，创建由 agent 提交的 `origin=manual` Submission、agent 产出/system 登记的 text Artifact，并进入 `waiting_review`；否则文本留在 Run，记录 `agent_run_submission_skipped`。owner 接受或返工，Run succeeded 不自动令 Task done。界面文本下载不是任意文件执行或受控文件产出链。
- **接口兼容与安全**：Actor 读取 API 增加可空只读 `agent_adapter_id`；前端把旧响应缺失字段归一化为 `null`，不能据固定 ID 或名称推断可执行。无任意 Shell、数据库直连或 WebView Bearer Token；模型凭据由 Sidecar keyring 经匿名管道进入子进程内存。删除、对外发送、付款确认等高风险动作不可委托。builtin 的受控模型网络访问不等于 OS 级全程禁网，external 仍须满足 ADR-003 全部闸门。
- **恢复与重试**：Sidecar 启动将遗留 queued/running Run 标记 `interrupted`；终态重试新建 Run，保留 `parent_run_id`、原输入快照和模型选择。界面重试同样限定 Task 状态并检查现有 Assignment，不为重试自动分派。取消已有 Run 不受环境未就绪阻断；停用 Adapter 不等于取消已有 Run；可能产生副作用的动作不静默重试。
...
- **剩余边界**：完成条件进入快照但尚未拼入执行器提示词；运行中取消的终态和超限文本处理尚有已知差距，本轮不扩展修复。后续 v0.2-C/D 还需受控文件/哈希链、Inbox 去重验收联动、macOS/Linux 生命周期验证及 external 安全闭环，详见 [本地 Agent 模块](modules/local-agents.md)。
- **关键路径与 ADR**：`services/sidecar/internal/database/migrations/071_agent_runs.sql`、`services/sidecar/internal/api/agent_adapters.go`、`services/sidecar/internal/api/agent_runs.go`、`services/sidecar/internal/agentexec/executor.go`、`apps/web/src/components/AgentAdapterSettings.tsx`、`apps/web/src/components/TaskAgentRunsSection.tsx`、[ADR-003](adr/003-local-agent-runtime-security.md)、[ADR-027](adr/027-builtin-agent-executor-and-run-lifecycle.md)。
...
| POST                 | /api/v1/ai/chat                           | 显式调用 AI 助手                                                                         | 已实现；SSE 返回 meta/delta/reasoning/replace/done/error/cancelled；AI5/AI6 上下文写前重验，ADR-011 在完成时剥离/校验 citation block 并保存五状态；稳定 Idempotency-Key 绑定输入摘要，已接受同键返回 `AI_CHAT_ALREADY_ACCEPTED` 与原 generation/session ID，异载荷复用键拒绝。无明确终态 EOF/截断不能当成功；生产无 knowledge tool                                                                                                                                 |
