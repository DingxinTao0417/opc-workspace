...
> **v9.79 更新说明**：真实浏览器 1280/820/620/480 px 验收发现设置菜单在窄屏横向滚动后可能把当前模块留在视口外；当前按钮现在会在打开、切换及导航尺寸变化时滚入最近可见位置。480 px 复测已确认“运行诊断”高亮完整可见，schema/API 不变。
...
| 系统托盘     | Tauri Tray API（`tray-icon` feature）        | 最小源码闭环完成：显示/隐藏/显式退出和安全降级已接；专注状态、快捷业务动作、设置开关及原生链接/三平台验收仍待                                     |
...
- **验证/剩余**：托盘源码单元测试覆盖固定菜单白名单、完成的左键点击门禁、fallback RGBA 尺寸/透明度、available 状态和能力快照；Web 覆盖严格能力解析、畸形降级和设置诊断展示。Cargo metadata/lock 与 Rust 格式通过。当前主机缺 MSVC `link.exe`/Windows SDK，`cargo check/test` 在依赖 build script 链接前受阻，因此不能宣称托盘原生编译或交互通过。既有 Go 覆盖运行锁、父管道 EOF 和恢复阶段，Web 覆盖四态/generation。T-02 仍部分完成：托盘专注状态/业务动作/设置开关、真实 Tauri/Sidecar 父崩溃、OS 进程树、三平台和安装包未验收；没有 Job Object、进程组或孙进程治理，hard-hung orphan 只被锁挡住且不自动回收；启动前备份选择仍待实现。
...
| 前端测试                                           | 通过         | 全量 90 个文件 / 604 项通过；新增桌面能力严格解析、畸形能力降级、设置托盘状态展示、活动设置模块滚入可视区与共享恢复 fixture 回归，既有内容日历/路线图 URL、目标日期排序、客户动态、启动进度、Agent Adapter、Automation、Reminder、内部回滚容量、业务与连接世代回归继续通过。组件测试不替代真实 WebView 验收 |
| Web 生产构建                                       | 通过         | `pnpm build:web` 的 TypeScript 与 Vite 构建通过；当前主入口 minified 938.30 kB、gzip 250.88 kB，仍有非阻断的 500 kB chunk 警告，后续按页面拆包                                                                                                                                                              |
| 前端格式                                           | 通过         | Web 范围 Prettier format check 通过                                                                                                                                                                                                                                                                         |
| Go 全量测试与静态检查                              | 通过         | `go test ./... -count=1` 与 `go vet ./...` 均通过；不虚构未统计的用例总数                                                                                                                                                                                                                                   |
| 统一源码质量门禁                                   | 通过         | 本轮完成源码门禁：Prettier、193 个 Go 文件与 Rust 格式、26 份 Markdown/本地链接、Web 类型/90 文件 604 项测试/构建、Go 无缓存测试/vet 与 x86_64-pc-windows-msvc Sidecar 构建                                                                                                                                 |
...
| 10   | 桌面与发布                | **部分完成**：generation-aware 两次有界重启、数据库运行锁、父管道 EOF、并发 shutdown、日志/request ID、全局恢复页、安全应用重启、固定命令面板/新建任务的原生快捷键注册/状态查询、托盘显示/隐藏/显式退出和不可用安全降级源码，以及白名单 `desktop_capabilities` 已交付；托盘原生链接/三平台交互、真实父崩溃/进程树、孤儿/孙进程治理、通知/自启/快捷键自定义/签名更新及三平台 CI 待实现                 | **部分完成**：starting/restarting/ready/error + generation、非 ready 连接/查询清理、状态重查、日志入口、安全重启、版本白名单、fixed shortcut 状态和托盘运行时能力诊断已交付；设置横向模块菜单已在 480 px 保持活动项可见；基础托盘交互不经 WebView，设置开关、专注状态/业务动作、启动前备份选择/恢复反馈与离线更新反馈待实现   | 本轮源码门禁已通过 Web 90 文件/604 测试、Go 无缓存全量测试/vet、文档、Sidecar 构建、Rust 格式与锁定 Cargo metadata。cargo check/test 受缺失 MSVC link.exe/Windows SDK 阻塞，托盘原生交互、真实父崩溃、进程树、签名、公证、干净机和三平台仍须验收                                                                                                                                     |
