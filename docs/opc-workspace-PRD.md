...
> **v9.75 更新说明**：路线图详情由 `?milestone=<id>` 驱动；Today 右侧每个临近节点现在直达指定详情，路线图卡片打开详情也写入同一 URL，关闭或进入编辑只清理 `milestone` 并保留其他查询参数。schema/API 保持不变。
...
| 前端测试                                           | 通过         | 全量 90 个文件 / 599 项通过；新增路线图 URL 指定详情、关闭参数清理/保留上下文及 Today 详情链接回归，既有目标日期排序、客户动态、路线图、内容日历、启动进度、Agent Adapter、Automation、Reminder、内部回滚容量、业务与连接世代回归继续通过。组件测试不替代真实 WebView 验收                   |
| Web 生产构建                                       | 通过         | `pnpm build:web` 的 TypeScript 与 Vite 构建通过；当前主入口 minified 934.89 kB、gzip 249.89 kB，仍有非阻断的 500 kB chunk 警告，后续按页面拆包                                                                                                                                               |
| 前端格式                                           | 通过         | Web 范围 Prettier format check 通过                                                                                                                                                                                                                                                          |
| Go 全量测试与静态检查                              | 通过         | `go test ./... -count=1` 与 `go vet ./...` 均通过；不虚构未统计的用例总数                                                                                                                                                                                                                    |
| 统一源码质量门禁                                   | 通过         | 本轮完成源码门禁：Prettier、193 个 Go 文件与 Rust 格式、26 份 Markdown/本地链接、Web 类型/90 文件 599 项测试/构建、Go 无缓存测试/vet 与 x86_64-pc-windows-msvc Sidecar 构建                                                                                                                  |
