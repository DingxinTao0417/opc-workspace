...
> **v9.25 更新说明**：交付 T-19 v0.2-A 的 Adapter 登记与安全诊断纵切。schema v34 新增空的 `agent_adapters`，只允许代码所有的 `builtin:*` 清单和版本化启停事实；API 支持列表、登记、详情、诊断、启用拒绝与停用，设置新增独立“本地 Agent”模块。当前诊断固定验证清单后返回 `PLATFORM_ISOLATION_UNVERIFIED`，保持 `execution_ready=false`；不会接收路径、启动子进程、创建 agent Actor/Assignment/Run 或执行任务。业务 JSON/ZIP 已纳入并严格校验 Adapter 身份与安全状态。app v0.1.0 / API v1 不变，SQLite 升至 schema v34。
...
| 前端测试                           | 通过         | 全量 83 个文件 / 535 项通过；新增 Agent Adapter API 严格契约与设置页登记/诊断交互覆盖，既有 Automation、Reminder、内部回滚容量、业务与连接世代回归继续通过。组件测试不替代真实 WebView/休眠或父崩溃验收                                                                                      |
| Web 生产构建                       | 通过         | `pnpm build:web` 的 TypeScript 与 Vite 构建通过；当前主入口 minified 845.41 kB、gzip 228.47 kB，仍有非阻断的 500 kB chunk 警告，后续按页面拆包                                                                                                                                               |
| 前端格式                           | 通过         | Web 范围 Prettier format check 通过                                                                                                                                                                                                                                                          |
| Go 全量测试与静态检查              | 通过         | `go test ./... -count=1` 与 `go vet ./...` 均通过；不虚构未统计的用例总数                                                                                                                                                                                                                    |
| 统一源码质量门禁                   | 通过         | 本轮以等价分项完成 `check:source` 全部门禁：Prettier、171 个 Go 文件与 Rust 格式、26 份 Markdown/本地链接、Web 类型/83 文件 535 项测试/构建、Go 无缓存测试/vet 与 x86_64-pc-windows-msvc Sidecar 构建                                                                                        |
