...
> 文档日期：2026-08-29
> 实现基线：app v0.1.0 / API v1 / SQLite schema v37

> **v9.52 更新说明**：内容日历 CC2 已交付版本化本地 API：创建、读取、时间范围/状态/平台/项目筛选、编辑、改期、手动发布确认和准备 Task 关联均通过 SQLite 事务与 `If-Match` 乐观锁保护；页面、拖拽、Inbox/调度仍待。无 AI/LLM、外发或 demo 数据。
...
> **当前状态**：路线图 `/roadmap` 已完成基础真实本地界面，R2 的 schema v36/API 提供 CRUD、筛选、项目关联、只读派生进度、乐观锁、归档恢复与重排；当前界面已接季度/状态浏览、新建、Project 深链和归档恢复，编辑、删除、拖拽与 Inbox/Reminder 自动化仍待。内容日历路由仍为明确的“后续版本”占位页；schema v37 与 CC2 API 已提供版本化 CRUD、范围筛选、改期、手动发布确认和准备 Task 关联，但无 React 业务交互、拖拽或 Inbox/调度。目标版本为 v0.3。
...
| 10   | 桌面与发布                | **部分完成**：generation-aware 两次有界重启、数据库运行锁、父管道 EOF、并发 shutdown、日志/request ID、全局恢复页、安全应用重启，以及固定命令面板/新建任务的原生快捷键注册与状态查询已交付；真实父崩溃/进程树、孤儿/孙进程治理、托盘/通知/自启/快捷键自定义/签名更新及三平台 CI 待实现 | **部分完成**：starting/restarting/ready/error + generation、非 ready 连接/查询清理、状态重查、日志入口、安全重启、版本白名单和 fixed shortcut registered/unavailable 状态已交付；备份选择/恢复进度、托盘与离线更新反馈待实现 | 本轮等价源码门禁已通过 Web 83 文件/535 测试、Go 无缓存全量测试/vet、文档和 Sidecar 构建。cargo check/test 受缺失 MSVC link.exe/Windows SDK 阻塞，真实父崩溃、进程树、签名、公证、干净机和三平台仍须验收 |
...
| v9.52    | 2026-08-29 | 内容日历 CC2：新增 `/api/v1/content-items` 的版本化本地 CRUD、时间范围/状态/平台/项目筛选、改期、手动发布确认、准备 Task 关联及测试；发布确认不访问外部平台，已发布条目暂不允许归档以保留发布时间事实。React 页面、拖拽、Inbox/调度仍待。 |
