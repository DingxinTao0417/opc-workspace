...
> 文档日期：2026-08-28
> 实现基线：app v0.1.0 / API v1 / SQLite schema v27

> **v7.1 更新说明**：继续交付 T-11E 系统维护来源。备份恢复演练的操作性失败或已验证包不可安全打开时尽力投影 `backup:drill`；恢复安排读取 pending/源目录/工作区身份、创建回滚点或发布计划失败时尽力投影 `backup:restore`。两类 incident 与 create/verify 共用 schema v26 的活动唯一约束和安全 payload，只保留固定 component/operation/failure_code/time/message，不记录备份 ID、路径、底层 error、Token、备注或请求正文；原 API 错误响应保持不变。`BACKUP_INVALID`、请求错误、包不存在、工作区不匹配和已有计划等可解释结果不投影。schema 保持 v27。
...
> **当前状态**：部分完成。T-11A1/A2/A3/B/C 已交付受理分诊、Reminder、Task 关系、拆分分派和自动结清；T-11F 已交付实时运营计数、Sidebar/Today 展示及 Inbox 风险深链筛选；schema v23–v25 已交付显式 follow-up Task Artifact、Task 阻塞与 Task 临期来源投影/删除协调；系统维护来源已交付备份创建、校验、恢复演练与恢复安排失败投影。迁移/Sidecar 启动等其他系统故障、重复/原生通知和 Agent Run 仍是规划。
...
- `system_maintenance`：schema v26 已建立通用系统维护约束，当前已接备份创建、校验、恢复演练和恢复安排四类操作性失败。Sidecar 分别尽力创建 `backup:create` / `backup:verify` / `backup:drill` / `backup:restore` 的 `kind=event` Inbox Item，payload 只含 component/operation/failure_code/occurred_at 和固定用户提示。同一 source id 仅允许一个 `open/tracking` incident；resolve/dismiss 后再次失败可创建新 incident。原错误码保持不变；包损坏/篡改返回 `BACKUP_INVALID`，请求错误、包不存在、工作区不匹配和已有计划等可解释结果均不投影 Inbox。迁移失败、Sidecar 启动前失败、数据库不可写和完整诊断包仍未实现。
...
| 收件箱与人工编排 | 本地 Actor 基础、事件受理、已读/稍后、任务拆分/关联、人工分派、验收/返工、审计和自动解决   | **部分完成**：schema v12–v15 已交付受理分诊、Reminder、Task 关系/拆分编排；T-11F 已交付 Sidebar/Today 运营计数与风险深链；schema v23–v25 已交付 follow-up Artifact、Task 阻塞与 Task 临期来源投影/删除协调；备份创建/校验/恢复演练/恢复安排失败已投影，迁移/启动等其他系统故障仍待实现 |
...
| 数据持久化       | Tauri `appDataDir`、SQLite 迁移、受控文件、手动/迁移前一致性备份、基础 JSON 导出与原子恢复 | **部分完成**：正式/开发隔离、WAL、外键、schema v27、受控 Task/Client/Project 文件与 Workspace Avatar、数据库身份强绑定、手动/迁移前备份恢复完整闭环、桌面安全重启及基础业务 JSON 已交付；恢复诊断、数据导入和含文件导出包仍待实现；每日计划和高级导入归 v0.3                           |
...
> 状态截止：2026-08-28。当前版本是可运行、可扩展的 v0.1 基座；T-18A/B/C/D D1/D2、T-12 Focus Core A+B+C、T-11A1/A2/A3/B/C/F、T-11E follow-up Artifact/Task 阻塞/Task 临期来源、备份创建/校验/恢复演练/恢复安排失败的系统维护投影、T-06A–H、T-07A–D 和 T-04B 一致性备份完整闭环/迁移前自动回滚包/桌面安全重启/基础业务 JSON 导出已经交付，但这不代表第 9.1 节的完整 MVP、Focus 高级分析、迁移/启动等其他系统故障来源、Agent、数据导入、含文件导出包或恢复诊断已经交付。
...
| API / Schema | API v1；SQLite schema v27                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 数据默认值   | 开发数据库默认空白，不自动注入 demo 业务数据                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 明确边界     | 当前代码不使用 Docker；已实现 Task 事实层、Actor/Assignment、D1 生命周期、Task 时间线、D2 manual Submission/Artifact/受控文件、Project 笔记/附件/产出聚合/活动时间线、Client 基础资料/Project 客户关联/人工活动/附件/person 关联、Focus Core A+B+C 与 D2b 日期范围回顾、手工 Inbox/Reminder/Task 编排、follow-up Artifact/Task 阻塞/Task 临期来源投影与删除协调、备份创建/校验/恢复演练/恢复安排失败的系统维护 Inbox 投影、基础备份闭环与业务 JSON 导出；未实现 Client 外部活动来源、Focus 高级分析/原生反馈、迁移/启动等其他系统故障来源、重复/原生通知、Agent、导入/含文件导出、AI 助手、知识库、客户回访或收入/支出/发票业务；person 只做本地责任记录，线上账号、云同步和远程协作均不在当前范围 |
...
| T-04 SQLite 初始化与迁移             | 已完成                           | schema v27、PRAGMA、嵌入迁移、demo 清理、Project/Task/Actor/Client/Focus/Inbox/Reminder、设置/保存视图、三类业务附件、Workspace Avatar 与来源 guards                                                                                                                                                                                                                     |
...
| T-11 收件箱与工作编排中心            | 部分完成                         | T-11A1/A2/A3/B/C/F 已交付；T-11E 已交付 follow-up Artifact、Task 阻塞、提前 24 小时 Task 临期，以及备份创建/校验/恢复演练/恢复安排失败的系统维护投影；迁移/启动等其他系统故障与 Agent 待实现                                                                                                                                                                             |
...
- **实现方法**：GORM 使用纯 Go SQLite 驱动和单物理连接，启用外键、WAL 与 5 秒 busy timeout。schema v1–v22 依次建立核心业务、Task/Actor/D2、Client、Focus、Inbox/Reminder、设置/保存视图、客户/项目扩展；schema v23–v26 新增四类 Inbox 来源 guards；schema v27 新增受控 Workspace Avatar、完整性/不可变墓碑、单 active 与跨四领域文件 ID 保护，均不改写既有模块事实或创建 demo 数据。不可变 `database_id`、一次性 `artifact_store_id` 与 marker 双向绑定整个受控根。开发数据库/受控文件位于 `.local/dev-data/`，生产数据位于 Tauri `appDataDir`。
...
- **需求映射**：5.6。
- **当前状态**：部分完成。schema v12–v15 已交付手工受理分诊、已有 Task 关系、一次性 Reminder 和 T-11C 拆分编排；T-11F 已交付实时运营计数、Today/Sidebar 展示和风险深链筛选；schema v23–v25 已交付显式 follow-up Task Artifact、Task 阻塞与提前 24 小时 Task 临期来源投影、稳定事件键、来源上下文与删除协调；系统维护已交付备份创建、校验、恢复演练与恢复安排失败投影。迁移/启动等其他系统故障、重复/原生通知和 Agent Run 仍未实现。
...
- **分阶段纵切**：
  ...
  7. **T-11E 事件源（部分完成）**：schema v23–v26 已接显式 follow-up Task Artifact、Task 阻塞、提前 24 小时 Task 临期，以及备份创建与校验失败；当前又复用系统维护契约交付备份恢复演练与恢复安排失败。临期扫描在 ready 前补偿并每 15 秒按截止时间/ID 处理最多 100 条，排除已投影 Task+截止时点以推进积压。后续继续接迁移/启动等其他系统故障；发票、客户回访和项目里程碑随对应业务模块启用。
...
- **验收要求**：当前纵切已覆盖纯离线手工受理、迁移保留、幂等/并发、事务回滚、全局未读、Task 关系/删除互锁、Reminder、follow-up Artifact、Task 阻塞、Task 临期和备份创建/校验/恢复演练/恢复安排失败唯一安全投影、重复阻塞、改期/100 条批次推进、Artifact/Task 来源删除协调、批量拆分全回滚、owner/person 分派、自动结清/重开与强制例外审计。完整编排仍需逐项验证迁移/启动等其他系统故障来源；Agent 成功不能跳过验收，返工必须保留 Run/产出。
...
4. **继续收件箱来源投影**：T-11A1/A2/A3/B/C/F 的手工受理分诊、已有 Task 关系、Reminder、批量拆分/Assignment、统一 reconciliation、自动解决和运营统计，以及 follow-up Artifact/Task 阻塞/Task 临期来源和备份创建/校验/恢复演练/恢复安排失败的系统维护投影均已交付；下一步按独立纵切接迁移/启动等其他系统故障来源投影。
...
| 5    | 收件箱人工编排            | **T-11A1/A2/A3/B/C/F 已交付**：schema v12–v15 的 Inbox/关系/Reminder/编排事实，manual 受理、Task 拆分分派、自动结清/重开和实时运营计数；schema v23–v26 已交付 follow-up Artifact、Task 阻塞、Task 临期及备份创建/校验失败，当前追加恢复演练/恢复安排失败；迁移/启动等其他系统故障仍待开发                                    | **已交付**：三视图、详情/分诊、Reminder、Task 关系/拆分、强制例外、风险筛选、Sidebar 徽标、Today 风险卡，以及已交付来源的上下文/图标                                     | 当前覆盖离线、迁移、幂等/并发、事务回滚、实时进度、Task 删除互锁、Reminder/follow-up/阻塞/临期/备份四类操作失败活动 incident 去重、拆分全回滚、自动结清/重开和计数口径；完整纵切仍需其他系统故障与来源删除可解释 |
...
| 8    | 设置与命令面板            | Actor API、health/version、schema v16 app_settings、schema v27 受控头像、Query committed、旧 localStorage 迁移、手动备份/恢复/业务导出及统一 search API 已接入                                                                                                                                                               | “人员与责任”支持 owner/person；个人资料支持名称与头像即时预览/保存/取消；设置支持 7 个左栏模块；命令面板支持多实体搜索和稳定详情路由                                     | 已覆盖设置/头像/备份/导出、搜索契约、IME、焦点恢复和 health；仍需真实 WebView/窄屏验收，通知、OS 快捷键和完整诊断仍为说明态                                                                                      |
...
- schema v12 已为非空 `source_event_key` 建立部分唯一约束；schema v14 Reminder 使用 `reminder:<id>:due`，schema v23 follow-up Artifact 使用 `task-artifact:<artifact-id>:followup`，schema v24 Task 阻塞使用 `task:<task-id>:blocked:<block-version>`，schema v25 Task 临期使用 `task:<task-id>:due:<due-at>`，schema v26 系统维护使用 `system:<component>:<operation>:<incident-id>` 且同一 `backup:create`、`backup:verify`、`backup:drill` 或 `backup:restore` 仅允许一个活动 incident。Artifact/阻塞分别与 Task 提交/block Event 同事务，临期来源与 system Event 同扫描事务，备份操作失败与调用失败响应解耦且尽力投影；已交付来源跨重试/扫描/重启不重复创建活动项，后续来源仍必须使用稳定键
