...
> **v8.1 更新说明**：Focus D2b 周期报告新增“周几 × 小时”二维热力图。API 在既有 completed-only interval 窗口内固定返回周一至周日、0–23 点共 168 格，与 24 小时分布共用 IANA/DST 安全的真实 UTC 流逝秒数归类；Focus 页展示可横向浏览的 7×24 强度矩阵和逐格分钟/块数提示。schema 保持 v28，标签分布与原生反馈仍待后续。
...
> **当前状态**：Focus Core A+B+C、D1、D2a，以及 D2b 日期范围回顾、项目时间分布、最佳小时段与二维热力图已完成。schema v11、Session API、任务绑定、绝对时间快照、刷新/启动恢复、15 秒心跳、暂停/继续/停止/取消、精确工时记账、Today 当地日统计、终态历史、7/30 天/本月/自定义趋势、Streak、项目/小时分布、周几×小时热力图和 Task 详情记录已接通；标签分布以及原生通知、托盘和勿扰集成未实现。
...
> **当前状态**：核心本地搜索、本地最近使用、设置运行诊断入口与全局渲染错误恢复已完成。支持已交付页面/安全操作命令、真实 Task/Project/Client/活动 Inbox 统一搜索、精确可刷新详情路由和完整键盘交互；空查询可展示有上限的非敏感最近命令/资源，并在确认资源 404 后清理。完整日志/诊断包、启动恢复页和 OS 全局快捷键仍待开发。
...
| 专注模式         | 番茄钟、环形进度、工时记录、连续天数统计、暂停本应用通知、系统专注模式引导                 | **Core A+B+C、D1、D2a 与 D2b 日期范围回顾/项目/小时/热力图已完成**：schema v11 Session/interval、任务绑定、绝对时间、心跳/恢复、并发/幂等、精确工时、Today 汇总、终态历史、7/30 天/本月/自定义趋势、Streak、项目/小时分布、周几×小时热力图与 Task 详情记录已实现；标签分布及原生通知/托盘/DND 待实现                 |
...
> 状态截止：2026-08-28。当前版本是可运行、可扩展的 v0.1 基座；T-18A/B/C/D D1/D2、T-12 Focus Core A+B+C/D1/D2a/D2b 项目/小时分布与二维热力图、T-11A1/A2/A3/B/C/F、T-11E follow-up Artifact/Task 阻塞/Task 临期/Project 完成来源、备份四类操作失败、数据库启动/迁移和 Sidecar 启动失败投影、T-06A–H、T-07A–D 和 T-04B 一致性备份完整闭环/迁移前自动回滚包/桌面安全重启/基础业务 JSON 导出已经交付，但这不代表第 9.1 节的完整 MVP、Focus 标签分布/原生反馈、运行期数据库等其他系统故障来源、完整日志/恢复页、Agent、数据导入、含文件导出包或恢复诊断已经交付。
...
| 明确边界     | 当前代码不使用 Docker；已实现 Task 事实层、Actor/Assignment、D1 生命周期、Task 时间线、D2 manual Submission/Artifact/受控文件、Project 笔记/附件/产出聚合/活动时间线、Client 基础资料/Project 客户关联/人工活动/附件/person 关联、Focus Core A+B+C 与 D2b 日期范围回顾/项目与小时分布/二维热力图、手工 Inbox/Reminder/Task 编排、follow-up Artifact/Task 阻塞/Task 临期来源投影与删除协调、备份四类操作失败、数据库启动/迁移和 Sidecar 启动失败的系统维护 Inbox 投影、基础备份闭环与业务 JSON 导出；未实现 Client 外部活动来源、Focus 标签分布/原生反馈、运行期数据库等其他系统故障来源、完整日志/恢复页、重复/原生通知、Agent、导入/含文件导出、AI 助手、知识库、客户回访或收入/支出/发票业务；person 只做本地责任记录，线上账号、云同步和远程协作均不在当前范围 |
...
| T-12 专注设置与全局计时              | Core+D1+D2a+D2b 热力图完成       | A+B+C：schema v11 Session/interval、任务绑定、绝对时间、心跳/恢复、状态命令、幂等/并发、精确工时与 Today 汇总；D1/D2b：终态历史分页、7/30 天/本月/最多 93 天自定义本地日趋势、Streak、项目分布、DST 安全小时分布/最佳时段与周几×小时二维热力图；D2a：Task 详情按需专注记录已交付；标签分布及通知/托盘/DND 延后                                                           |
| T-13 命令面板与基础反馈              | 核心搜索、最近使用与恢复反馈完成 | WebView 快捷键、已交付页面命令、Task/Project/Client/活动 Inbox 统一本地搜索、稳定详情路由、设置模块直达、combobox/listbox、焦点圈闭/恢复、IME 保护、加载/错误/重试/空状态、非敏感最近使用/404 资源清理、脱敏运行诊断与全局渲染错误恢复；完整日志/诊断包和 OS 全局快捷键待后续                                                                                            |
...
- **需求映射**：5.7。
- **完成范围**：Core A 事实迁移、Core B API/状态机/事务、Core C 前端接入与恢复、D1 终态历史/七日报告、D2a Task 详情记录，以及 D2b 7/30 天、本月、自定义日期范围回顾、项目时间分布、DST 安全小时分布/最佳时段与周几×小时二维热力图已完成；标签分布和系统集成延后。Today 与周期 completed-only 统计属于已交付闭环。
...
- **验证/当前限制**：自动化测试覆盖迁移、状态机、绝对时间、心跳、恢复、并发创建/停止、幂等重放、事务回滚、跨 Session 余秒、Task/Project 版本、历史筛选/分页、Task 详情按需读取/翻页/错误隔离，以及 IANA/DST/跨午夜 completed-only 日与周期统计、Streak、项目/未归项目分布、小时/二维热力图和秋季重复小时合并。Focus 参数已从 SQLite committed 读取；标签分布、原生通知、托盘和系统勿扰仍未实现，真实三平台后台/睡眠场景仍需桌面验收。
...
- **验证/当前限制**：Go 契约测试覆盖四类型结果、归档/终态过滤、类型去重、稳定分页、文字 `%/_`、非法查询和只读空库；前端覆盖 strict 契约、防抖、错误重试、精确导航及 Task/Inbox 直达路由、最近使用的容量/去重/过期/非敏感存储/404 清理、Sidecar 状态白名单规范化与诊断模块，以及全局错误边界的脱敏、重试、诊断直达和路由恢复。当前不是 FTS/模糊搜索，没有高亮、桌面启动恢复页、完整日志/诊断包或 OS 全局快捷键；真实浏览器和桌面 WebView 焦点/视觉仍需人工验收。
...
5. **扩展 Focus D2b**：Core A+B+C、D1、D2a 与日期范围回顾的持久化、恢复、精确工时、Today 统计、终态历史、7/30 天/本月/自定义趋势、Streak、项目时间分布、DST 安全小时分布/最佳时段、周几×小时二维热力图和 Task 详情记录已交付；后续独立实现标签分布和经平台验收的原生通知、托盘、DND 引导。
...
| 8    | 设置与命令面板            | Actor API、health/version、Tauri Sidecar 生命周期、schema v16 app_settings、schema v27 受控头像、Query committed、旧 localStorage 迁移、手动备份/恢复/业务导出及统一 search API 已接入                                                                                                                                       | “人员与责任”支持 owner/person；个人资料支持名称与头像即时预览/保存/取消；设置支持 8 个左栏模块，运行诊断可重查并复制脱敏摘要；命令面板支持多实体搜索和稳定详情路由       | 已覆盖设置/头像/备份/导出、搜索契约、IME、焦点恢复、health 与桌面状态脱敏；仍需真实 WebView/窄屏验收，通知、OS 快捷键、完整日志/诊断包和恢复页仍为后续                                                                      |
