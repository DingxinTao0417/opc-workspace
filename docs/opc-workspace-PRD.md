...
> **v9.21 更新说明**：完成内部自动回滚包容量准入。破坏性迁移、业务 JSON/含文件 ZIP 导入与恢复安排在任何备份 staging、`VACUUM INTO` 或业务变化前复用手工备份的安全估算和 backup root 单点探测；恢复按当前工作区回滚包、目标 pending 副本、双 manifest 与 plan 上界合计计算 20% 且最低 64 MiB 余量。导入和恢复分别以专用脱敏 507/503 拒绝，迁移在 destructive SQL 前安全终止；拒绝不创建回滚包、不改业务事实、不投影 generic 备份故障。设置页补齐四类可操作中文反馈。app v0.1.0 / API v1 / SQLite schema v31 不变，无 migration。
...
| 前端测试                           | 通过         | 全量 79 个文件 / 528 项通过；本轮新增内部回滚容量 507/503 中文反馈，既有 restarting/generation、业务与连接世代回归继续通过。组件测试不替代真实 WebView/父崩溃验收                                                                                        |
...
| 统一源码质量门禁                   | 通过         | `pnpm check:source` 已通过：Prettier、159 个 Go 文件和 Rust 格式、25 个 Markdown 文件/本地链接、Web 类型/测试/构建、Go 无缓存测试/vet 与当前平台 Sidecar 构建均纳入同一命令                                                                              |
