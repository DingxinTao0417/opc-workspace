...
> 文档日期：2026-08-29
> 实现基线：app v0.1.0 / API v1 / SQLite schema v33

> **v9.24 更新说明**：接受 T-19 本地 Agent Runtime 安全与传输 ADR。首版选择每个 Run 一个短生命周期子进程，以 Sidecar 创建的匿名 stdin/stdout 管道作为唯一能力通道；不开放 Runtime HTTP、不复用 WebView Token、不接受任意 Shell/命令/绝对路径，输入只使用业务资源 ID，输出只进入单次受控 staging。Windows/macOS/Linux 的进程沙箱、禁网和子树清理在各自验证前必须保持 `execution_ready=false`，不得创建可分派 agent Actor 或启动 Run。当前 app v0.1.0 / API v1 / schema v33 不变，Adapter 与 Run 代码仍未实现。
...
- `reminder_due`：用户创建的一次性或 daily/weekly Reminder occurrence 到期后生成收件箱项；Reminder occurrence 是调度事实，Inbox Item 是到期后的处理事实。重复系列在同一事务按 IANA 当地日历创建唯一下一 occurrence。
...
**reminders** - 本地提醒调度（schema v14 + schema v32 / T-11A3 已实现）

字段包括 `id`、`source_entity_type / source_entity_id`、`title / summary / priority`、`trigger_at`、`status`、`source_event_key`、`created_by_actor_id`、`series_id`、`recurrence_type / recurrence_interval / recurrence_timezone`、`occurrence_number`、`fired_at / inbox_item_id`、`cancelled_by_actor_id / cancelled_at / cancel_reason`、`version` 和创建/更新时间。公开创建固定 `source_entity_type=manual` 且来源 ID 为空；状态为 `scheduled / fired / cancelled`。一次性规则规范为 `none / 1 / UTC`；重复规则只接受 `daily / weekly`、1–365 间隔和稳定 IANA 时区。`series_id + occurrence_number` 唯一且同一系列最多一个 scheduled occurrence；系列身份与 occurrence 序号不可修改。scheduled 不携带任一终态事实；fired 必须成组记录触发时间和来源/事件键均匹配的 Reminder Inbox Item；cancelled 必须成组记录 owner、时间和非空原因。fired/cancelled 终态不可变，记录不允许硬删除。

创建时生成唯一稳定键 `reminder:<id>:due`。创建、编辑/改期、取消分别写 `reminder_created / reminder_updated / reminder_cancelled` owner Event；Sidecar 到期投影写 system Inbox `created` Event 和 `reminder_fired` Event，重复规则再为新 occurrence 写 `reminder_recurrence_scheduled` Event。启动先补扫，运行中每 15 秒按 `trigger_at/id` 扫描最多 100 条；每个 occurrence 的 Inbox 创建/复用、事件、fired 更新和下一 occurrence 在同一事务提交，稳定键、系列部分唯一索引与条件更新使重复扫描和重启不会重复生成。下一时刻按 IANA 当地日历推进，跨 DST 保持当地钟点；离线漏过多个周期只补当前一条并把 occurrence 序号推进到首个未来时刻。应用关闭时不后台唤醒；每月/自定义规则和系统原生通知留待后续。
...
schema v31 同样是普通非破坏性迁移：只为 `client_activities` 的 Project Workflow Event 系统来源增加部分唯一索引，不改现有行、不增加 `source_id` 外键，也不回填历史 Project 事件。schema v32 继续以普通加法列、索引和 trigger 扩展 Reminder；既有行只回填 `series_id=id / none / 1 / UTC / occurrence=1`，不改变标题、状态、触发时间、Inbox 引用或事件。schema v33 只新增空的 `automation_rules/automation_runs` 及身份、形状、去重、重试和不可变约束；迁移不插入预设或 Run，Sidecar 启动幂等登记代码所有的默认禁用规则。后续迁移从 `034_*` 继续。
...
| 收件箱与人工编排 | 本地 Actor 基础、事件受理、已读/稍后、任务拆分/关联、人工分派、验收/返工、审计和自动解决       | **部分完成**：schema v12–v15 已交付受理分诊、Reminder、Task 关系/拆分编排；T-11F 已交付 Sidebar/Today 运营计数与风险深链；schema v23–v25 已交付 follow-up Artifact、Task 阻塞与 Task 临期来源投影/删除协调；备份四类操作性失败、数据库启动/迁移、Sidecar 启动、运行期数据库操作失败和可配置低空间监测已安全投影，手工与内部自动回滚包的容量准入拒绝明确不投影 generic incident；物理卷同卷去重及无路径手动容量检查已在设置页交付，卷级趋势仍待实现 |
...
| 数据持久化       | Tauri `appDataDir`、SQLite 迁移、受控文件、手动/迁移前一致性备份、基础 JSON 导入导出与原子恢复 | **部分完成**：正式/开发隔离、WAL、外键、schema v33、受控文件、版本化设置、数据库身份强绑定、备份/自动回滚包容量准入、恢复诊断，以及业务 JSON/含文件 ZIP 的空工作区同 schema 安全导入导出已交付；v32 扩展 Reminder，v33 新增空 Automation Rule/Run 表并由 Sidecar 幂等登记默认禁用预设，迁移均不造业务 Run；数据库打开前实时进度页和高级冲突导入仍待后续                                                                                            |
...
> 状态截止：2026-08-29。当前版本是可运行、可扩展的 v0.1 基座，并已前置交付 v0.2 首个受限预设自动化纵切。schema v33、Project 完成→本地核对 Inbox、daily/weekly 本地 Reminder、设置预览/保存/启停、Run 历史/重试、IANA/DST、离线折叠和导入导出已经接通；这不代表完整 v0.2、Agent Runtime、发票/客户回访、自由规则、Task 自动化动作、Focus 原生反馈、复杂通知、数据库打开前恢复进度或冲突合并导入已经交付。
...
| API / Schema | API v1；SQLite schema v33                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
迁移文件位于 `services/sidecar/internal/database/migrations/`，通过 Go `embed` 编入 Sidecar。当前 schema 为 v33，新增结构只能从 `034_*` 继续追加递增文件：
...
当前 `001_initial_schema.sql` 建立核心业务表和索引；002 清理旧固定 demo；003–010 依次增加 Project、Task、Actor/Assignment/Event、Submission/Artifact 与 Client 事实；011–014 依次交付 Focus Session、手工 Inbox、Inbox–Task 关系和一次性 Reminder；015–031 交付编排、设置、客户/项目扩展、受控文件与来源 guards；032 非破坏性增加 Reminder 系列、daily/weekly、IANA 时区和 occurrence 约束；033 非破坏性增加空的 Automation Rule/Run 表、稳定身份、事件/计划形状、去重、重试、不可变和因果深度约束，不写 demo/预设/Run。迁移文件头可声明连续组合的 `-- migration: foreign_keys=off` 与 `-- migration: destructive`；迁移器在固定 `sql.Conn` 上于事务外关闭外键，事务内执行 SQL 与 `foreign_key_check`，提交/回滚后恢复外键，恢复失败与原错误一并返回；破坏性迁移还要求先生成并验证回滚包。数据库使用单物理连接，并启用外键、WAL 和 5000 ms `busy_timeout`；退出时执行 `wal_checkpoint(TRUNCATE)`。
...
| T-04 SQLite 初始化与迁移             | 已完成                           | schema v33、PRAGMA、嵌入迁移、demo 清理、Project/Task/Actor/Client/Focus/Inbox/Reminder、设置/保存视图、三类业务附件、Workspace Avatar、存储阈值、Submission origin、父任务 rollup guards、Project→Client Activity 唯一来源、Reminder 系列/occurrence、Automation Rule/Run 与其他来源 guards                                                                                                                                                                                      |
| T-04B 一致性备份                     | 基础安全闭环完成                 | 专用 backup root、维护写锁、SQLite `VACUUM INTO`、全部 active 受控文件+marker+manifest、完整校验、同卷原子发布、创建幂等、手工及迁移/恢复/导入自动回滚包低空间准入、列表/重新校验、隔离演练、pending/applied 提交点、重启原子恢复、桌面安全重启、启动后恢复结果诊断、确认删除、白名单业务 JSON 与含文件 ZIP 的空工作区安全导入导出、设置 UI、全局启动故障恢复页 v1，以及备份操作性失败安全 Inbox 投影已交付。非空目标/跨 schema 合并与数据库打开前备份选择/实时恢复进度待后续纵切 |
...
| T-11 收件箱与工作编排中心            | 部分完成                         | T-11A1/A2/A3/B/C/F 已交付；A3 已含 schema v32 daily/weekly Reminder、IANA/DST 推进、离线积压折叠和幂等下一 occurrence。拆分可继承/清除/改选可信来源 Project，写入独立完成条件并明确 person 本地责任，关系行可打开共享 Task；成功 mutation 会失效来源 Project，split 另失效 Task/Today/Project。T-11E 已交付既定来源和系统维护投影；复杂 Reminder/原生通知、卷级趋势与 Agent 待实现                                                                                                |
...
| T-19 本地 Agent 执行                 | 前置 ADR 完成                    | ADR-003 已冻结单次匿名管道、无 Runtime HTTP/WebView Token、资源 ID/单次 staging、能力枚举和隔离未验证即禁用；Adapter schema/API/UI、Run、产出、取消/重试、验收/返工和崩溃恢复仍待实现，属于 v0.2                                                                                                                                                                                                                                                                                  |
...
- **实现方法**：GORM 使用纯 Go SQLite 驱动和单物理连接，启用外键、WAL 与 5 秒 busy timeout。schema v1–v22 依次建立核心业务、Task/Actor/D2、Client、Focus、Inbox/Reminder、设置/保存视图、客户/项目扩展；schema v23–v26 新增来源 guards；schema v27 新增受控 Workspace Avatar；schema v28 新增 Project 完成节点来源和删除协调；schema v29 在破坏性迁移闸门和回滚包后重建 app_settings 约束以增加 storage key；schema v30 非破坏性新增 Submission origin 和 child_rollup guards；schema v31 增加 Project Workflow Event 客户动态来源唯一约束；schema v32 为 Reminder 追加系列、daily/weekly、IANA 时区和 occurrence 约束；schema v33 新增空的 Automation Rule/Run 事实表，预设由 Sidecar 幂等登记且默认禁用。迁移不创建 demo 数据。不可变 `database_id`、一次性 `artifact_store_id` 与 marker 双向绑定整个受控根。开发数据库/受控文件位于 `.local/dev-data/`，生产数据位于 Tauri `appDataDir`。
...
- **需求映射**：5.6。
- **当前状态**：部分完成。schema v12–v15 已交付手工受理分诊、已有 Task 关系、一次性 Reminder 和 T-11C 拆分编排；schema v32 已把 Reminder 扩展为 daily/weekly 系列与独立 occurrence，支持 IANA/DST 当地钟点、离线积压折叠、幂等下一 occurrence 和系列停止。T-11F 已交付实时运营计数、Today/Sidebar 展示和风险深链筛选；schema v23–v25 已交付显式 follow-up Task Artifact、Task 阻塞与提前 24 小时 Task 临期来源投影、稳定事件键、来源上下文与删除协调。系统维护已交付备份四类操作失败、数据库启动/迁移、Sidecar 启动、运行期数据库操作失败、可配置低空间投影、物理卷同卷去重和无路径手动容量检查。卷级趋势、复杂 Reminder/原生通知和 Agent Run 仍未实现；v0.1 不启用 AI/LLM/Agent。
...
- **分阶段纵切**：
  ...
  4. **T-11A3 Reminder（已完成）**：schema v14 一次性 `reminders` + schema v32 daily/weekly 系列/occurrence；创建/分页查询/详情、scheduled 编辑/改期/规则切换、带原因停止系列、ETag/幂等、启动补偿、15 秒扫描、IANA/DST 推进、离线积压折叠和稳定事件键 Inbox/下一 occurrence 投影。
...
- **实现方法**：schema v11 以 `focus_sessions`、`focus_session_intervals`、`task_focus_totals` 保存事实。API 快照统一返回 `session/server_now/elapsed_seconds/remaining_seconds` 并为 Session 返回 ETag；现有 Session 命令强制 `If-Match`，create/stop/cancel 支持 `Idempotency-Key`。Sidecar 启动把 active 转为 recovery_pending，每 15 秒刷新 heartbeat 且不递增 version。stop→completed 在一个事务内结算 interval、累计精确秒数、只把新增完整分钟写入 Task并每次递增 Task version；只有 `actual_minutes` 实际增加时既有 trigger 才递增 Project 聚合版本。cancel/interrupted 不入账。终态历史按 `ended_at DESC, id ASC` 分页；周期统计按显式 IANA 时区将 completed 的闭合正时长 interval overlap 切到 1–93 个本地日，并派生 distinct Session、秒/分钟和 Streak。历史与周期接口可选 canonical UUID `project_id`，不存在 404、归档可读，并按 Task 查询时当前 Project 归属分类；无 Task/Task 已删除/当前无项目不进入项目过滤。前端 TanStack Query 共享活动、历史与报告快照，并以 Project/日期/页码隔离 key；本地 persist store 只保存 work/break/cycle 编排与绝对休息截止时间。该 Focus 扩展没有新增自己的迁移；当前整体基线为 API v1/schema v33。
...
- **当前状态**：前置 ADR 已完成，代码未开始；没有 Agent Adapter、Run、单次管道能力、产出或恢复逻辑。ADR-003 已决定 Runtime 不开放 HTTP，每个 Run 使用 Sidecar 创建的短生命周期子进程和匿名 stdin/stdout 管道；平台沙箱、禁网与进程树清理未验证时执行保持关闭。
...
- **ADR 证据**：[ADR-003：本地 Agent Runtime 安全与传输边界](adr/003-local-agent-runtime-security.md)。
...
| schema v29→v30 迁移                | 通过         | 定向数据库测试覆盖既有 Submission 保留并默认 manual、origin/system/non-inferred/no-Artifact/历史不可变 guards、无 destructive gate、迁移失败回滚与外键检查；最终工作区继续迁移到最新 schema=33                                                           |
| schema v32→v33 / 预设自动化        | 通过         | 迁移不插入业务 Rule/Run；身份、事件/计划二选一、attempt/重试、去重、不可变和因果深度约束通过。Go 覆盖五预设、preview/启停、Project 事件 exactly-once、IANA/DST 与离线折叠、失败/重试/来源隔离、导入导出；Web 覆盖严格客户端契约和设置预览/保存/启用。    |
...
| schema v30→v31 迁移                | 通过         | 定向数据库测试覆盖 v30 事实与 Client version 保留、不回填历史 Project Event、`project_workflow_event` 部分唯一索引及其他来源豁免、无新增 source FK、无 destructive gate、回滚、外键检查及最终迁移到最新 schema=33                                        |
...
| schema v31→v32 与重复 Reminder     | 通过         | 定向数据库/API 测试覆盖既有一次性事实保留、系列与 occurrence 唯一约束、daily/weekly 创建编辑、IANA 时区、DST 当地钟点、离线跳过积压、重复扫描幂等及导入预检；最终继续迁移至 schema=33                                                                    |
...
| 前端测试                           | 通过         | 全量 81 个文件 / 532 项通过；新增 Automation API 严格契约与设置页预览、保存、启用交互覆盖，既有 Reminder、内部回滚容量、业务与连接世代回归继续通过。组件测试不替代真实 WebView/休眠或父崩溃验收                                                          |
| Web 生产构建                       | 通过         | `pnpm build:web` 的 TypeScript 与 Vite 构建通过；当前主入口 minified 837.36 kB、gzip 226.44 kB，仍有非阻断的 500 kB chunk 警告，后续按页面拆包                                                                                                           |
| 前端格式                           | 通过         | Web 范围 Prettier format check 通过                                                                                                                                                                                                                      |
| Go 全量测试与静态检查              | 通过         | `go test ./... -count=1` 与 `go vet ./...` 均通过；不虚构未统计的用例总数                                                                                                                                                                                |
| 统一源码质量门禁                   | 通过         | 本轮 `pnpm check:source` 通过：覆盖 Prettier、167 个 Go 文件与 Rust 格式、25 份 Markdown/本地链接、Web 类型/81 文件 532 项测试/构建、Go 无缓存测试/vet 与 x86_64-pc-windows-msvc Sidecar 构建                                                            |
| 文档链接/机器路径/冲突标记         | 通过         | `pnpm check:docs` 已检查根 README 与 `docs/**/*.md` 共 25 份 Markdown 的本地链接、机器路径和冲突标记；Prettier 与 `git diff --check` 通过                                                                                                                |
...
9. **扩展本地规划与预设自动化**：预设自动化首个纵切已交付 Project 完成 Inbox 与 daily/weekly Reminder；下一步完成真实休眠/WebView 专项，并在发票/Agent 事实上线后分别启用对应预设。路线图、内容日历、Task 动作和自由规则仍待后续，任何规则都不得自动对外发送。
...
| 8    | 设置与命令面板            | Actor API、health/version、Tauri Sidecar 生命周期、schema v16 app_settings、schema v27 受控头像、Query committed、旧 localStorage 迁移、手动备份/恢复/业务导出、诊断包、Sidecar/Tauri 壳轮转日志、打开日志目录、WebView→Sidecar request ID、全局启动故障恢复页 v1、schema v33 自动化 API 及统一 search API 已接入                                                                                     | “人员与责任”支持 owner/person；个人资料支持名称与头像即时预览/保存/取消；设置支持 9 个左栏模块，自动化在模块内独立预览/保存/启停/重试，运行诊断可重查、复制脱敏摘要、下载诊断包并打开日志目录；命令面板支持多实体搜索和稳定详情路由           | 已覆盖设置/头像/备份/导出、自动化、诊断包白名单与隐私、双进程日志、request ID、启动门禁、搜索契约、IME、焦点恢复、health 与桌面状态脱敏；仍需真实 WebView/窄屏验收，通知、OS 快捷键和数据库打开前恢复进度仍为后续                                                                                                                    |
...
| 10   | 桌面与发布                | **部分完成**：generation-aware 两次有界重启、数据库运行锁、父管道 EOF、并发 shutdown、日志/request ID、全局恢复页与安全应用重启已交付；真实父崩溃/进程树、孤儿/孙进程治理、托盘/通知/快捷键/自启/签名更新及三平台 CI 待实现                                                                                                                                                                           | **部分完成**：starting/restarting/ready/error + generation、非 ready 连接/查询清理、状态重查、日志入口、安全重启和版本白名单已交付；备份选择/恢复进度、托盘与离线更新反馈待实现                                                               | `pnpm check:source` 已通过 Web 81 文件/532 测试、Go 无缓存全量测试/vet、文档和 Sidecar 构建。cargo check/test 受缺失 MSVC link.exe/Windows SDK 阻塞，真实父崩溃、进程树、签名、公证、干净机和三平台仍须验收                                                                                                                          |
| 11   | 本地 Agent（v0.2）        | **ADR-003 已完成**：单次匿名管道、无 Runtime HTTP/WebView Token、能力/资源/staging 和隔离硬闸门已冻结；Adapter schema/API/UI、平台探针、Agent Run、取消/重试/中断恢复仍待实现                                                                                                                                                                                                                         | 下一步只展示 Adapter 诊断与缺失闸门；只有健康且隔离已验证的 Agent 才能进入分派/启动/运行/输出/失败/重试/待验收/返工                                                                                                                           | ADR 文档门禁先行；后续必须验证无任意 Shell/数据库/目录、禁网失败即禁用、成功只进 waiting_review、产出校验和历史完整                                                                                                                                                                                                                  |
