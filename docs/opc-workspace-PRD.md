...
> **v9.39 更新说明**：负责人存在任一 `planned` 客户回访时，Actor 停用 API 返回冲突；用户必须先改派、完成、跳过或取消该计划。终态回访只保留本地历史，不阻止停用，也不自动改派、完成或外发消息。无新 migration、无 AI/LLM。app v0.1.0 / API v1 / schema v35 不变。
