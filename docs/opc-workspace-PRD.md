...
> **v9.63 更新说明**：路线图 R4 首个安全排序纵切已交付：只对 2–100 条完整未归档季度集合开放，支持卡片拖拽与键盘可达的上移/下移预览，显式保存会携带所有 `expected_version` 原子重排；筛选、归档、超过容量或列表未完整读取时不会局部保存，失败恢复服务端顺序。目标日期/跨季度拖拽仍待，schema 仍为 v38，无 AI/LLM、外发或 demo 数据。
...
> **当前状态**：路线图 `/roadmap` 的 R3 基础界面已完成，R4 已交付完整同季度集合的安全卡片排序；R2 的 schema v36/API 提供 CRUD、筛选、项目关联、只读派生进度、乐观锁、归档恢复与重排。当前界面已接季度/状态/Project 浏览、稳定分页、新建、基础事实编辑、按 ID 详情、Project 深链、归档恢复、保护性永久删除，以及 2–100 条完整未归档季度集合的拖拽/键盘顺序预览与原子保存；目标日期/跨季度拖拽和 Inbox/Reminder 自动化仍待。内容日历已交付 schema v37 数据/API、六周月格、新建/编辑、IANA/DST 安全改期、人工发布确认、Task 关联及 schema v38 Inbox 来源契约，并完成审核/发布到期的启动/周期扫描、版本化去重、变更终结、删除协调、Inbox 来源详情与六周范围自动分页聚合。目标版本为 v0.3。
