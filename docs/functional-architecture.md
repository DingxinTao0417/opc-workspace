...
> 文档版本：2.72
> 日期：2026-08-29
> 依据：[PRD v9.61](opc-workspace-PRD.md)
> 当前实现基线：app v0.1.0 / API v1 / SQLite schema v38

> 2.72 说明：路线图 R3 已接入共享的服务端搜索 Project 选择器与 20 条/页稳定分页；筛选切换复位页码、总数收缩夹回末页，后台换页保留已读事实并显示刷新反馈。
...
| [路线图](modules/roadmap.md)               | Project/Task 派生进度                                                                                                   | 已交付季度里程碑数据/API、项目关联、只读进度、服务端 Project 筛选/分页以及新建/编辑/归档恢复/保护性删除；详情、拖拽与本地事件待后续                                                       | 当前无 Inbox 事件；后续临期/达成投影                                                                                                                             |
