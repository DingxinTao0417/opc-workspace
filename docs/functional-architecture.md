...
> 文档版本：1.8
> 日期：2026-08-28
> 依据：[PRD v2.5](opc-workspace-PRD.md)
> 当前实现基线：app v0.1.0 / API v1 / SQLite schema v13
...
- Go 已提供健康检查、Task/Project/Client/Actor/Assignment、D1/D2、Focus Session、手工 Inbox 受理/分诊、已有 Task 关系和 Today 统计 API；Focus、Inbox 单条命令与关系写入使用 `If-Match`、幂等快照和事务维护事实。
- SQLite 当前为 schema v13：schema v11 以 `011_focus_sessions.sql` 重建 `focus_sessions` 并新增 `focus_session_intervals`、`task_focus_totals`；schema v12 以 `012_inbox_items.sql` 加法迁移新增手工 Inbox Item；schema v13 再以 `013_inbox_item_tasks.sql` 新增活动/历史 Task 关系、软解除、删除快照与 Task 删除保护，不改写 v12 既有事实，也不改变其他模块结构。
...
- T-11A1/T-11B 已交付手工 Inbox Item 创建、三视图列表、详情编辑、单条/快照式全部已读、稍后/恢复、带原因解决/忽略、重开和 Inbox Event 时间线；T-11A2 已交付已有 Task 活动/历史关系、服务端实时进度、required 修改、带原因软解除、`open / tracking` 联动、按活动关系重开、关系事件和 Task 删除互锁。关系操作不改变 Task version 或生命周期。
- 当前仍未实现 Focus D（历史、周报、Streak、高级分析、原生通知/托盘/DND）、Client 活动/附件/回访/财务、项目附件与来源投影、Reminder、Inbox Task 批量拆分/分派/自动解决、Sidebar/Today Inbox 计数、Agent Runtime 和产品化备份/恢复，因此完整工作编排仍是部分完成。

### 3.2 目标扩展

- v0.1：在已交付的 Task/Project/Client、Actor/Assignment、D2、Focus Core、手工 Inbox 受理/分诊和已有 Task 关系基础上，继续完成客户/项目增强、Inbox 批量拆分/分派/自动解决、Reminder/来源投影、基础备份恢复和桌面可靠性；Focus D 独立延后。
...
| [收件箱](modules/inbox.md)                 | 当前为 owner 手工录入及已有 Task；项目/Reminder/Agent/系统投影待开发 | 已交付手工受理/分诊、已有 Task 活动/历史关系、实时进度、required、软解除、状态联动和事件；批量拆分、分派与自动解决待开发 | 输出 Inbox/关系 Event 与 Task 只读派生进度；Assignment 和今日待处理统计待后续接通       |
...
当前 Task、Tag、Project、Client 与 person Actor 创建已实现该原则的基础版本。Actor/Assignment、Task 生命周期、D2 命令、Focus create/stop/cancel，以及手工 Inbox 创建/单条命令/read-all/Task 关系写入都保存请求摘要与首次响应；Focus 与 Inbox 命令在版本检查前安全重放同键同请求，不重复结算、改写事实或写事件。Inbox 事实 PATCH 仍只使用 `If-Match`，不保存幂等快照。当前幂等 key 仍未加入调用 Actor 作用域；Inbox 来源事件投影、Reminder 和其他自动事件的完整去重契约仍待后续编排纵切实现。
...
- 断开网络后，当前已实现的 Task/Assignment/manual 提交验收、Client 基础资料/Project 客户关联，以及手工 Inbox 受理分诊与已有 Task 关系主链路可用；完整 v0.1 Inbox 批量拆分/分派/自动解决、Reminder/来源投影、Client 活动/附件和备份闭环仍待交付。
- 每个业务状态有且只有一个事实源。
- 跨模块写操作具备事务、幂等和冲突检测。
- 任何来源事件重扫和重启后不重复创建工作。
