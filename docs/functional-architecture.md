...
> 文档版本：2.19
> 日期：2026-08-28
> 依据：[PRD v8.6](opc-workspace-PRD.md)
...
- 含文件业务 ZIP 导出在维护写锁内完整生成后才响应：`manifest.json` 记录 source、业务 JSON 及全部 active 受控文件的安全相对路径、size/SHA-256，`files/` 携带正文；复制时任一文件漂移都会整体失败并清理 staging。该包排除 SQLite、identity、store marker 和运行维护事实，目前不开放直接导入。
...
- 当前仍未实现 Focus 原生反馈、Client 外部活动来源/回访/财务、运行期数据库不可写等其他系统故障来源、重复提醒、Agent Runtime、含文件包导入和非空目标冲突导入，因此完整工作编排仍是部分完成。Focus 分析与含文件 ZIP 导出已交付；已登记来源投影到 Inbox；Sidecar 脱敏轮转日志已交付，Tauri 壳日志、打开日志入口和恢复页仍未交付。Project 产出区仍只读聚合，正文/下载/验收继续由 Task 领域处理。
...
| [数据管理](modules/data-management.md)     | SQLite 与本地文件 | 已实现受控文件一致性、备份恢复、失败 Inbox、业务 JSON/含文件 ZIP 导出与空工作区安全 JSON 导入；含文件/冲突合并导入仍规划 | 当前文件安全、已校验备份、恢复状态、失败 Inbox 与便携导出 |
...
- v0.1：Reminder 到期、显式 follow-up Task Artifact、Task 阻塞、提前 24 小时 Task 临期、备份四类操作失败、数据库启动/迁移和 Sidecar 启动失败已交付；运行期数据库不可写等其他系统故障待实现。
...
已交付：Task D1 + D2 / Workflow Event / manual Submission / Artifact store
  ...
  → 已交付数据库启动/迁移与 Sidecar 启动失败补偿、Sidecar 脱敏轮转日志；继续运行期数据库故障 / Tauri 壳日志与恢复
...
- 断开网络后，当前已实现的 Task/Assignment/manual 提交验收、Project/Client、手工 Inbox、Reminder、Task 编排、follow-up Artifact/Task 阻塞/Task 临期/备份四类操作失败/数据库启动与迁移/Sidecar 启动失败来源投影、备份闭环、业务 JSON 与含文件 ZIP 导出均可用；运行期数据库等其他系统故障、Client 外部活动来源和原生通知仍待交付。
- 每个业务状态有且只有一个事实源。
- 跨模块写操作具备事务、幂等和冲突检测。
- 任何来源事件重扫和重启后不重复创建工作。
