...
- Roadmap Milestone 继续以 `roadmap_milestones`、关联 Project 与派生 Task 汇总为唯一事实；列表白名单 `sort=target_date` 在分页前按纯日期和 ID 稳定排序，缺省仍保留季度手工顺序。左侧路线图导航角标统计目标日期落在今天起 7 天内的 planned/active 节点数作提醒，点击进路线图页，不写 Today 副本。路线图卡片仍以 `?milestone=<id>` 打开同一个最新详情读模型，关闭/编辑只清理该参数并保留其他 URL 上下文。右侧概览为双形态：展开时与主工作面同处一个框架（共用外边框、圆角与阴影，内部以 1px 发丝线分隔），带页签停靠面板（概要/子智能体/文件/浏览器），宽度可在 240–480px 之间拖拽或键盘调整且仅作用于当前会话；收起时仅 AI 助手页显示独立悬浮状态卡（环境/子智能体/后台进程/来源）。专注控制已移至左侧 FocusMiniCard。
