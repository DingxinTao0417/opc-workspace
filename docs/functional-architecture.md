...
> 文档版本：2.17
> 日期：2026-08-28
> 依据：[PRD v6.7](opc-workspace-PRD.md)
...
- SQLite 当前为 schema v26：schema v11–v22 依次交付 Focus、Inbox/Reminder/编排、设置/保存视图、Client 扩展和 Project 笔记/附件；schema v23–v26 为 follow-up Task Artifact、Task 阻塞、Task 临期和备份创建失败的系统维护增加 Inbox 来源身份、索引、不可变与删除/incident 协调 guards，不回填历史事实或创建 demo 数据。
...
- 当前仍未实现 Focus 高级分析/原生反馈、Client 外部活动来源/回访/财务、其他系统故障来源、重复提醒、Agent Runtime、数据导入和含文件外部导出包，因此完整工作编排仍是部分完成。显式 follow-up Artifact、Task 阻塞、提前 24 小时 Task 临期和备份创建失败已投影到 Inbox；迁移失败、Sidecar 启动前失败和数据库不可写仍未投影。Project 产出区仍只读聚合，正文/下载/验收继续由 Task 领域处理。
...
| [收件箱](modules/inbox.md)                 | owner 手工录入、Reminder 到期、已有/新建 Task、follow-up Artifact、Task 阻塞/临期与备份创建失败 | 已交付受理分诊、来源上下文、Task 编排、自动结清/重开、来源删除协调、强制例外和运营计数/风险深链                | 输出 Event、实时进度及 Today/Sidebar 计数；其他系统故障来源待 T-11E           |
...
POST /api/v1/backups 创建失败
  ...
  → Sidecar 尽力投影 source_entity_type=system_maintenance、source_entity_id=backup:create
  → 同一 open/tracking incident 复用；resolve/dismiss 后再失败开新条目
  → owner 在 Inbox 查看固定安全说明并打开数据与备份
```

该链路已由 schema v26/T-11E 第四项交付。payload 只含 `component / operation / failure_code / occurred_at / message`，不保存 Go error、本机路径、备份 note、Token 或请求正文。投影失败只记内部日志，不改变备份错误响应。迁移失败、Sidecar 启动前失败、数据库不可写和完整诊断包仍未实现。
...
- v0.1：Reminder 到期、显式 follow-up Task Artifact、Task 阻塞、提前 24 小时 Task 临期和备份创建失败已交付；迁移/Sidecar 启动前/数据库不可写等其他系统故障待实现。
...
当前可重试业务命令继续保存请求摘要与首次响应。Reminder 到期使用 `reminder:<id>:due`；follow-up Artifact 使用 `task-artifact:<artifact-id>:followup`；Task 阻塞使用 `task:<task-id>:blocked:<block-version>`；Task 临期使用 `task:<task-id>:due:<due-at>`；备份创建失败使用 `system:backup:create:<incident-id>`，且同一 `backup:create` 在 open/tracking 时只允许一个活动 incident。Artifact/阻塞分别与 Task 提交/block Event 同事务，临期来源由启动补偿和 15 秒扫描器按 Task+截止时点稳定投影；备份失败投影尽力执行且不改变 `BACKUP_CREATE_FAILED` 响应。命令重放、重复扫描和重启都不重复创建活动事项，改期、重复阻塞和归档后再失败按新事实形成独立事项。幂等 key 仍未加入调用 Actor 作用域；其他系统故障和业务来源仍待后续纵切。
...
| 备份创建失败                    | 数据管理 + Inbox                 | 同步命令保留现有数据库/Artifact、清理未发布 staging，并返回带 request ID 的 `BACKUP_CREATE_FAILED`；随后尽力创建 `system_maintenance` Inbox Item，只记录固定安全字段。投影失败不改变备份错误。桌面诊断包、迁移失败、Sidecar 启动前失败和数据库不可写仍未实现 |
...
已交付：Task D1 + D2 / Workflow Event / manual Submission / Artifact store
  ...
  → 已交付：批量拆分 / 人工分派 / 自动解决；T-11E 已交付 follow-up / 阻塞 / 临期 / 备份创建失败
...
- 断开网络后，当前已实现的 Task/Assignment/manual 提交验收、Project/Client、手工 Inbox、Reminder、Task 编排、follow-up Artifact/Task 阻塞/Task 临期/备份创建失败来源投影、备份闭环和业务 JSON 导出均可用；其他系统故障、Client 外部活动来源和原生通知仍待交付。
- 每个业务状态有且只有一个事实源。
- 跨模块写操作具备事务、幂等和冲突检测。
- 任何来源事件重扫和重启后不重复创建工作。
