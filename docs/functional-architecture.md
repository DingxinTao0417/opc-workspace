...
> 文档版本：2.19
> 日期：2026-08-28
> 依据：[PRD v7.9](opc-workspace-PRD.md)
...
- 当前仍未实现 Focus 标签分布/热力图/最佳时段/原生反馈、Client 外部活动来源/回访/财务、运行期数据库不可写等其他系统故障来源、重复提醒、Agent Runtime、数据导入和含文件外部导出包，因此完整工作编排仍是部分完成。Focus 项目时间分布已按 Task 查询时当前归属派生；显式 follow-up Artifact、Task 阻塞、提前 24 小时 Task 临期、Project 完成节点、备份四类操作失败、数据库启动/迁移和 Sidecar 启动失败已投影到 Inbox；完整日志/恢复页仍未交付。Project 产出区仍只读聚合，正文/下载/验收继续由 Task 领域处理。

### 3.2 目标扩展

- v0.1：在已交付的 Task/Project/Client、Actor/Assignment、D2、Focus Core+D1+D2a+D2b 日期范围回顾/项目分布、手工 Inbox 受理/分诊、Reminder、Inbox Task 编排、基础备份闭环和业务 JSON 导出上，继续完成客户/项目增强、非 Reminder 来源投影和桌面可靠性；Focus 标签分布/热力图/最佳时段、重复/原生通知独立延后。
