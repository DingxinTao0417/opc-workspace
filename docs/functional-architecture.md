...
> 文档版本：2.39
> 日期：2026-08-29
> 依据：[PRD v9.16](opc-workspace-PRD.md)
...
- v0.1：在已交付的 Task/Project/Client、共享服务端搜索 Client 选择器、Actor/Assignment、D2、Focus Core+D1+D2a+Project 详情读取+D2b 日期范围回顾/项目/标签/小时分布与二维热力图、手工 Inbox 受理/分诊、Reminder、Inbox Task 编排、基础备份闭环和业务 JSON 导出上，继续完成真实浏览器/窄屏/大数据量验收、客户/项目增强、非 Reminder 来源投影和桌面可靠性；Focus 原生反馈、重复/原生通知独立延后。
...
| [任务](modules/tasks.md)                   | Project、Client 服务端选择结果、Actor、Inbox 关系与来源                                                                 | 唯一工单、六态生命周期、完成条件、Submission/Artifact、manual 验收、直属子任务汇总待验收，以及沿 Task→Project→Client 当前关系执行客户筛选与阻塞来源投影                 | Project 进度、Task 事件、阻塞 Inbox Item、后续 Inbox 进度与 Focus 工时                                                                   |
| [项目](modules/projects.md)                | Client 服务端选择结果、Task、Focus、受控文件 store                                                                      | 已实现资料、生命周期、Client 选择/筛选、任务/Artifact 聚合、笔记、附件、项目级 Focus 报告/历史、活动时间线、follow-up/阻塞/Task 临期及完成节点→Inbox                    | 完成收尾事项进入 Inbox；complete/reopen 向事件时关联 Client 输出只读系统活动；Focus 按 Task 当前归属只读派生，产出操作仍直达共享任务详情 |
