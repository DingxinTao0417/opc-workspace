...
> 当前实现基线：app v0.1.0 / API v1 / SQLite schema 56（052–056 为 AI 助手远程/本地 Provider、会话表、推理思考流列、Provider kind 列与长期记忆表，独立轨道交付；此前的 v44 基线声明已过期）
...
| [AI 助手](modules/ai-assistant.md)         | 用户显式输入与用户配置的 Provider（远程 API key 存 OS 安全存储；本地部署为回环 OpenAI 兼容端点、无密钥）                | 流式只读问答（harness 运行时，生产零工具）、语义建任务建议卡片（确认后经既有任务 API 创建）                                                                                          | 会话/消息本地事实、已创建任务静态引用卡片（点击跳任务详情）；AI 失败不投影 Inbox                                                                                 |
...
- AI 助手是唯一例外并经明确授权（ADR-004/005/006）：Sidecar 以用户配置的 Provider 出站（远程 API key 存 OS 安全存储；本地部署为回环 OpenAI 兼容端点、无密钥），仅外发该次用户输入与确认记忆的注入文本；不进日志/导出。Harness 生产不注册任何工具；自进化只发生在数据层（人工确认的记忆），提示词与工具集永远代码所有；其余边界不变。
...
已交付：Task D1 + D2 / Workflow Event / manual + child_rollup Submission / Artifact store
  ...
  → v0.4 财务 / 发票 / 客户回访
  → 本地知识库
  → AI 助手（首个远程只读会话、本地部署与记忆纵切已按独立轨道先行交付，无知识库检索能力；长程上下文压缩与记忆工具设计已固化（ADR-007，实施待启动）；上下文读取与来源引用仍依赖上述顺序）
```
