...
> 文档版本：2.1
> 日期：2026-08-28
> 依据：[PRD v3.6](opc-workspace-PRD.md)
...
- React 已具备三栏框架、今日/任务/项目/客户能力、手工 Inbox 三视图/详情/分诊时间线与已有 Task 活动/历史关系管理，以及共享持久化 Session 驱动的 FocusPage、RightOverview、ticker 和恢复弹窗；任务页已接服务端分页/搜索/筛选、根任务树、标签、批量和计划组排序，Today 已接所选精确日期/未排期活动组的按钮式持久排序、恢复默认、同组拖拽与失败回滚，Project 已接 Client 选择/筛选。
...
- 设置事实层已接 schema v16 与 `GET/PATCH /api/v1/settings`：固定 workspace/general/appearance/focus 四个非敏感模块，缺失行由 GET 返回未存储默认值，PATCH 以模块版本原子保存并追加不含设置值的审计事件。前端仍读取 localStorage，Query 接入与一次性迁移属于下一纵切。
...
| ------------------------------------------ | ---------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| [今日](modules/today.md)                   | Task、Focus、Inbox 派生统计                                                  | 当日执行入口、聚合展示、精确计划组按钮/拖拽排序与乐观预览                                                   | 原子任务排序/操作、开始专注、打开收件箱                                                 |
...
| [设置](modules/settings.md)                | 当前 localStorage 偏好、schema v16 设置 API、Actor API 与 `/health`          | 本地偏好界面、版本化非敏感设置事实、person 管理和只读运行诊断；前端设置 API 接入与旧值迁移待实现            | 布局、主题、Actor、运行版本、备份和桌面行为                                             |
...
已交付：Task D1 + D2 / Workflow Event / manual Submission / Artifact store
  ...
  → 已交付：Focus 持久化/Task 工时/IANA Today Focus 统计、Today 完整日期分组/导航/按钮式及同组拖拽排序；跨日期改期继续
  → 备份恢复 / 桌面日志与故障恢复
