...
- Go `/health` 与版本化 `/api/v1`，统一请求 ID、错误响应、Bearer 鉴权和 Origin 白名单；设置“关于”展示真实 app/commit/API/schema/SQLite 状态，“运行诊断”对照 Tauri Sidecar 生命周期并可复制脱敏摘要
...
- Focus Core A+B+C+D1+D2a 和 D2b 日期范围回顾/项目/小时/热力图：持久化 Session/interval、任务绑定、暂停/继续/停止/取消、服务端绝对时间、15 秒心跳、启动/刷新恢复、`If-Match`/幂等、精确秒数结转 Task 完整分钟、IANA 当地日 completed-only Today/周期统计、终态历史分页、7/30 天/本月/最多 93 天自定义趋势与 Streak、按 Task 当前归属的项目时间分布、DST 安全 24 小时分布/最佳时段与周几×小时二维热力图、Task 详情按需专注记录，以及共享前端循环/恢复 UI
...
受控任务 D1/D2、计划/筛选/保存视图、Project 任务树/平铺及项目内搜索/状态筛选/服务端分页、笔记/附件/产出聚合/活动时间线与完成节点→Inbox、Client 本地事实、Focus Core/日期范围回顾/项目/小时分布与二维热力图、Today、统一本地搜索、设置/受控头像/脱敏运行诊断、全局渲染错误恢复、备份恢复/迁移前自动备份/业务 JSON，以及 Inbox/Reminder/Task 编排已经交付；备份操作、数据库启动/迁移和 Sidecar 启动失败均会投影安全的系统维护 Inbox Item。Focus 标签分布/原生反馈、客户外部来源/回访/财务、重复提醒、完整日志/启动恢复页、全局系统快捷键及三平台安装包仍属于后续实现。[PRD v8.1](docs/opc-workspace-PRD.md) 记录了这条边界。
...
- [文档索引](docs/README.md)
- [产品需求文档（PRD v8.1）](docs/opc-workspace-PRD.md)
...
[PRD v8.1](docs/opc-workspace-PRD.md) 是范围、目标契约与当前实施状态依据。v0.1 基座已交付 Actor/Assignment、Task D1/D2、任务计划/筛选/保存视图、Project 任务树/平铺及项目内搜索/状态筛选/服务端分页与 Client 本地纵切、Focus Core/日期范围回顾/项目与小时分布/二维热力图、Today、统一本地搜索、设置/受控工作区头像/脱敏运行诊断、全局渲染错误恢复、手工一致性备份恢复、迁移前自动回滚包、业务 JSON、Inbox/Reminder/Task 编排，以及显式 follow-up Artifact、Task 阻塞、提前 24 小时 Task 临期、Project 完成节点、备份四类操作失败、数据库启动/迁移失败和 Sidecar 启动失败来源投影；明确未交付 Focus 标签分布/原生反馈、任务/项目看板、内容日历、客户外部活动/回访/财务、完整日志/启动恢复页、重复/原生通知、Agent Runtime、导入、恢复诊断、自动化规则、SQLCipher、云同步、AI 助手或知识库。
