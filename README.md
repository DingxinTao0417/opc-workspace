...
- SQLite schema v24、WAL、外键、busy timeout 和嵌入式版本化迁移；v3–v22 交付项目、Task/Actor/D2、Client、Focus、Inbox/Reminder/设置/保存视图及项目笔记/附件事实，v23–v24 追加 follow-up Artifact 与 Task 阻塞→Inbox 来源身份、索引及删除协调 guards
...
受控任务 D1/D2、计划/筛选/保存视图、Project 笔记/附件/产出聚合/活动时间线、Client 本地事实、Focus Core、Today、统一本地搜索、设置、备份恢复/业务 JSON，以及 Inbox/Reminder/Task 编排已经交付；schema v23–v24 进一步交付显式 follow-up Artifact 与 Task 阻塞来源投影、上下文和删除协调。受控头像文件、Focus D2b、客户外部来源/回访/财务、任务临期和系统故障来源、重复提醒、迁移前自动备份、恢复诊断、全局系统快捷键及三平台安装包仍属于后续实现。[PRD v6.3](docs/opc-workspace-PRD.md) 记录了这条边界。
...
- [文档索引](docs/README.md)
- [产品需求文档（PRD v6.3）](docs/opc-workspace-PRD.md)
...
迁移 SQL 位于 `services/sidecar/internal/database/migrations/`，随 Sidecar 二进制嵌入。当前最新版本为 schema v24；启动时按文件版本顺序执行，并记录到 `schema_migrations`。v6–v22 交付 Task/Actor/D2、Client、Focus、Inbox/Reminder、设置/保存视图及 Project 扩展；v23 增加 follow-up Artifact 来源保护，v24 增加 Task 阻塞事件来源身份、索引、不可变和删除协调保护；两者都不回填旧事实或创建 demo 数据。后续从 `025_*` 追加。每个连接启用：
...
[PRD v6.3](docs/opc-workspace-PRD.md) 是范围、目标契约与当前实施状态依据。v0.1 基座已交付 Actor/Assignment、Task D1/D2、任务计划/筛选/保存视图、Project/Client 本地纵切、Focus Core、Today、统一本地搜索、设置、手工一致性备份恢复/业务 JSON、Inbox/Reminder/Task 编排，以及显式 follow-up Artifact 与 Task 阻塞来源投影/删除协调；明确未交付受控头像文件、Focus D2b、任务/项目看板、内容日历、客户外部活动/回访/财务、任务临期和系统故障来源、重复/原生通知、Agent Runtime、导入、迁移前自动备份、恢复诊断、自动化规则、SQLCipher、云同步、AI 助手或知识库。
