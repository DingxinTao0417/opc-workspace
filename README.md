...
- SQLite schema v12、WAL、外键、busy timeout 和嵌入式版本化迁移；v3–v10 依次增加项目、Task、Actor/Assignment/Event、Submission/Artifact 与 Client 事实，v11 重建 Focus Session 并增加 interval 与 Task 精确秒数账本，v12 追加手工 Inbox Item 事实
...
受控任务生命周期 D1、T-18D D2、客户基础资料/Project 客户关联、Focus Core A+B+C，以及 T-11A1/T-11B 手工 Inbox 受理分诊已经交付。Focus D 的 Session 历史、周报、Streak、高级分析、原生通知、托盘和 DND 引导仍延后；客户活动与附件、Actor 显式关联、项目事件/Inbox 来源投影、Inbox Task 关联/拆分/分派/自动解决、Reminder、Sidebar/Today Inbox 计数、备份/恢复、全局系统快捷键、签名离线更新和三平台安装包也仍属于后续实现；在线 Updater 不在当前阶段。[PRD v2.4](docs/opc-workspace-PRD.md) 记录了这条边界。第一阶段不引入多人登录、云同步、远程通知或线上 Agent；`person` 仅作本机责任记录。客户回访、收入/支出、发票 CRUD/PDF、AI 助手和本地知识库归入更后续版本。开发数据库默认从空业务数据开始；完成的绑定 Focus Session 会通过精确秒数账本把新增完整分钟写入 Task `actual_minutes`，Project 沿既有 Task 聚合读取结果。
...
- [文档索引](docs/README.md)
- [产品需求文档（PRD v2.4）](docs/opc-workspace-PRD.md)
...
当前 Inbox API 只接受 `kind / source_entity_type / resolution_policy = manual` 的手工条目，不接受来源 ID 或事件键。列表支持 `inbox / snoozed / archive` 三视图、标题/摘要搜索、优先级和分页；`meta.unread_total` 始终统计全局当前待处理视图的未读，不受当前视图或筛选影响。`read_at`、`snoozed_until` 与主状态相互独立；resolve/dismiss 要求 1–2,000 字符原因、清除稍后但不隐式已读，未读终态仍可直接 read；reopen 清除终态和稍后事实、回到 open，并保留 read/triaged。PATCH 与单条命令使用 `ETag`/`If-Match`；创建、命令和 read-all 支持幂等快照。read-all 提交列表 `snapshot_at` 作为 `through_created_at` 时间截止，只标记创建与最后更新时间均不晚于 cutoff、且按该 cutoff 仍属于待处理可见范围的未读；截止后发生编辑、分诊、重开等更新的条目会保守跳过。这不是历史状态重建，也不是由 created_at+稳定序列组成的严格游标。当前没有 Task 关系/拆分/分派、Reminder、来源投影、自动解决或 Inbox 删除路由。
...
迁移 SQL 位于 `services/sidecar/internal/database/migrations/`，随 Sidecar 二进制嵌入。当前最新版本为 schema v12；启动时按文件版本顺序执行，并记录到 `schema_migrations`。schema v6–v10 依次交付 Task、Actor/Assignment/Event、Submission/Artifact 和 Client 事实；`011_focus_sessions.sql` 重建 Focus Session，移除旧分钟/布尔双写字段，新增 interval 与 Task 精确秒数 ledger，并兼容映射历史 Session 而不二次增加 Task 工时；`012_inbox_items.sql` 以加法迁移追加手工 Inbox Item、查询索引及 nullable `source_event_key` 部分唯一约束，v11→v12 不改写既有事实。需要临时关闭外键的迁移由迁移器锁定单连接，在事务提交前执行 `foreign_key_check`，成功或失败都恢复外键；一致性失败会整体回滚。每个连接启用：
...
[PRD v2.4](docs/opc-workspace-PRD.md) 是范围、目标契约与当前实施状态依据。v0.1 基座已交付 Actor/Assignment、Task D1/D2、Client/Project 基础纵切、Focus Core A+B+C 和手工 Inbox 受理/分诊；明确未交付 Focus D、任务/项目看板、内容日历业务、客户活动/附件/回访、收入/支出/发票业务、Inbox Task 关系/拆分/分派/自动解决、Reminder 与来源投影、Agent Runtime、备份/恢复、自动化规则、白噪音、网站屏蔽、SQLCipher、多币种、移动端、云同步、AI 助手或知识库。对应页面骨架、预留字段或规划 API 不代表完整工作流已经可用。
