...
> [!IMPORTANT]
> opc-workspace 仍处于积极开发阶段。当前基线为 app v0.1.1、API v1、SQLite schema v44；Windows x64 已能生成未签名的本地测试安装包，但尚未达到正式发布、签名和多平台完整验收标准。
...
- AI 助手与知识库尚未实现；当前版本不调用 AI/LLM，也不会运行 Agent。
...
flowchart TB
    ...
    Data["本地事实层<br/>SQLite v44 · 受控文件 · 备份 · 日志"]
...
更完整的模块关系和安全边界见[整体功能架构](./docs/functional-architecture.md)与[本地 Agent Runtime ADR](./docs/adr/003-local-agent-runtime-security.md)。
