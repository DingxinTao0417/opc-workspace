...
- 本地 Agent 已有 Windows 内置文本执行器、任务详情 Run 界面与条件式 manual-review 文本提交（ADR-027）。设置显式启停，任务启动前核对真实 Adapter/Actor/Assignment；缺少初始化条件时禁用并引导设置，无负责人时明示分派，已有他人负责人不静默替换。支持显式本地/在线 OpenAI 兼容模型；成功不等于任务完成。其他平台、external 和文件流仍待，本轮初始化修复不包含真实模型验收。
