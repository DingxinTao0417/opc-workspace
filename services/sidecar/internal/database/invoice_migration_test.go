...
func TestInvoiceFactsMigrationOpensEmptyWorkspaceAtV46(t *testing.T) {
	...
	defer store.Close()
	if gate != nil || store.SchemaVersion != 54 {
...
func TestInvoiceFactsMigrationGatesV45AndPreservesLegacyInvoices(t *testing.T) {
	...
	if gated.SchemaVersion != 45 || gate == nil || gate.CurrentVersion != 45 || gate.TargetVersion != 54 || !reflect.DeepEqual(gate.PendingVersions, []int{46, 47, 48, 49, 50, 51, 52, 53, 54}) {
		_ = gated.Close()
	...
	if store.SchemaVersion != 54 {
		t.Fatalf("invoice schema version = %d, want 54", store.SchemaVersion)
	}
