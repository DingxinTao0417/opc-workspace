...
func TestInvoiceFactsMigrationOpensEmptyWorkspaceAtV46(t *testing.T) {
	...
	if gate != nil || store.SchemaVersion != 49 {
		t.Fatalf("empty invoice workspace schema=%d gate=%#v, want schema 49 without a gate", store.SchemaVersion, gate)
	}
...
func TestInvoiceFactsMigrationGatesV45AndPreservesLegacyInvoices(t *testing.T) {
	...
	if gated.SchemaVersion != 45 || gate == nil || gate.CurrentVersion != 45 || gate.TargetVersion != 49 || !reflect.DeepEqual(gate.PendingVersions, []int{46, 47, 48, 49}) {
		_ = gated.Close()
	...
	if store.SchemaVersion != 49 {
		t.Fatalf("invoice schema version = %d, want 49", store.SchemaVersion)
	}
