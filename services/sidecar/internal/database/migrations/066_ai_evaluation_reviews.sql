-- 066: append-only local human review decisions for one exact evaluation
-- quality-group snapshot. Free-text reasons stay outside portable business
-- exports; no row can enable a Provider or change evaluation facts.
CREATE TABLE ai_evaluation_reviews (
    id TEXT PRIMARY KEY,
    ...
    suite_key TEXT NOT NULL CHECK (suite_key IN ('smoke', 'full')),
