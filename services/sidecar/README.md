...
`GET /api/v1/focus-sessions` and `GET /api/v1/stats/focus` accept an optional `project_id` without changing the unfiltered response contract. The value must be a lowercase canonical UUID; an empty, invalid, uppercase, or otherwise non-canonical value returns `400 INVALID_PROJECT_ID`, and a canonical missing Project returns `404 PROJECT_NOT_FOUND`. Archived Projects remain readable. Validation and Project existence checks do not add a schema migration: the app remains on API v1 / SQLite schema v29.
