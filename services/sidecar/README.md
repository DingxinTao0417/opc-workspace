...
- one-time plus daily/weekly/monthly local Reminder CRUD, optimistic concurrency, cancellation, startup compensation, IANA/DST/month-end-aware periodic due scanning, offline folding, and exactly-once Reminder-to-Inbox/next-occurrence projection;
...
Router startup synchronously projects overdue scheduled rows before readiness, then scans every 15 seconds in stable batches of 100. Each projection transaction finds or creates one `kind=reminder` Inbox Item using `reminder:<id>:due`, appends the system Inbox event, marks the Reminder fired with the Inbox ID, appends the system Reminder event, and for daily/weekly/monthly rules creates the one next occurrence by IANA local-calendar arithmetic. The unique event key, series/occurrence constraints, conditional update, and transaction make repeated scans and restarts safe. Native OS notifications, remote delivery, weekday/custom rules, and free-form business-source rules are not implemented.
