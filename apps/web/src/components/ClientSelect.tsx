...
type ClientOption = Pick<Client, "id" | "name" | "status"> & {
  fallback?: boolean;
};
...
function uniqueClients(items: Client[]): Client[] {
...
export function ClientSelect({
  ...
  const listboxId = `client-select-${useId().replace(/:/g, "")}`;
  ...
  const selectedQuery = useClientQuery(value || null);

  const pageClients = useMemo(
    () => uniqueClients(optionsQuery.data?.items ?? []),
    [optionsQuery.data?.items],
  );

  const selectedOption = useMemo<ClientOption | null>(() => {
    if (!value) return null;
    if (selectedQuery.data?.id === value) return selectedQuery.data;
    if (rememberedSelection?.id === value) return rememberedSelection;
    const pageClient = pageClients.find((client) => client.id === value);
    if (pageClient) return pageClient;
    return {
      id: value,
      name: selectedName?.trim() || "已选客户",
      status: "inactive",
      fallback: true,
    };
  }, [
  ...
  const resultOptions = useMemo(
    () => pageClients.filter((client) => client.id !== value),
    [pageClients, value],
  );
  ...
  return (
    <div
      className={`client-select client-select--${variant}`}
      onBlur={(event) => {
    ...
    >
      ...
      >
        ...
        <input
          ...
          onFocus={openMenu}
          onKeyDown={onKeyDown}
          placeholder={open ? "搜索客户…" : emptyLabel}
          ref={inputRef}
          role="combobox"
          spellCheck={false}
        ...
        {!open && value && selectedOption ? (
          ...
          >
            {selectedOption.fallback
              ? "当前选择"
              : clientStatusLabels[selectedOption.status]}
          </span>
        ) : null}
        {value && !disabled ? (
          <button
                ...
                {selectedOption ? (
                  <>
                    ...
                    <ClientOptionButton
                      ...
                      onSelect={() => selectClient(selectedOption)}
...
function ClientOptionButton({
  ...
  return (
    <button
      ...
      aria-label={`${option.name}${option.fallback ? "，当前选择" : `，${clientStatusLabels[option.status]}`}`}
    ...
    >
      <span className="client-select__avatar" aria-hidden="true">
        {option.name.trim().charAt(0).toUpperCase() || "客"}
      </span>
      <span className="client-select__option-copy">
        ...
        <small>
          {option.fallback
            ? "当前选择 · 详情暂不可用"
            : clientStatusLabels[option.status]}
        </small>
      </span>
